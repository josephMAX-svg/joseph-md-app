# PENDIENTES DE JOSEPH · régimen v5.7

**Generado:** 08-sep-2026 · **D1 = miércoles 9-sep-2026**
...
**Ya resuelto — no lo busques aquí:** el SQL de ENCAPS está aplicado (100 filas, 9-sep→29-ene, backup
`study_schedule_bk_0908`; **no re-ejecutarlo**: un segundo DELETE+INSERT pisaría ese backup) · los 12
overlays de hito y las series del Calendar están re-fechados · `PALMERTON_METODO_COMPLETO.md` y
`PALMERTON_POR_MATERIA.md` v3 están escritos · `gen_encaps_intensivo_2027.js` ya tiene `--base 2026-09-09`
· el SHIP del vibecoding está alineado con SYNAPSE · la memoria (`MEMORY.md`, `coach-stance-encaps.md`)
ya declara el estado vigente.

---

## 🔴 ANTES DEL D1 (hoy 8-sep / mañana 9-sep temprano)
...
- [ ] **Crear los 10 sub-decks `APEX::DERMA::A…X`** en Anki escritorio (o `createDeck` por AnkiConnect) y
      ...
      decks con nombre suelto al importar. *(Derma — antes de la sesión d1, mié 9-sep)*
...
- [ ] **Cerrar cada sesión de ENCAPS desde el 9-sep** (rutina diaria):
...
> verificado con `get_event` que ANKI AM ya dice "régimen v5.7: 95 días desde mié 9-sep ≈ 594h totales"
> y que las 16 series (10 de estudio + 6 de rutina) llevan la etiqueta v5.7.
...
- [ ] **Alinear `D:\agente_estudio\config\fases.json`** — sigue con `FASE_4_ENCAPS_DOMINANTE.is_current_phase
      ...
      `FASE_7.inicio = 2026-09-09`. Verificar antes que `orquestador.py` lee ese campo. *(sistema)*
- [ ] **Reponer stock del banco ENCAPS antes de la semana 3 (21-sep)** — avisos reales del runner al
      ...
      sub-eje `tipos_vigilancia` solo 2Q propias, y tras el banco del 10-sep quedan **0Q de I-3**.
...
- [ ] **Importar `DATA/BUSINESS/ANKI_COLA/LIVIANO_mecanismo.csv`** en Anki (Archivo → Importar,
      tabulador, deck en columna 3, etiquetas en columna 4, FSRS) — **antes del D6 (mié 16-sep)**, tope
      D16 (30-sep). *(LIVIANO)*
...
> ~~Corregir `DATA/SYNAPSE/vibecoding_proyectos.json`~~ — **HECHO el 8-sep**: el JSON fuente ya dice
> "plan de 95 días" y "7 corrimientos (31-ago→9-sep)", así que la próxima regeneración no lo revierte.
...
- [ ] **Crear los overlays de hitos Research en Google Calendar** (naranja, como los NBME): C-2 24-sep ·
      C-6 14-oct · CR-1 30-oct · T-8 19-nov · X-7 29-dic · CR-9 1-feb · R10 19-feb · R43 29-jun-2027.
...
- [ ] **Reservar el Free 120 en el Prometric de Lima** para el **vie 22-ene-2027 (D95)** — hay que hacerlo
...
- [ ] **Verificar DeLorenzi 2017** (ASJ, PMID 28333326; requiere acceso OUP/UF) — las **UI de
      ...
      **no fijar esas cifras en Anki**. Deadline: antes del drill de oclusión **d19 = jue 29-oct**.
      *(Derma)*
...
*El plan las agenda como átomos D39-D44 (2 → 9-nov), pero cierran los 2 pendientes ROJOS que arrastras
...
- [ ] **D39 · lun 2-nov — Registro sanitario DIGEMID**: consultar el portal público (URL sin verificar),
...
- [ ] **D40 · mar 3-nov — Condición de venta por molécula** (con receta / receta retenida) leída del
      registro → columna `condicion` + flujo receta → farmacia → paciente en el CRM.
- [ ] **D41 · mié 4-nov — 2 cotizaciones escritas y fechadas** (cadena + independiente) por
      presentación/dosis → columna `precioFarmacia`; **recalcular el "medicamento 3 m = S/ 3,600"** del
      value stack (`LIVIANO_OFERTA`).
- [ ] **D42 · jue 5-nov — Dictamen de legalidad del magistral** (QF + abogado de salud: **nombres a
...
- [ ] **D44 · lun 9-nov — Tiempo permitido fuera de refrigeración** por producto (ficha técnica) → guion
      de cadena de frío del kit de bienvenida (Anexo A del protocolo).
- [ ] **D38 · vie 30-oct — Rellenar `LIVIANO_PROTOCOLO_CLINICO_v1.md` §2**: criterio numérico de
...
- [ ] **Al usar `--sim100 2025-2 / 2025-1A / 2024-2A` en la intensiva**: **122 ítems** de esos exámenes ya
      ...
      *(ENCAPS)*
- [ ] **Antes del d19 (jue 29-oct) · importar `DATA/DERMATOLOGIA/ANKI_COLA/PLANTILLA_SESION.txt`** en Anki
      (Archivo → Importar, notetype Basic); a partir de ahí copiar la plantilla por sesión
      (`SESION_dNN_<fecha>.txt`). *(Derma)*
- [ ] **Antes del d45 (mié 13-ene-2027) · decidir y comprar el dermatoscopio de bolsillo** (DermLite o
...
- [ ] **Post-D89 (jue 14-ene-2027) · revisión por par del protocolo LIVIANO** por un médico con
...
| 2 | **Viernes de nivel 4** (20-30Q mixtos de sistemas dominados) desde **S11 / D53 = vie 20-nov** | No activado (requiere flag en `gen_usmle_v5.js` + regenerar) |
...
| 4 | **Free 120 + maratón de resistencia**: la única ventana sin tocar franjas es **D93, mié 20-ene** (NBME 33 + 3 bloques de *flagged* ≈5 h) **cediendo Research/Derma/AURUM ese día** | Solo el Free 120 en D95 (vie 22-ene) |
| 5 | **Protocolo D-2 / D-1** (lun 25 y mar 26-ene: Anki maduro + flagged / solo Anki vencido), **fuera del plan de 95 días** | Sin escribir |
...
- [ ] **Régimen · ¿se acepta seguir con 5 días L-V o se recupera algo?** El plan bajó de **~606 h (97 d) a
      ~594 h (95 d)** y el colchón consumido desde el 31-ago ya son **7 días hábiles**. Escrito en README §4
      y en CALENDARIO "Reglas de reprogramación" punto 4. *(USMLE)*
- [ ] **Puente MIR ↔ Step 1** — Cardiología MIR (D5, 15-sep) ahora precede a Cardiovascular Step 1 (D6,
      16-sep) por **1 solo día**: el "prime en español 7 días antes" no se cumple en ese bloque; Psiquiatría
      arranca **el mismo día** (21-dic) que *Psychiatry & Behavioral*. Los otros 9 bloques quedan entre +6 y
...
- [ ] **Materias sin columna vertebral en el corpus Palmerton — decidir la fuente sustituta** *(USMLE)*:
      - **13 días completos**: Micro/ID (D57-D64, 26-nov→7-dic), MSK/Reuma+Derma (D70-D72, 15-17 dic) y
      ...
      - **Cardio D8 (18-sep) y D15 (29-sep)**: bucles presión-volumen, Frank-Starling con curvas de retorno
      ...
      - **Bioestadística (D77, jue 24-dic)**: el corpus cubre la mitad conceptual pero **no la de cálculo**
...
- [ ] **MIR · modo "reducido" de enero** — el mantenimiento del 4 al 22-ene está fijado por defecto en
      `gen_mir_mantenimiento.js` (`REDUCIDO_HASTA = 2027-01-22`) como **solo Anki + 10Q**. Si prefieres el
...
- [ ] **AURUM · política de semanas después del 25-dic.** La "semana" del plan (5 hábiles consecutivos)
      deja de alinearse con el viernes: las **variantes LIVIANO D80-D115 y los PITCH v5-v7 caen en lunes y
      miércoles**. Se respeta el invariante "1 de cada 5 drills". Si quieres viernes estrictos, hay que
      cambiar la política en `gen_aurum_plan.js`. *(AURUM)*
- [ ] **Business · los 2 últimos OUTPUT caen lun 4 y mar 5-ene-2027** (S16 documento de cierre + retro del
      formato L) porque las lecturas se agotan antes. Si quieres que **todos** los outputs sean viernes,
      hay que añadir 2 lecturas META en `plan_pulso_v3_L.json`. *(Business)*
- [ ] **LIVIANO · posición de los drills de cifras.** Quedaron en **D36/D57/D76/D87**; el análisis pedía
      D38/D58/D75 (pero D38 es la Síntesis M2 y D75 es viernes-caso). Si prefieres D38 exacto, mover
...
- [ ] **Renombrar `DATA/ENCAPS/CALENDAR_SEGMENTOS_V5_6.md` → `_V5_7.md`?** El contenido ya es v5.7, el
...
- [ ] **Semanas 13-20 (30-nov → 22-ene) sin misión** en 04:15, 12:30 y PC sáb/dom: SYNAPSE termina el
      26-nov y el vibecoding el 27-nov, pero el Step 1 sigue 8 semanas más — justo la fase más dura →
...
**ENCAPS**
- [ ] **Stock de preguntas por código/sub-eje = 0** frente a una demanda de ~2.100-2.550Q en 102 días:
      etiquetar los 400 ítems reales con clave (2024-2A / 2025-1A / 2025-2) con `codigo` v3 + `sub_eje` +
      `formato_pretest` y pre-generar los sets por lotes fuera de la hora. `gaps_v3b_encaps` #1.
...
- [ ] **No hay taper de examen en Derma** — lo contrario de lo que Palmerton exige en la última semana:
      d45/d46 (13/15-ene) chocan con el NBME 31 y el GO/NO-GO, y **d47-d52 (19-ene → 2-feb) traen contenido
      NUEVO de estética en plena semana del Step 1** (d51 = 27-ene es el día objetivo del examen) → swap de
