# MÉTODO PALMERTON — DE LA A A LA Z (síntesis maestra · 8-sep-2026 · §12 remapeado al régimen v5.13 el 16-sep-2026)
...
# 12. MAPEO AL PLAN v5.13 DE JOSEPH

> **Fuente de los datos de esta sección**: parseo directo de `src/lib/usmleStep1Daily.ts` con Node — versión original 8-sep-2026, **re-parseada el 10-sep-2026 (v5.9), el 12-sep-2026 (v5.10), el 14-sep-2026 (v5.11), el 15-sep-2026 (v5.12) y el 16-sep-2026 (v5.13)** (`DAILY_META`, `FRANJAS`, `USMLE_NIVELES`, `USMLE_GATE`, `USMLE_TAPER`, `DIAS`) y de `src/lib/usmleScores.ts` (`HITOS_ONTRACK`, `TIPO_ERROR_INFO`). **Ninguna fecha de esta sección está estimada de memoria.**
>
> **v5.13 (16-sep-2026; regla de Joseph): no se fusionó ni se recortó nada.** El miércoles 16-sep tampoco se estudió (duodécimo corrimiento: 31-ago→16-sep sin estudiar = 13 hábiles perdidos); el temario salió 1:1 y el desfase se absorbió alargando el plan hasta el **lun 1-feb-2027**; 11 de los 12 hitos conservan su fecha y solo cambian de D#. **El UWSA1 vuelve a cambiar de fecha** (mié 16-sep, ya pasada → **jue 17-sep**) y sigue siendo el D1. 🔴 **El plan rebasa la ventana 25-29 ene: D94 = vie 29-ene = D-2 (última sesión de banco) y D95 = lun 1-feb = D-1 DENTRO del plan → el examen SALE de la ventana: target MAR 2-FEB-2027**; sáb 30 y dom 31-ene quedan libres (solo Anki vencido) y el D-1 es sesión mínima por la mañana + ritual de test-day. Joseph debe agendar/reprogramar el Prometric y confirmar que su eligibility period cubre el 2-feb (si no: extenderlo); la alternativa es recortar temario (decisión suya, §12.6 #5). ⚠ **Desde hoy, cada día no estudiado mueve el examen un día hábil más (o exige recortar temario).**
>
> 🔴 **El D1 es el UWSA1** (jue 17-sep: baseline antes de estudiar nada, como prescribe Palmerton) y el **contenido arranca el vie 18-sep = D2**.
>
> **Segunda pasada del 12-sep (tarde), vigente en v5.13**: se implementaron, sin tocar horario, temario ni fechas, tres divergencias de §12.6 — **viernes de nivel 4 desde S11** (en v5.13: solo D62, vie 11-dic; el vie 27-nov = D52 es 2.º día de Heme/Onc → nivel 1), **taper D94-D95 + D-1 + test day** (`USMLE_TAPER`; en v5.13 el D-1 es el lun 1-feb = D95, dentro del plan) y el **protocolo de burnout como REGLA** (`gateHito` → 'ALERTA BURNOUT'). El total de Q es **5.560** (el multiconjunto de subtemas no cambia con el corrimiento). Además, el repaso anclado de las 07:15 muestra "MIR precedió esta semana: <asignatura>" con el tag Anki compartido `sys::<sistema>` y el D73 (dermato Step 1, mar 29-dic) cuenta doble con los 8 átomos Derma `step1`.

## 12.1 El marco: qué dice el plan hoy

- **95 días hábiles · D1 = jueves 17-sep-2026 (UWSA1) → D95 = lunes 1-feb-2027 (D-1 dentro del plan).** El plan rebasa **la ventana 25-29 ene 2027** (D90-D94 caen dentro de ella; D94 = vie 29-ene = D-2, última sesión de banco) → **examen target MARTES 2-FEB-2027, fuera de la ventana** (`DAILY_META.examenTarget = '2027-02-02'`); **sáb 30 y dom 31-ene libres** (solo Anki vencido) y **D95 = lun 1-feb = D-1** (`descansoD1 = '2027-02-01'`: sesión mínima AM ≤2 h + ritual). Sábados y domingos **libres**; skips extra: 25-dic, 31-dic, 1-ene.
- **Fases** (`faseDe`): **A · Contenido por sistemas D1-D81** (17-sep → **12-ene**; el contenido empieza en D2, vie 18-sep; **el UWSA2, D79 vie 8-ene, cae DENTRO de la Fase A** y el cierre es el segundo día doble de Bioquímica, D81 mar 12-ene) · **B · Banco intensivo D82-D84** (13-ene → 15-ene: 2 días de random timed + NBME 31 la cierra) · **C · Sprint final D85-D95** (18-ene → **1-feb**). D86 (mar 19-ene) y D88 (jue 21-ene, random timed + Mehlman), D90 (lun 25-ene), D91 (mar 26-ene), D92 (mié 27-ene) y D93 (jue 28-ene) siguen siendo días de banco (`system = Banco intensivo`: sistema débil #2 · incorrects 2.ª pasada + AMBOSS 200 mitades 1 y 2) alojados dentro de la semana del sprint.
...
- **Distribución de niveles UWorld** (v5.13): Fase A → **28 días de nivel 1** (30 Q), **41 días de nivel 2** (40 Q), **3 días de nivel 3** (40 Q), **1 viernes de nivel 4** (D62, 40 Q: mixto de sistemas dominados desde S11) y 8 hitos en formato nivel 5 (4.160 Q); Fase B (D82-D84) → **2 días de nivel 4** (D82 · D83, 80 Q) y **1 de nivel 5** (D84 NBME 31) = 360 Q; Fase C (D85-D95) → **2 días de nivel 4** (D86 · D88, 80 Q) y **9 de nivel 5** = 1.040 Q (**D94 y D95 = taper, 20 Q cada uno**). Total: **5.560 Q** (idéntico a la v5.12: el corrimiento no toca volumen ni contenido; el multiconjunto de subtemas es el mismo, verificado con `node` contra `git show HEAD~1`).
...
## 12.3 Fase A (D1-D81 · jue 17-sep → mar 12-ene): los niveles 1→3 y el gate del 80%
...
**Calendario real de la Fase A por sistema** (parseado de `DIAS`, v5.13):

| Sistema | Días | Fechas reales | Nivel(es) |
|---|---|---|---|
| **UWSA1 — baseline** | **D1** | **jue 17-sep** | 5 (160 Q) |
| Fundamentos (Pathoma 1-3) | D2 · D3 | vie 18-sep → lun 21-sep | 1 (D2 es viernes con el sistema recién abierto: sin bloque de sistema completo) |
| Immunology | D4 · D5 | mar 22-sep → mié 23-sep | 1 |
| **Cardiovascular (1.ª parte)** | D6-D11 | jue 24-sep → jue 1-oct | 1 → 2 (D7, vie 25-sep, es el 2.º día del sistema: sin viernes de nivel 3; el bloque de sistema se hace del sistema anterior) |
| **NBME 25** | **D12** | **vie 2-oct** | 5 (200 Q) |
| Cardiovascular (cierre) | D13-D16 | lun 5-oct → jue 8-oct | 2 (aterosclerosis/isquemia · SCA · IC + shock · valvulopatías/miocardiopatías/congénitas) |
| **Respiratory** | D17-D22 | vie 9-oct → vie 16-oct | 1 → 2 → **3 (D22, vie 16-oct: TEP/TVP + HTP + ARDS + cáncer de pulmón)** (D17, vie 9-oct, es el 1.er día del sistema → nivel 1; el bloque de sistema de ese viernes se hace de Cardio) |
| **Renal (1.ª parte)** | D23-D26 | lun 19-oct → jue 22-oct | 1 → 2 (sin viernes propio: su viernes es el NBME 26) |
| **NBME 26** | **D27** | **vie 23-oct** | 5 (200 Q) |
| Renal (cierre) | D28 · D29 | lun 26-oct → mar 27-oct | 2 (glomerulares · AKI + ERC + litiasis + poliquistosis) |
| **Gastrointestinal** | D30-D36 | mié 28-oct → jue 5-nov | 1 → **3 (D32, vie 30-oct: intestino delgado/malabsorción/celiaquía/EII)** → 2 (bloque íntegro detrás del hito) |
| **Endocrine** | D37-D41 | vie 6-nov → jue 12-nov | 1 → 2 (D37, vie 6-nov, es el 1.er día del sistema: sin viernes de nivel 3; el bloque de sistema se hace de GI) |
| **NBME 27** | **D42** | **vie 13-nov** | 5 (200 Q) |
| **Nervous System** | D43-D50 | lun 16-nov → mié 25-nov | 1 → 2 → **3 (D47, vie 20-nov: ictus)** → 2 (bloque íntegro detrás del hito) |
| **Hematology & Oncology** | D51-D56 | jue 26-nov → jue 3-dic | 1 → 2 (D52, vie 27-nov, es el 2.º día del sistema: sin viernes de nivel 4 ni 3) |
| **NBME 28** | **D57** | **vie 4-dic** | 5 (200 Q) |
| **Microbiology / ID** | D58-D64 | lun 7-dic → mar 15-dic | 1 → 2 → **4 (D62, vie 11-dic: virus DNA/herpes/hepatitis virales — único viernes de nivel 4 del plan, S13, mixto de los 9 sistemas ya cerrados)** → 2 (bloque íntegro detrás del hito) |
| Reproductive (1.ª parte) | D65 · D66 | mié 16-dic → jue 17-dic | 1 |
| **NBME 29** | **D67** | **vie 18-dic** | 5 (200 Q) |
| Reproductive (cierre) | D68-D70 | lun 21-dic → mié 23-dic | 2 (gineco-oncología · mama + aparato masculino + próstata · ITS + anticoncepción + amenorreas + SOP) |
| Musculoskeletal / Rheum (+ derma Step 1) | D71-D73 | jue 24-dic → mar 29-dic | 1 → 2 (bloque íntegro; el vie 25-dic es skip) · **D73 = cuenta doble Derma ↔ Step 1**: el pre-test 08:15 y el repaso 07:15 apuntan a `deck:APEX::DERMA tag:step1` + fallos del ledger derma (8 átomos ya estudiados en sep-nov: psoriasis, eccemas, ampollosas, SJS/TEN/DRESS, bacterianas, HSV/VZV/VPH, BCC/SCC, melanoma); el contenido USMLE del día no cambia |
| **NBME 30 — cierre del contenido de 2026** | **D74** | **mié 30-dic** | 5 (200 Q) |
| **Psychiatry & Behavioral** | D75-D77 | lun 4-ene → mié 6-ene | 1 → 2 (bloque íntegro detrás del hito: ánimo/psicóticos · ansiedad/personalidad/infancia · psicofármacos) |
| Biostats / epi / ética (`system` = Psychiatry & Behavioral) | D78 | jue 7-ene | 2 |
| **UWSA2** | **D79** | **vie 8-ene** | 5 (160 Q) — **dentro de la Fase A** |
| Biochemistry (día doble 1: metabolismo) | D80 | lun 11-ene | 1 |
| Biochemistry (día doble 2: biología molecular + genética + farmacología general) — **cierre de Fase A** | D81 | mar 12-ene | 1 |
> En v5.13 el **NBME 30 (D74, mié 30-dic) va delante de Psiquiatría entera (D75-D77)**, de Biostats (D78, jue 7-ene) y de los dos días dobles de Bioquímica, que **el UWSA2 ya no parte**: **UWSA2 el vie 8-ene (D79)**, **D80 el lun 11-ene** y **D81 el mar 12-ene-2027** (jue 31-dic y vie 1-ene son skip). La Fase A muere el **12-ene**. El NBME 25 (D12) parte Cardio (aterosclerosis, SCA, IC y valvulopatías detrás), el NBME 26 (D27) parte Renal (glomerulares y AKI/ERC detrás) y el NBME 29 (D67) parte Repro (gineco-onco, mama/próstata e ITS/anticoncepción detrás); Neuro (D43-D50), Micro (D58-D64), Psiquiatría (D75-D77), GI (D30-D36) y MSK (D71-D73) van íntegros detrás de su hito.
...
- **Milestone 2 (subtema timed)**: los **41 días de nivel 2**, es decir, desde el 3.er día de cada sistema.
- **Milestone 3 (sistema completo timed, 20Q)**: en v5.13 son **3 días**, todos viernes sin hito de S1-S10 con el sistema ya ≥3 días abierto — **D22 (vie 16-oct, Respiratory: TEP/TVP + HTP + ARDS + cáncer de pulmón)**, **D32 (vie 30-oct, Gastrointestinal: intestino delgado/malabsorción/celiaquía/EII)** y **D47 (vie 20-nov, Nervous System: ictus)**. Los otros cuatro viernes sin hito de S1-S10 (**D2 18-sep** Fundamentos, **D7 25-sep** Cardiovascular, **D17 9-oct** Respiratory, **D37 6-nov** Endocrine) caen con un sistema recién abierto → van a nivel 1 y el bloque de sistema se hace **del sistema anterior**; **Cardiovascular pierde su gate de sistema en la v5.13 y Respiratory lo recupera** (en la v5.12 lo tenían Cardio, GI y Neuro), Renal y Repro siguen sin viernes propio (sus viernes son el NBME 26 y el NBME 29). *(Cambio v5.12 → v5.13: al correr todo +1 día hábil, el viernes de cada bloque cae sobre el subtema anterior — el N3 pasa de Cardio curvas PV a Resp TEP/TVP, de GI colon a GI intestino delgado, de Neuro convulsiones a Neuro ictus. Los 8 hitos de Fase A siguen usando formato nivel 5 como medición.)*
- **Milestone 4 (mixto de ≥3 sistemas)**: como **dosis diaria** en la eval de las 18:00 (10Q mixtas timed) durante toda la Fase A; **desde S11 también el viernes a las 11:00** (divergencia #6, flag `VIERNES_N4_DESDE_SEMANA = 11`): en v5.13 solo **D62 (vie 11-dic, Micro/ID)** = 20-30Q timed mixtos de los 9 sistemas ya cerrados (Fundamentos, Immuno, Cardio, Resp, Renal, GI, Endo, Neuro, Heme/Onc) + 10Q tutor del subtema de virus DNA/herpes/hepatitis virales — el subtema no cambia, cambia el formato del bloque (`DIAS[62].franjaNota`); el vie 27-nov (D52) es el 2.º día de Heme/Onc → nivel 1, sin bloque mixto; y como fase propia en D82 · D83 (+ D86 · D88 dentro del sprint). Los viernes 25-dic y 1-ene son skip y el viernes de Repro es el NBME 29, así que en v5.13 el flag afecta solo a **D62**; es una regla, no una lista.
...
- **Regla de los 2 minutos y "juez, no abogado"**: 18:00 desde **D2 (vie 18-sep)** — se pueden entrenar desde el primer día sin coste.
- **Ancla CAD**: el subtema de **enfermedad coronaria** que Palmerton recomienda como primera validación cae en **D13 (lun 5-oct, "aterosclerosis + isquemia + angina" — ya detrás del NBME 25)** y **D14 (mar 6-oct, SCA)**; el sistema cardiovascular arranca el **jue 24-sep**.
...
|---|---|---|---|---|---|
| **UWSA1 (baseline)** | D1 | **jue 17-sep-2026** | 160 | — (cualquier valor sirve) | Baseline temprano: rompe el miedo a evaluarse y fija prioridades (§9.8). **Con la regla del 5%/mes, un baseline <45-48% haría matemáticamente inalcanzable el 70%** → es el "Goldilocks check" real, no el 15-ene |
| **NBME 25** | D12 | **vie 2-oct-2026** | 200 | **51%** | Pase seguro de la forma 25 = 64% |
| **NBME 26** | D27 | **vie 23-oct-2026** | 200 | **54%** | forma 26 = 64% |
| **NBME 27** | D42 | **vie 13-nov-2026** | 200 | **57%** (gate 1 ECFMG ≥55%) | forma 27 = 65% |
| **NBME 28** | D57 | **vie 4-dic-2026** | 200 | **61%** | forma 28 = 63% |
| **NBME 29** | D67 | **vie 18-dic-2026** | 200 | **63%** (gate 2 ≥60%) | forma 29 = 64% |
| **NBME 30 (cierre Fase A)** | D74 | **mié 30-dic-2026** | 200 | **65%** | **umbral de seguridad Palmerton ≈95% de pase**; forma 30 = la más difícil (65%) |
| **UWSA2** | D79 | **vie 8-ene-2027** | 160 | "low risk" | **El UWSA sobreestima 10-15+ puntos**: resistencia y señal secundaria; **la fecha la decide el NBME** · en v5.13 cae **dentro de la Fase A** (delante de los dos días dobles de Bioquímica, que ya no parte) |
| **NBME 31 — GO/NO-GO** | D84 | **vie 15-ene-2027** | 200 | **68%** | 68% está entre el 65% (≈95%) y el 70% (≈99%) de Palmerton; el criterio de **2 NBME consecutivos** es **más robusto** que su regla |
| **NBME 32** | D85 | **lun 18-ene-2027** | 200 | **68%** | confirma el GO |
| **NBME 33** | D87 | **mié 20-ene-2027** | 200 | **68%** | confirma el GO |
| **Free 120** | D89 | **vie 22-ene-2027** | 120 | **70%** (heurística comunitaria, **no** cifra Palmerton) | Sweet spot 1-2 semanas antes ✔ (D-11 del examen del mar 2-feb) · **rendirlo en el Prometric real** = decisión pendiente (§12.6) |

**Cadencia**: los 7 hitos NBME/UWSA1 de Fase A caen cada ~3 semanas (y el UWSA2, D79, cierra la serie de Fase A) → dentro del rango operativo 1-4 semanas de Palmerton, en su extremo lento (justificado: las formas NBME son finitas, 25-33). Once de las 12 fechas no se han movido en ningún corrimiento; el UWSA1 sigue al D1 (vie 11-sep → lun 14-sep → mar 15-sep → mié 16-sep → **jue 17-sep**).
...
|---|---|---|---|
| **D79** | vie 8-ene | **UWSA2** (160 Q) — hito de la Fase A (`faseDe(79) = 'A'`), delante de los dos días dobles de Bioquímica | Resistencia + señal secundaria |
| **D80 · D81** | lun 11-ene → mar 12-ene | Bioquímica días dobles 1 y 2 (metabolismo · biología molecular + genética + farmacología general) — **D81 = cierre de la Fase A** | Nivel 1 (últimos subtemas nuevos del plan) |
| **D82 · D83** | mié 13-ene → jue 14-ene | Random timed **2×40Q (80 Q/día)** + revisión profunda + **sistema débil #1** según NBME — **Fase B** | **Nivel 4** (mixto de sistemas dominados) + el reparto de Melody: *"distribute more time to your weakest subjects"* |
| **D84** | vie 15-ene | **NBME 31** + decisión **GO/NO-GO** — cierra la Fase B | §9.1: nunca presentarse si los dos últimos NBME no respaldan |
| **D85** | lun 18-ene | **NBME 32** + repaso FA sistemas 1-5 — abre la Fase C | Nivel 5 |
| **D86 · D88** | mar 19-ene · jue 21-ene | Random timed 2×40Q + **sistema débil #2** (D88 con Mehlman HY del sistema) — `system = Banco intensivo` dentro del sprint | **Nivel 4** (últimos días de mixto de sistemas dominados) |
| **D87** | mié 20-ene | **NBME 33** + repaso FA sistemas 11-14 | Nivel 5 · **candidato natural a la maratón de resistencia** (§12.6 #4) |
| **D89** | vie 22-ene | **FREE 120 oficial** + logística del examen + cierre | Free 120 en la ventana correcta (D-11 del examen del mar 2-feb) |
| **D90 · D91** | lun 25-ene · mar 26-ene | **uWorld incorrects (2.ª pasada) + flagged** (nunca re-hacer acertadas) + sistema débil #3 — `system = Banco intensivo` dentro del sprint | **Nivel 5** · coherente con "una sola pasada, nunca reset": los incorrects/flagged sí |
| **D92 · D93** | mié 27-ene · jue 28-ene | **AMBOSS 200 Concepts (mitades 1 y 2)** + incorrects (80 Q) — siguen siendo `system = Banco intensivo` dentro de la semana del sprint | Nivel 5 · AMBOSS 200 = repaso de conceptos ya vistos, no banco nuevo |
| **D94** | **vie 29-ene** | Repaso FA sistemas 6-10 + **Anki marathon** + incorrects — **TAPER D-2 · ÚLTIMA SESIÓN DE BANCO (20 Q)** | §8.3: solo Anki **maduro** + 20 Q flagged/incorrects ya vistos, sin bloque timed ni AMBOSS; cero preguntas y cero tarjetas nuevas; dormir ≥7 h ya desde hoy (`DIAS[94].franjaNota`, `USMLE_TAPER.d94`) |
| **sáb 30 · dom 31** | sáb 30-ene · dom 31-ene | **Libres** (entre D94 y D95): sin banco, solo Anki vencido | §8.3 · descanso |
| **D95 (D-1)** | **lun 1-feb** | Rapid review de First Aid + Anki + laboratorio de dudas — **TAPER D-1 · DENTRO del plan (20 Q)**: sesión MÍNIMA solo por la mañana (≤2 h); tarde de logística (`USMLE_TAPER.d95`/`.dMenos1`, `DAILY_META.descansoD1 = '2027-02-01'`) | §8.3 D-1: Anki maduro/vencido + 20 Q flagged con los mejores esquemas e imágenes, rapid review FA; cero preguntas nuevas, cero tarjetas nuevas, ningún bloque timed; **prohibido** temas densos y abrir First Aid "para ver cuánto sé"; **tarde**: permiso impreso + digital, 2 ID con el nombre exacto, bolsas Ziploc numeradas, ruta al Prometric; nada después de las 17:00 (journaling, ejercicio suave, visualización, cena con proteína); somnífero nunca por primera vez; dormir ≥7-8 h, alarma y ruta comprobadas |
| **(examen)** | **mar 2-feb** | USMLE Step 1 — target v5.13, **FUERA de la ventana 25-29 ene** (`DAILY_META.examenTarget = '2027-02-02'`, `USMLE_TAPER.examen`; agendar/reprogramar Prometric y confirmar eligibility period) | Test day (§8.4): desayuno proteína + grasa, café de siempre, tutorial → +15 min de descanso, bloques 1-2 → 10' · 3-4 → 10' · 5 → almuerzo 20-30' · 6 → 10' · 7; nunca revisar entre bloques ni salir a mitad de bloque; premio post-test |
**Stress sets** (§7.5): el plan los sitúa a las **05:00 en Fases B y C** (D82 en adelante, desde el **mié 13-ene**), lo que respeta el criterio de entrada de su autor (≥60% del contenido cubierto, últimas 2-3 semanas) — con la Fase A cerrada el **12-ene** (D81), el contenido está cubierto al 100% desde el primer stress set.

**Cese de contenido nuevo**: Palmerton pide parar preguntas y tarjetas nuevas **1-2 semanas antes**. Con examen el **mar 2-feb**, eso equivale a partir del **19-26 de enero** → el plan lo adelanta a **NBME 31 (D84, vie 15-ene) en adelante**, es decir, toda la Fase C. El plan trata **AMBOSS 200 Concepts como repaso** de conceptos ya vistos, no como banco nuevo, y la Fase C ya sólo usa flagged. **El taper es explícito**: D94 y D95 bajan a 20 Q (flagged + Anki maduro, `TAPER_ACTIVO` en el generador); **en la v5.13 el D-1 vuelve a caer DENTRO del plan (D95 = lun 1-feb, sesión mínima + ritual), con el finde 30-31 ene libre entre la última sesión de banco (D94) y el D-1**: el plan rebasa la ventana (D90-D94 = 25-29 ene) y el examen pasa al mar 2-feb; el test day está escrito paso a paso en `USMLE_TAPER` (todo tomado de §8.3-§8.4; lo A VERIFICAR sigue marcado: repasos 200-300/día, costo del Free 120 en Lima). **Volver a la ventana 25-29 ene exigiría recortar temario — decisión de Joseph, no del plan; y cada día más sin estudiar mueve el examen otro hábil.**

**Burnout (§9.12) como REGLA del plan** (divergencia #29 → §E-7, 12-sep tarde): si **2 hitos consecutivos con mínimo** quedan bajo su mínimo on-track (§12.4) **y** hay síntomas → **3-5 días con sólo Anki AM (30-45 min de tarjetas viejas) + sueño**, frenar QBank y adquisición; el corrimiento determinista +1 día hábil absorbe la pausa (no se recorta temario) y se reanuda por el gate del 80%. ⚠ En v5.13 esa pausa mueve el examen más allá del mar 2-feb: activar el protocolo implica, además, reprogramar la fecha (plan B feb-mar, mismo eligibility period). En la app: `usmleScores.gateHito` → `'ALERTA BURNOUT'` (banner en `UsmleHub`); `BURNOUT_PROTOCOLO` lleva los pasos de §9.12.

## 12.6 Divergencias que quedan abiertas

> Estas son **exactamente** las de `PALMERTON_DIVERGENCIAS_PLAN.md` (§E). Este documento **no las contradice ni las resuelve**: las hereda. **Estado al 16-sep (v5.13)**: las #2, #5 y #7 quedaron **implementadas** el 12-sep (no tocan horario, metas ni temario) y siguen vigentes remapeadas; las demás siguen a decisión de Joseph. **Ninguna decisión abierta se resolvió con el corrimiento v5.13** (la #5 mantiene su forma: "rendir el mar 2-feb vs recortar temario"; el D-1 vuelve a quedar dentro del plan como D95).
...
2. ✅ **Viernes de nivel 4** (mixto de 20-30Q de sistemas dominados) desde S11 — **implementado** con el flag `VIERNES_N4_DESDE_SEMANA = 11` en `gen_usmle_v5.js` (§12.3); en v5.13 afecta solo a **D62 (vie 11-dic)** (el vie 27-nov = D52 es 2.º día de Heme/Onc → nivel 1); contenido idéntico, sólo cambia el formato del bloque de las 11:00. *(divergencia #6)*
...
4. **Free 120 en el Prometric real de Lima + maratón de resistencia**: (a) reservar la sesión de práctica en Prometric (costo internacional citado por el cuaderno: **$155 — A VERIFICAR (16-sep)**; registro con hasta 7 días de espera); (b) la maratón de 7+ bloques **no cabe** en la tarde de un viernes de plan y los sábados son libres por régimen → única opción sin tocar franjas: **NBME 33 (D87, mié 20-ene) + 3 bloques de flagged** (≈5 h), cediendo Research/Derma/AURUM ese día. *(divergencia #21)* — ⏳ decisión Joseph.
5. ✅ **Protocolo taper / D-1 / test day** — **implementado** (§12.5) y **remapeado en v5.13**: **D94 (vie 29-ene) = D-2, última sesión de banco** y **D95 (lun 1-feb) = D-1 DENTRO del plan** (taper de 20 Q; D95 = sesión mínima AM + ritual §8.3); **el plan rebasa la ventana 25-29 ene → el examen sale de ella: target MAR 2-FEB-2027**, con sáb 30 y dom 31-ene libres y test day con el plan de bloques de §8.4; código en `USMLE_TAPER`. ⚠ **Decisión de Joseph (16-sep; sustituye a la del 15-sep)**: **rendir el mar 2-feb** (agendar/reprogramar Prometric y **confirmar que el eligibility period cubre el 2-feb**, extendiéndolo si hace falta) **o recortar temario** para volver a la ventana. Y desde hoy cada día perdido mueve el examen otro hábil (o exige recortar). *(divergencia #22)*
...
8. **Si el UWSA1 del jue 17-sep sale <40%**: bajar S1-S2 (semanas del 14 y del 21-sep) a **20 Q/día** al estilo Jay/Melody. *(divergencia #2)* — ⏳ se decide el mismo jue 17-sep con el % real.
...
*Documento generado el 8-sep-2026 sobre los extractos v3 del cuaderno "STEP 1 · Palmerton Engine"; **§12 remapeado el 10-sep-2026 al régimen v5.9, el 12-sep-2026 al v5.10, el 14-sep-2026 al v5.11, el 15-sep-2026 al v5.12 y el 16-sep-2026 al v5.13**. Todas las fechas y los D# de §12 provienen del parseo de `src/lib/usmleStep1Daily.ts` (95 días, D1 = 2026-09-17 → D95 = 2027-02-01 = D-1; examen target 2027-02-02) y de `src/lib/usmleScores.ts`; no hay fechas estimadas.*
