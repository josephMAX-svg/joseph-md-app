# MÉTODO PALMERTON — DE LA A A LA Z (síntesis maestra · 8-sep-2026 · §12 remapeado al régimen v5.16 el 26-sep-2026)

> **Qué es este documento.** La síntesis completa del método de **Alec Palmerton, MD** (Stanford Med · anestesiología en Harvard-MGH · Step 1 = 270, top 0,1%) tal como está publicado en su canal de YouTube y en **yousmle.com**, ordenado para ejecutarse. Es el documento maestro del bloque USMLE Step 1 de Joseph: la doctrina (§1-§11) y su traducción exacta al plan **v5.16** día por día (§12).
>
...
12. **MAPEO AL PLAN v5.16 DE JOSEPH** — cada técnica en su bloque horario, fase, días y fechas reales + divergencias abiertas
...
# 12. MAPEO AL PLAN v5.16 DE JOSEPH

> **Fuente de los datos de esta sección**: parseo directo de `src/lib/usmleStep1Daily.ts` con Node — versión original 8-sep-2026, **re-parseada el 10-sep-2026 (v5.9), el 12-sep-2026 (v5.10), el 14-sep-2026 (v5.11), el 15-sep-2026 (v5.12), el 16-sep-2026 (v5.13), el 19-sep-2026 (v5.14), el 22-sep-2026 (v5.15) y el 26-sep-2026 (v5.16)** (`DAILY_META`, `FRANJAS`, `USMLE_NIVELES`, `USMLE_GATE`, `USMLE_TAPER`, `DIAS`) y de `src/lib/usmleScores.ts` (`HITOS_ONTRACK`, `TIPO_ERROR_INFO`). **Ninguna fecha de esta sección está estimada de memoria.**
>
> **v5.16 (26-sep-2026; segundo CORRIMIENTO RÍGIDO con la regla de Joseph de la v5.15): no se fusionó ni se recortó nada y tampoco se tocó el reparto contenido↔hitos.** Tampoco se estudiaron el mié 23, el jue 24 ni el vie 25-sep (decimoquinto corrimiento: 31-ago→25-sep sin estudiar = 20 hábiles perdidos); el plan ENTERO — contenido **y** los 12 hitos — se desplazó en bloque +3 hábiles y el desfase se absorbió alargando el final hasta el **mié 10-feb-2027**. **Los hitos siguen sin estar anclados por fecha: corren con el plan y conservan su D# exacto**, así que ningún NBME pierde días de contenido por delante; donde un plan tenía un fin clavado, **se amplían días** en vez de perder sesiones. ⚠ Efecto colateral (inverso al de la v5.15): con el D1 en lunes **8 de los 12 hitos vuelven a caer en viernes** (NBME 25-28 y 30, UWSA2, NBME 31, Free 120; NBME 29 y 32 en lunes, NBME 33 en miércoles) y queda **1 viernes de nivel 4** (D60) en vez de dos. **El UWSA1 vuelve a cambiar de fecha** (mié 23-sep, ya pasada → **lun 28-sep**) y sigue siendo el D1; con el D1 en lunes las 20 semanas del plan coinciden con las de calendario (S1 = 28-sep → 2-oct · S20 = 8-10 feb + examen). 🔴 **El plan rebasa la ventana 25-29 ene: D94 = mar 9-feb = D-2 (última sesión de banco) y D95 = mié 10-feb = ÚLTIMO DÍA DEL PLAN y D-1 REAL → examen target JUE 11-FEB-2027, al día siguiente, SIN fin de semana en medio** (el sáb 6 y el dom 7-feb caen entre D92 y D93); el D-1 es sesión mínima por la mañana + ritual de test-day. **Herencia de la v5.14: el contenido cierra el jue 21-ene (D81) y el NBME 31 GO/NO-GO cae al día siguiente (D82, vie 22-ene), sin banco de consolidación delante** (los 2 random timed van en D84 y D86; Fase B = D82-D86; Fase C = D87-D95 desde el Free 120). Joseph debe agendar/reprogramar el Prometric y confirmar que su eligibility period cubre el 11-feb (si no: extenderlo); la alternativa es recortar temario (decisión suya, §12.6 #5). ⚠ **Desde hoy, cada día no estudiado mueve el examen un día hábil más (o exige recortar temario).** *(Histórico v5.15, 22-sep: D1 = mié 23-sep → D95 = vie 5-feb, examen lun 8-feb con el finde 6-7 feb libre; 9 hitos en martes y 2 viernes N4, D53 y D63.)*
>
> 🔴 **El D1 es el UWSA1** (lun 28-sep: baseline antes de estudiar nada, como prescribe Palmerton) y el **contenido arranca el mar 29-sep = D2**.
>
> **Segunda pasada del 12-sep (tarde), vigente en v5.16**: se implementaron, sin tocar horario ni temario, tres divergencias de §12.6 — **viernes de nivel 4 desde S11** (en v5.16 es UNO: D60, vie 18-dic, S12, Micro/ID — con el D1 en lunes los hitos vuelven a ocupar viernes y S13-S14 no tienen viernes hábil), **taper D94-D95 + D-1 + test day** (`USMLE_TAPER`; en v5.16 el D-1 real es el mié 10-feb = D95, último día del plan, con el examen al día siguiente y sin finde en medio) y el **protocolo de burnout como REGLA** (`gateHito` → 'ALERTA BURNOUT'). El total de Q es **5.560** (el multiconjunto de subtemas no cambia con el corrimiento: dif 0). Además, el repaso anclado de las 07:15 muestra "MIR precedió esta semana: <asignatura>" con el tag Anki compartido `sys::<sistema>` y el D74 (dermato Step 1, mar 12-ene) cuenta doble con los 8 átomos Derma `step1`.

## 12.1 El marco: qué dice el plan hoy

- **95 días hábiles · D1 = lunes 28-sep-2026 (UWSA1) → D95 = miércoles 10-feb-2027 (último día del plan y D-1 real).** El plan rebasa **la ventana 25-29 ene 2027** (D83-D87 caen dentro de ella; D94 = mar 9-feb = D-2, última sesión de banco) → **examen target JUEVES 11-FEB-2027, fuera de la ventana** (`DAILY_META.examenTarget = '2027-02-11'`); **el examen es al día siguiente del D95, sin fin de semana en medio** (el sáb 6 y el dom 7-feb caen entre D92 y D93), y **D95 = mié 10-feb = D-1 real** (`descansoD1 = '2027-02-10'`: sesión mínima AM ≤2 h + ritual). Sábados y domingos **libres**; skips extra: 25-dic, 31-dic, 1-ene.
- **Fases** (`faseDe`): **A · Contenido por sistemas D1-D81** (28-sep → **21-ene**; el contenido empieza en D2, mar 29-sep; **el UWSA2, D77 vie 15-ene, cae DENTRO de la Fase A** y el cierre es el segundo día doble de Bioquímica, D81 jue 21-ene) · **B · Banco intensivo D82-D86** (22-ene → 28-ene: **el NBME 31 (D82, vie 22-ene) la abre el día siguiente al cierre de contenido, sin banco de consolidación delante**; NBME 32 D83 · random timed D84 · NBME 33 D85 · random timed D86) · **C · Sprint final D87-D95** (29-ene → **10-feb**, abre con el Free 120). D88 (lun 1-feb) y D89 (mar 2-feb, random timed + Mehlman), D90 (mié 3-feb), D91 (jue 4-feb), D92 (vie 5-feb) y D93 (lun 8-feb) siguen siendo días de banco (`system = Banco intensivo`: sistema débil #2 · incorrects 2.ª pasada + AMBOSS 200 mitades 1 y 2) alojados dentro de la semana del sprint.
- **Carga de preguntas de Fase A** (suma de `qDia` D1-D81): **4.160 Q**, de las cuales **1.520** corresponden a los 8 hitos (UWSA1 160 + 6 NBME × 200 + UWSA2 160) y **2.640** a los 73 días de contenido.
- **Distribución de niveles UWorld** (v5.16): Fase A → **28 días de nivel 1** (30 Q), **39 días de nivel 2** (40 Q), **5 días de nivel 3** (40 Q), **1 viernes de nivel 4** (D60, 40 Q: mixto de sistemas dominados desde S11) y 8 hitos en formato nivel 5 (4.160 Q); Fase B (D82-D86) → **2 días de nivel 4** (D84 · D86, 80 Q) y **3 de nivel 5** (D82 NBME 31 · D83 NBME 32 · D85 NBME 33) = 760 Q; Fase C (D87-D95) → **2 días de nivel 4** (D88 · D89, 80 Q) y **7 de nivel 5** = 640 Q (**D94 y D95 = taper, 20 Q cada uno**). Total: **5.560 Q** (idéntico a la v5.14 y la v5.15: el corrimiento rígido no toca volumen ni contenido; el multiconjunto de subtemas es el mismo, verificado con `node` por `(system, sub)`: dif 0).
...
## 12.3 Fase A (D1-D81 · lun 28-sep → jue 21-ene): los niveles 1→3 y el gate del 80%
...
**Calendario real de la Fase A por sistema** (parseado de `DIAS`, v5.16):

| Sistema | Días | Fechas reales | Nivel(es) |
|---|---|---|---|
| **UWSA1 — baseline** | **D1** | **lun 28-sep** | 5 (160 Q) |
| Fundamentos (Pathoma 1-3) | D2 · D3 | mar 29-sep → mié 30-sep | 1 |
| Immunology | D4 · D5 | jue 1-oct → vie 2-oct | 1 (D5 es viernes, pero es el 2.º día del sistema: el bloque de sistema completo se hace de Fundamentos) |
| **Cardiovascular (1.ª parte)** | D6-D9 | lun 5-oct → jue 8-oct | 1 → 2 (anatomía/fisiología · hemodinámica · curvas · electrofisiología/ECG) |
| **NBME 25** | **D10** | **vie 9-oct** | 5 (200 Q) |
| Cardiovascular (cierre) | D11-D16 | lun 12-oct → lun 19-oct | 2 → **3 (D15, vie 16-oct: insuficiencia cardíaca + shock + fármacos IC — viernes de nivel 3)** → 2 (taquiarritmias · antiarrítmicos · aterosclerosis/isquemia · SCA · IC + shock · valvulopatías/miocardiopatías/congénitas) |
| **Respiratory** | D17-D22 | mar 20-oct → mar 27-oct | 1 → 2 → **3 (D20, vie 23-oct: restrictivas + intersticiales + ocupacionales — viernes de nivel 3)** → 2 (bloque íntegro detrás del NBME 25) |
| **Renal (1.ª parte)** | D23 · D24 | mié 28-oct → jue 29-oct | 1 |
| **NBME 26** | **D25** | **vie 30-oct** | 5 (200 Q) |
| Renal (cierre) | D26-D29 | lun 2-nov → jue 5-nov | 2 (electrolitos · ácido-base · glomerulares · AKI + ERC + litiasis + poliquistosis) — sin viernes propio en v5.16: su único viernes es el NBME 26 |
| **Gastrointestinal** | D30-D36 | vie 6-nov → lun 16-nov | 1 (D30, vie 6-nov, es el 1.er día del sistema: el bloque de sistema de ese viernes se hace de Renal) → 2 → **3 (D35, vie 13-nov: hígado II — cirrosis + complicaciones + HCC + hereditarias)** → 2 (bloque íntegro detrás del hito) |
| **Endocrine (1.ª parte)** | D37-D39 | mar 17-nov → jue 19-nov | 1 → 2 |
| **NBME 27** | **D40** | **vie 20-nov** | 5 (200 Q) |
| Endocrine (cierre) | D41 · D42 | lun 23-nov → mar 24-nov | 2 (DM 1 y 2 + DKA/HHS · calcio/PTH + MEN + hipófisis) |
| **Nervous System** | D43-D50 | mié 25-nov → vie 4-dic | 1 → **3 (D45, vie 27-nov: tronco + pares craneales + reflejos)** → 2 → **3 (D50, vie 4-dic: EM/desmielinizantes + meningitis + NMJ + tumores SNC)** — dos viernes de nivel 3 (bloque íntegro detrás del hito) |
| **Hematology & Oncology (1.ª parte)** | D51-D54 | lun 7-dic → jue 10-dic | 1 → 2 — sin viernes propio en v5.16: su único viernes es el NBME 28 |
| **NBME 28** | **D55** | **vie 11-dic** | 5 (200 Q) |
| Hematology & Oncology (cierre) | D56 · D57 | lun 14-dic → mar 15-dic | 2 (leucemias + mielodisplasia · linfomas + mieloma + transfusión + fármacos onco) |
| **Microbiology / ID** | D58-D64 | mié 16-dic → jue 24-dic | 1 → **4 (D60, vie 18-dic: Gram− bacilos entéricos/respiratorios + zoonosis — único viernes de nivel 4 del plan, S12, mixto de los 9 sistemas ya cerrados)** → 2 (bloque íntegro detrás del hito) |
| **NBME 29** | **D65** | **lun 28-dic** | 5 (200 Q) |
| **Reproductive** | D66-D70 | mar 29-dic → mié 6-ene | 1 → 2 (bloque íntegro detrás del hito; el vie 25-dic es skip) — **cierra el contenido de 2026** |
| Musculoskeletal / Rheum (1.ª parte) | D71 | jue 7-ene | 1 (artritis + autoanticuerpos) |
| **NBME 30 — cierre de Fase A** | **D72** | **vie 8-ene** | 5 (200 Q) |
| Musculoskeletal / Rheum (cierre, + derma Step 1) | D73 · D74 | lun 11-ene → mar 12-ene | 1 → 2 (LES/conectivopatías/vasculitis · hueso + anatomía MSK) · **D74 = cuenta doble Derma ↔ Step 1**: el pre-test 08:15 y el repaso 07:15 apuntan a `deck:APEX::DERMA tag:step1` + fallos del ledger derma (8 átomos ya estudiados: d7·d8·d10·d12·d14·d16·d23·d24) |
| **Psychiatry & Behavioral (1.ª parte)** | D75 · D76 | mié 13-ene → jue 14-ene | 1 (ánimo/psicóticos · ansiedad/personalidad/infancia/sustancias) |
| **UWSA2** | **D77** | **vie 15-ene** | 5 (160 Q) — **dentro de la Fase A**; parte Psiquiatría |
| Psychiatry & Behavioral (cierre) | D78 | lun 18-ene | 2 (psicofármacos) |
| Biostats / epi / ética (`system` = Psychiatry & Behavioral) | D79 | mar 19-ene | 2 |
| Biochemistry (día doble 1: metabolismo) | D80 | mié 20-ene | 1 |
| Biochemistry (día doble 2: biología molecular + genética + farmacología general) — **cierre de Fase A** | D81 | jue 21-ene | 1 |
> En v5.16 el **NBME 30 (D72, vie 8-ene) parte MSK** (artritis delante; LES/vasculitis y hueso detrás) y ya no depende de los skips de fin de año (Reproductive, D66-D70, cierra 2026); el **UWSA2 (D77, vie 15-ene) parte Psiquiatría** (psicofármacos D78 y Biostats D79 detrás) y los dos días dobles de Bioquímica van juntos al final: **D80 el mié 20-ene** y **D81 el jue 21-ene-2027**. La Fase A muere el **jue 21-ene** y **el NBME 31 (D82, vie 22-ene) cae al día siguiente, sin banco de consolidación delante** (herencia de la v5.14, que el corrimiento rígido conserva): el gate del 80% de Psych y Bioquímica tiene que estar validado el mismo 21-ene.
...
- **Milestone 2 (subtema timed)**: los **39 días de nivel 2**, es decir, desde el 3.er día de cada sistema.
- **Milestone 3 (sistema completo timed, 20Q)**: en v5.16 son **5 días**, todos viernes sin hito de S1-S10 con el sistema ya ≥3 días abierto — **D15 (vie 16-oct, Cardiovascular: insuficiencia cardíaca + shock)**, **D20 (vie 23-oct, Respiratory: restrictivas + intersticiales + ocupacionales)**, **D35 (vie 13-nov, Gastrointestinal: hígado II — cirrosis + HCC)**, **D45 (vie 27-nov, Nervous System: tronco + pares craneales + reflejos)** y **D50 (vie 4-dic, Nervous System: EM + meningitis + NMJ + tumores SNC)**. Los otros dos viernes sin hito de Fase A (D5 Inmuno · D30 GI) caen en el 1.º-2.º día de su sistema → nivel 1, y el bloque de 20Q se hace del sistema anterior. Con el D1 en lunes **los hitos vuelven a ocupar viernes** (6 de los 15 viernes de Fase A: NBME 25-28, NBME 30, UWSA2): quedan 8 viernes de Fase A sin hito (5 N3 + 1 N4 + 2 N1).
- **Milestone 4 (mixto de ≥3 sistemas)**: como **dosis diaria** en la eval de las 18:00 (10Q mixtas timed) durante toda la Fase A; **desde S11 también el viernes a las 11:00** (divergencia #6, flag `VIERNES_N4_DESDE_SEMANA = 11`): en v5.16 es **D60 (vie 18-dic, S12, Micro/ID: Gram− bacilos + zoonosis)** = 20-30Q timed mixtos de los sistemas ya cerrados (el viernes de S11 es el NBME 28 y S13-S14 no tienen viernes hábil). En Fase B: D84 · D86 (random timed 2×40Q, intercalados con NBME 32 y 33); en Fase C: D88 · D89.
...
- **Regla de los 2 minutos y "juez, no abogado"**: 18:00 desde **D2 (mar 29-sep)** — se pueden entrenar desde el primer día sin coste.
- **Ancla CAD**: el subtema de **enfermedad coronaria** que Palmerton recomienda como primera validación cae en **D13 (mié 14-oct, "aterosclerosis + isquemia + angina" — ya detrás del NBME 25)** y **D14 (jue 15-oct, SCA)**; el sistema cardiovascular arranca el **lun 5-oct** (D6).
...
|---|---|---|---|---|---|
| **UWSA1 (baseline)** | D1 | **lun 28-sep-2026** | 160 | — (cualquier valor sirve) | Baseline temprano: rompe el miedo a evaluarse y fija prioridades (§9.8). **Con la regla del 5%/mes, un baseline <45-48% haría matemáticamente inalcanzable el 70%** → es el "Goldilocks check" real, no el 22-ene |
| **NBME 25** | D10 | **vie 9-oct-2026** | 200 | **51%** | Pase seguro de la forma 25 = 64% |
| **NBME 26** | D25 | **vie 30-oct-2026** | 200 | **54%** | forma 26 = 64% |
| **NBME 27** | D40 | **vie 20-nov-2026** | 200 | **57%** (gate 1 ECFMG ≥55%) | forma 27 = 65% |
| **NBME 28** | D55 | **vie 11-dic-2026** | 200 | **61%** | forma 28 = 63% |
| **NBME 29** | D65 | **lun 28-dic-2026** | 200 | **63%** (gate 2 ≥60%) | forma 29 = 64% |
| **NBME 30 (cierre Fase A)** | D72 | **vie 8-ene-2027** | 200 | **65%** | **umbral de seguridad Palmerton ≈95% de pase**; forma 30 = la más difícil (65%) |
| **UWSA2** | D77 | **vie 15-ene-2027** | 160 | "low risk" | **El UWSA sobreestima 10-15+ puntos**: resistencia y señal secundaria; **la fecha la decide el NBME** · en v5.16 cae **dentro de la Fase A** (parte Psiquiatría: psicofármacos y Biostats quedan detrás; los dos días dobles de Bioquímica cierran la fase) |
| **NBME 31 — GO/NO-GO** | D82 | **vie 22-ene-2027** | 200 | **68%** | 68% está entre el 65% (≈95%) y el 70% (≈99%) de Palmerton; el criterio de **2 NBME consecutivos** es **más robusto** que su regla · **v5.16: cae el día siguiente al cierre de contenido (D81, jue 21-ene), sin banco de consolidación delante; abre la Fase B** |
| **NBME 32** | D83 | **lun 25-ene-2027** | 200 | **68%** | confirma el GO |
| **NBME 33** | D85 | **mié 27-ene-2027** | 200 | **68%** | confirma el GO |
| **Free 120** | D87 | **vie 29-ene-2027** | 120 | **70%** (heurística comunitaria, **no** cifra Palmerton) | Sweet spot 1-2 semanas antes ✔ (D-13 del examen del jue 11-feb; abre la Fase C) · **rendirlo en el Prometric real** = decisión pendiente (§12.6) |

**Cadencia**: los 7 hitos NBME/UWSA1 de Fase A caen cada ~3 semanas (y el UWSA2, D77, cierra la serie de Fase A) → dentro del rango operativo 1-4 semanas de Palmerton, en su extremo lento (justificado: las formas NBME son finitas, 25-33). 🆕 **Desde la v5.15 las 12 fechas corren con el plan** (antes 11 estaban clavadas y solo bajaba su D#, lo que les robaba preparación): ahora cada hito conserva su D# y su colchón de contenido. El UWSA1 sigue al D1 (vie 11-sep → lun 14-sep → mar 15-sep → mié 16-sep → jue 17-sep → lun 21-sep → mié 23-sep → **lun 28-sep**).

**Day-After Protocol** (§9.2): el NBME se rinde por la mañana (en v5.16 casi siempre un **viernes**: 8 de los 12 hitos); sus **4 h de auditoría** (trayectoria 15' · auditoría de honestidad 15' · triaje 1 h · **10 PC cards manuscritas 2,5 h**) caben en el **deep prime del día siguiente** (09:00-11:00 + el resto del bloque de mañana; con los hitos en viernes, ese día es el lunes siguiente). **Leer cada NBME por sistema**, exigiendo ≥80% en lo ya estudiado.

## 12.5 Fases B y C: niveles 4-5, cierre y test day

| Día | Fecha | Contenido del plan | Técnica Palmerton |
|---|---|---|---|
| **D77** | vie 15-ene | **UWSA2** (160 Q) — hito de la Fase A (`faseDe(77) = 'A'`), entre los dos tramos de Psiquiatría y delante de los dos días dobles de Bioquímica | Resistencia + señal secundaria |
| **D80 · D81** | mié 20-ene → jue 21-ene | Bioquímica días dobles 1 y 2 (metabolismo · biología molecular + genética + farmacología general) — **D81 = cierre de la Fase A** | Nivel 1 (últimos subtemas nuevos del plan) |
| **D82** | vie 22-ene | **NBME 31** + decisión **GO/NO-GO** — **abre la Fase B el día siguiente al cierre de contenido (sin banco de consolidación delante: herencia de la v5.14 que el corrimiento rígido conserva)** | §9.1: nunca presentarse si los dos últimos NBME no respaldan |
| **D83** | lun 25-ene | **NBME 32** + repaso FA sistemas 1-5 — Fase B | Nivel 5 |
| **D84 · D86** | mar 26-ene · jue 28-ene | Random timed **2×40Q (80 Q/día)** + revisión profunda + **sistema débil #1** según NBME — **Fase B**, intercalados con NBME 32 y 33 | **Nivel 4** (mixto de sistemas dominados) + el reparto de Melody: *"distribute more time to your weakest subjects"* |
| **D85** | mié 27-ene | **NBME 33** + repaso FA sistemas 11-14 — Fase B | Nivel 5 · **candidato natural a la maratón de resistencia** (§12.6 #4) |
| **D87** | vie 29-ene | **FREE 120 oficial** + logística del examen + cierre — **abre la Fase C** | Free 120 en la ventana correcta (D-13 del examen del jue 11-feb) |
| **D88 · D89** | lun 1-feb · mar 2-feb | Random timed 2×40Q + **sistema débil #2** (D89 con Mehlman HY del sistema) — `system = Banco intensivo` dentro del sprint | **Nivel 4** (últimos días de mixto de sistemas dominados) |
| **D90 · D91** | mié 3-feb · jue 4-feb | **uWorld incorrects (2.ª pasada) + flagged** (nunca re-hacer acertadas) + sistema débil #3 — `system = Banco intensivo` dentro del sprint | **Nivel 5** · coherente con "una sola pasada, nunca reset": los incorrects/flagged sí |
| **D92** | vie 5-feb | **AMBOSS 200 Concepts (mitad 1)** + incorrects (80 Q) — `system = Banco intensivo` | Nivel 5 · AMBOSS 200 = repaso de conceptos ya vistos, no banco nuevo |
| **sáb 6 · dom 7** | sáb 6-feb · dom 7-feb | Finde normal del plan (entre D92 y D93): sin banco, solo Anki vencido — en v5.16 **no** es la víspera del examen | §8.3 · descanso |
| **D93** | lun 8-feb | **AMBOSS 200 Concepts (mitad 2)** + incorrects (80 Q) — último día de banco alojado en el sprint | Nivel 5 |
| **D94** | **mar 9-feb** | Repaso FA sistemas 6-10 + **Anki marathon** + incorrects — **TAPER D-2 · ÚLTIMA SESIÓN DE BANCO (20 Q)** | §8.3: solo Anki **maduro** + 20 Q flagged/incorrects ya vistos, sin bloque timed ni AMBOSS; cero preguntas y cero tarjetas nuevas; dormir ≥7 h ya desde hoy (`DIAS[94].franjaNota`, `USMLE_TAPER.d94`) |
| **D95 (D-1)** | **mié 10-feb** | Rapid review de First Aid + Anki + laboratorio de dudas — **TAPER D-1 REAL · último día del plan (20 Q)**: sesión MÍNIMA solo por la mañana (≤2 h); tarde de logística (`USMLE_TAPER.d95`/`.dMenos1`, `DAILY_META.descansoD1 = '2027-02-10'`) | §8.3 D-1: Anki maduro/vencido + 20 Q flagged con los mejores esquemas e imágenes, rapid review FA; cero preguntas nuevas, cero tarjetas nuevas, ningún bloque timed; **prohibido** temas densos y abrir First Aid "para ver cuánto sé"; **tarde**: permiso impreso + digital, 2 ID con el nombre exacto, bolsas Ziploc numeradas, ruta al Prometric; nada después de las 17:00 (journaling, ejercicio suave, visualización, cena con proteína); somnífero nunca por primera vez; dormir ≥7-8 h |
| **(examen)** | **jue 11-feb** | USMLE Step 1 — target v5.16, **FUERA de la ventana 25-29 ene y al día siguiente del D95** (`DAILY_META.examenTarget = '2027-02-11'`, `USMLE_TAPER.examen`; agendar/reprogramar Prometric y confirmar eligibility period) | Test day (§8.4): desayuno proteína + grasa, café de siempre, tutorial → +15 min de descanso, bloques 1-2 → 10' · 3-4 → 10' · 5 → almuerzo 20-30' · 6 → 10' · 7; nunca revisar entre bloques ni salir a mitad de bloque; premio post-test |

**Stress sets** (§7.5): el plan los sitúa a las **05:00 en Fases B y C** (D82 en adelante, desde el **vie 22-ene**), lo que respeta el criterio de entrada de su autor (≥60% del contenido cubierto, últimas 2-3 semanas) — con la Fase A cerrada el **21-ene** (D81), el contenido está cubierto al 100% desde el primer stress set.

**Cese de contenido nuevo**: Palmerton pide parar preguntas y tarjetas nuevas **1-2 semanas antes**. Con examen el **jue 11-feb**, eso equivale a partir del **28 de enero-4 de febrero** → el plan lo adelanta a **NBME 31 (D82, vie 22-ene) en adelante**, es decir, toda la Fase B y la Fase C. El plan trata **AMBOSS 200 Concepts como repaso** de conceptos ya vistos, no como banco nuevo, y la Fase C ya sólo usa flagged. **El taper es explícito**: D94 y D95 bajan a 20 Q (flagged + Anki maduro, `TAPER_ACTIVO` en el generador); **en la v5.16 el D-1 real es el D95 (mié 10-feb, sesión mínima + ritual), precedido del D94 (mar 9-feb) y seguido directamente del examen (jue 11-feb)**: ya no hay finde libre entre el taper y el examen (el sáb 6 y el dom 7-feb caen entre D92 y D93). El plan rebasa la ventana 25-29 ene y el examen es el jue 11-feb (target; agendar/reprogramar Prometric y confirmar eligibility period, o recortar temario: decisión de Joseph).

**Burnout (§9.12) como REGLA del plan** (divergencia #29 → §E-7, 12-sep tarde): si **2 hitos consecutivos con mínimo** quedan bajo su mínimo on-track (§12.4) **y** hay síntomas → **3-5 días con sólo Anki AM (30-45 min de tarjetas viejas) + sueño**, frenar QBank y adquisición; el corrimiento determinista +1 día hábil absorbe la pausa (no se recorta temario) y se reanuda por el gate del 80%. ⚠ En v5.16 esa pausa mueve el examen más allá del jue 11-feb (y arrastra con él todos los hitos, que ya no están clavados por fecha): activar el protocolo implica, además, reprogramar la fecha (plan B feb-mar, mismo eligibility period). En la app: `usmleScores.gateHito` → `'ALERTA BURNOUT'` (banner en `UsmleHub`); `BURNOUT_PROTOCOLO` lleva los pasos de §9.12.

## 12.6 Divergencias que quedan abiertas

> Estas son **exactamente** las de `PALMERTON_DIVERGENCIAS_PLAN.md` (§E). Este documento **no las contradice ni las resuelve**: las hereda. **Estado al 26-sep (v5.16)**: las #2, #5 y #7 quedaron **implementadas** el 12-sep (no tocan horario, metas ni temario) y siguen vigentes remapeadas; las demás siguen a decisión de Joseph. **Ninguna decisión abierta se resolvió con el corrimiento v5.16** (la #5 mantiene su forma: "rendir el jue 11-feb vs recortar temario"; el D-1 real sigue siendo el D95; lo nuevo — hitos que corren con el plan y conservan su D# — es una regla de corrimiento, no una decisión pendiente).
...
2. ✅ **Viernes de nivel 4** (mixto de 20-30Q de sistemas dominados) desde S11 — **implementado** con el flag `VIERNES_N4_DESDE_SEMANA = 11` en `gen_usmle_v5.js` (§12.3); en v5.16 afecta solo a **D60 (vie 18-dic, S12)** — uno, porque con el D1 en lunes los hitos vuelven a ocupar viernes y S13-S14 no tienen viernes hábil; contenido idéntico, sólo cambia el formato del bloque de las 11:00. *(divergencia #6)*
...
4. **Free 120 en el Prometric real de Lima + maratón de resistencia**: (a) reservar la sesión de práctica en Prometric (costo internacional citado por el cuaderno: **$155 — A VERIFICAR (16-sep)**; registro con hasta 7 días de espera); (b) la maratón de 7+ bloques **no cabe** en la tarde de un viernes de plan y los sábados son libres por régimen → única opción sin tocar franjas: **NBME 33 (D85, mié 27-ene) + 3 bloques de flagged** (≈5 h), cediendo Research/Derma/AURUM ese día. *(divergencia #21)* — ⏳ decisión Joseph.
5. ✅ **Protocolo taper / D-1 / test day** — **implementado** (§12.5) y **remapeado en v5.16**: **D94 (mar 9-feb) = D-2, última sesión de banco** y **D95 (mié 10-feb) = D-1 REAL, último día del plan** (taper de 20 Q, seguidos; D95 = sesión mínima AM + ritual §8.3); **el plan rebasa la ventana 25-29 ene → examen target JUE 11-FEB-2027, al día siguiente del D95 y sin finde en medio**, con test day según el plan de bloques de §8.4; código en `USMLE_TAPER`. **Dato heredado (no decisión): el NBME 31 (D82, vie 22-ene) cae el día siguiente al cierre de contenido (D81), sin banco de consolidación delante.** ⚠ **Decisión de Joseph (26-sep; sustituye a la del 22-sep)**: **rendir el jue 11-feb** (agendar/reprogramar Prometric y **confirmar que el eligibility period cubre el 11-feb**, extendiéndolo si hace falta) **o recortar temario** para volver atrás. *(divergencia #22)*
...
8. **Si el UWSA1 del lun 28-sep sale <40%**: bajar S1-S2 (semanas del 28-sep y del 5-oct) a **20 Q/día** al estilo Jay/Melody. *(divergencia #2)* — ⏳ se decide el mismo lun 28-sep con el % real.
...
*Documento generado el 8-sep-2026 sobre los extractos v3 del cuaderno "STEP 1 · Palmerton Engine"; **§12 remapeado el 10-sep-2026 al régimen v5.9, el 12-sep-2026 al v5.10, el 14-sep-2026 al v5.11, el 15-sep-2026 al v5.12, el 16-sep-2026 al v5.13, el 19-sep-2026 al v5.14, el 22-sep-2026 al v5.15 (corrimiento RÍGIDO) y el 26-sep-2026 al v5.16 (2.º corrimiento RÍGIDO)**. Todas las fechas y los D# de §12 provienen del parseo de `src/lib/usmleStep1Daily.ts` (95 días, D1 = 2026-09-28 → D95 = 2027-02-10 = D-1 real; examen target 2027-02-11) y de `src/lib/usmleScores.ts`; no hay fechas estimadas.*
