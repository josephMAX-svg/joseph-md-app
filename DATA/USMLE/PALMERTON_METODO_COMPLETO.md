# MÉTODO PALMERTON — DE LA A A LA Z (síntesis maestra · 8-sep-2026 · §12 remapeado al régimen v5.14 el 19-sep-2026)

> **Qué es este documento.** La síntesis completa del método de **Alec Palmerton, MD** (Stanford Med · anestesiología en Harvard-MGH · Step 1 = 270, top 0,1%) tal como está publicado en su canal de YouTube y en **yousmle.com**, ordenado para ejecutarse. Es el documento maestro del bloque USMLE Step 1 de Joseph: la doctrina (§1-§11) y su traducción exacta al plan **v5.12** día por día (§12).
>
...
# 12. MAPEO AL PLAN v5.14 DE JOSEPH

> **Fuente de los datos de esta sección**: parseo directo de `src/lib/usmleStep1Daily.ts` con Node — versión original 8-sep-2026, **re-parseada el 10-sep-2026 (v5.9), el 12-sep-2026 (v5.10), el 14-sep-2026 (v5.11), el 15-sep-2026 (v5.12), el 16-sep-2026 (v5.13) y el 19-sep-2026 (v5.14)** (`DAILY_META`, `FRANJAS`, `USMLE_NIVELES`, `USMLE_GATE`, `USMLE_TAPER`, `DIAS`) y de `src/lib/usmleScores.ts` (`HITOS_ONTRACK`, `TIPO_ERROR_INFO`). **Ninguna fecha de esta sección está estimada de memoria.**
>
> **v5.14 (19-sep-2026; regla de Joseph): no se fusionó ni se recortó nada.** Ni el jueves 17 ni el viernes 18-sep se estudiaron (decimotercer corrimiento: 31-ago→18-sep sin estudiar = 15 hábiles perdidos); el temario salió 1:1 y el desfase se absorbió alargando el plan hasta el **mié 3-feb-2027**; 11 de los 12 hitos conservan su fecha y solo cambian de D# (−2). **El UWSA1 vuelve a cambiar de fecha** (jue 17-sep, ya pasada → **lun 21-sep**) y sigue siendo el D1; con el D1 en lunes las semanas del plan coinciden con las de calendario. 🔴 **El plan rebasa la ventana 25-29 ene: D94 = mar 2-feb = D-2 (última sesión de banco) y D95 = mié 3-feb = D-1 DENTRO del plan → examen target JUE 4-FEB-2027**; sáb 30 y dom 31-ene quedan libres (solo Anki vencido) entre D92 y D93 y el D-1 es sesión mínima por la mañana + ritual de test-day. **Consecuencia nueva: el contenido cierra el jue 14-ene (D81) y el NBME 31 GO/NO-GO cae al día siguiente (D82, vie 15-ene), sin banco de consolidación delante** (los 2 random timed pasan a D84 y D86; Fase B = D82-D86; Fase C = D87-D95 desde el Free 120). Joseph debe agendar/reprogramar el Prometric y confirmar que su eligibility period cubre el 4-feb (si no: extenderlo); la alternativa es recortar temario (decisión suya, §12.6 #5). ⚠ **Desde hoy, cada día no estudiado mueve el examen un día hábil más (o exige recortar temario).**
>
> 🔴 **El D1 es el UWSA1** (lun 21-sep: baseline antes de estudiar nada, como prescribe Palmerton) y el **contenido arranca el mar 22-sep = D2**.
>
> **Segunda pasada del 12-sep (tarde), vigente en v5.14**: se implementaron, sin tocar horario, temario ni fechas, tres divergencias de §12.6 — **viernes de nivel 4 desde S11** (en v5.14: solo D60, vie 11-dic, S12; los viernes de S11 y S13 son NBME 28 y 29), **taper D94-D95 + D-1 + test day** (`USMLE_TAPER`; en v5.14 el D-1 es el mié 3-feb = D95, dentro del plan) y el **protocolo de burnout como REGLA** (`gateHito` → 'ALERTA BURNOUT'). El total de Q es **5.560** (el multiconjunto de subtemas no cambia con el corrimiento). Además, el repaso anclado de las 07:15 muestra "MIR precedió esta semana: <asignatura>" con el tag Anki compartido `sys::<sistema>` y el D74 (dermato Step 1, mar 5-ene) cuenta doble con los 8 átomos Derma `step1`.

## 12.1 El marco: qué dice el plan hoy

- **95 días hábiles · D1 = lunes 21-sep-2026 (UWSA1) → D95 = miércoles 3-feb-2027 (D-1 dentro del plan).** El plan rebasa **la ventana 25-29 ene 2027** (D88-D92 caen dentro de ella; D94 = mar 2-feb = D-2, última sesión de banco) → **examen target JUEVES 4-FEB-2027, fuera de la ventana** (`DAILY_META.examenTarget = '2027-02-04'`); **sáb 30 y dom 31-ene libres** (solo Anki vencido) entre D92 y D93, y **D95 = mié 3-feb = D-1** (`descansoD1 = '2027-02-03'`: sesión mínima AM ≤2 h + ritual). Sábados y domingos **libres**; skips extra: 25-dic, 31-dic, 1-ene.
- **Fases** (`faseDe`): **A · Contenido por sistemas D1-D81** (21-sep → **14-ene**; el contenido empieza en D2, mar 22-sep; **el UWSA2, D77 vie 8-ene, cae DENTRO de la Fase A** y el cierre es el segundo día doble de Bioquímica, D81 jue 14-ene) · **B · Banco intensivo D82-D86** (15-ene → 21-ene: **el NBME 31 (D82, vie 15-ene) la abre el día siguiente al cierre de contenido, sin banco de consolidación delante**; NBME 32 D83 · random timed D84 · NBME 33 D85 · random timed D86) · **C · Sprint final D87-D95** (22-ene → **3-feb**, abre con el Free 120). D88 (lun 25-ene) y D89 (mar 26-ene, random timed + Mehlman), D90 (mié 27-ene), D91 (jue 28-ene), D92 (vie 29-ene) y D93 (lun 1-feb) siguen siendo días de banco (`system = Banco intensivo`: sistema débil #2 · incorrects 2.ª pasada + AMBOSS 200 mitades 1 y 2) alojados dentro de la semana del sprint.
...
## 12.3 Fase A (D1-D81 · lun 21-sep → jue 14-ene): los niveles 1→3 y el gate del 80%
...
**Calendario real de la Fase A por sistema** (parseado de `DIAS`, v5.14):

| Sistema | Días | Fechas reales | Nivel(es) |
|---|---|---|---|
| **UWSA1 — baseline** | **D1** | **lun 21-sep** | 5 (160 Q) |
| Fundamentos (Pathoma 1-3) | D2 · D3 | mar 22-sep → mié 23-sep | 1 |
| Immunology | D4 · D5 | jue 24-sep → vie 25-sep | 1 (D5 es viernes con el sistema recién abierto: sin bloque de sistema completo; se hace de Fundamentos) |
| **Cardiovascular (1.ª parte)** | D6-D9 | lun 28-sep → jue 1-oct | 1 → 2 (anatomía/fisiología · hemodinámica · curvas PV/Wiggers · electrofisiología/ECG) |
| **NBME 25** | **D10** | **vie 2-oct** | 5 (200 Q) |
| Cardiovascular (cierre) | D11-D16 | lun 5-oct → lun 12-oct | 2 → **3 (D15, vie 9-oct: IC + shock + fármacos IC — viernes de nivel 3)** → 2 (taquiarritmias · antiarrítmicos · aterosclerosis/isquemia · SCA · IC + shock · valvulopatías/miocardiopatías/congénitas) |
| **Respiratory** | D17-D22 | mar 13-oct → mar 20-oct | 1 → 2 → **3 (D20, vie 16-oct: restrictivas + intersticiales + ocupacionales)** → 2 (bloque íntegro detrás del NBME 25) |
| **Renal (1.ª parte)** | D23 · D24 | mié 21-oct → jue 22-oct | 1 (sin viernes propio: su viernes es el NBME 26) |
| **NBME 26** | **D25** | **vie 23-oct** | 5 (200 Q) |
| Renal (cierre) | D26-D29 | lun 26-oct → jue 29-oct | 2 (electrolitos · ácido-base · glomerulares · AKI + ERC + litiasis + poliquistosis) |
| **Gastrointestinal** | D30-D36 | vie 30-oct → lun 9-nov | 1 (D30, vie 30-oct, es el 1.er día del sistema: el bloque de sistema de ese viernes se hace de Renal) → 2 → **3 (D35, vie 6-nov: hígado II — cirrosis/complicaciones/HCC/hereditarias)** → 2 (bloque íntegro detrás del hito) |
| **Endocrine (1.ª parte)** | D37-D39 | mar 10-nov → jue 12-nov | 1 → 2 (sin viernes propio: su viernes es el NBME 27) |
| **NBME 27** | **D40** | **vie 13-nov** | 5 (200 Q) |
| Endocrine (cierre) | D41 · D42 | lun 16-nov → mar 17-nov | 2 (DM 1 y 2 + DKA/HHS · calcio/PTH + MEN + hipófisis) |
| **Nervous System** | D43-D50 | mié 18-nov → vie 27-nov | 1 → **3 (D45, vie 20-nov: tronco + pares craneales + reflejos)** → 2 → **3 (D50, vie 27-nov: EM/desmielinizantes + meningitis + NMJ + tumores SNC)** (bloque íntegro detrás del hito; único sistema con dos viernes de nivel 3) |
| **Hematology & Oncology (1.ª parte)** | D51-D54 | lun 30-nov → jue 3-dic | 1 → 2 (sin viernes propio: su viernes es el NBME 28) |
| **NBME 28** | **D55** | **vie 4-dic** | 5 (200 Q) |
| Hematology & Oncology (cierre) | D56 · D57 | lun 7-dic → mar 8-dic | 2 (leucemias + mielodisplasia · linfomas + mieloma + transfusión + fármacos onco) |
| **Microbiology / ID** | D58-D64 | mié 9-dic → jue 17-dic | 1 → **4 (D60, vie 11-dic: Gram− bacilos + zoonosis — único viernes de nivel 4 del plan, S12, mixto de los 9 sistemas ya cerrados)** → 2 (bloque íntegro detrás del hito) |
| **NBME 29** | **D65** | **vie 18-dic** | 5 (200 Q) |
| **Reproductive** | D66-D70 | lun 21-dic → lun 28-dic | 1 → 2 (bloque íntegro detrás del hito; el vie 25-dic es skip) |
| Musculoskeletal / Rheum (1.ª parte) | D71 | mar 29-dic | 1 (artritis + autoanticuerpos) |
| **NBME 30 — cierre del contenido de 2026** | **D72** | **mié 30-dic** | 5 (200 Q) |
| Musculoskeletal / Rheum (cierre, + derma Step 1) | D73 · D74 | lun 4-ene → mar 5-ene | 1 → 2 (LES/conectivopatías/vasculitis · hueso + anatomía MSK) · **D74 = cuenta doble Derma ↔ Step 1**: el pre-test 08:15 y el repaso 07:15 apuntan a `deck:APEX::DERMA tag:step1` + fallos del ledger derma (8 átomos ya estudiados: d7·d8·d10·d12·d14·d16·d23·d24) |
| **Psychiatry & Behavioral (1.ª parte)** | D75 · D76 | mié 6-ene → jue 7-ene | 1 (ánimo/psicóticos · ansiedad/personalidad/infancia/sustancias) |
| **UWSA2** | **D77** | **vie 8-ene** | 5 (160 Q) — **dentro de la Fase A**; parte Psiquiatría |
| Psychiatry & Behavioral (cierre) | D78 | lun 11-ene | 2 (psicofármacos) |
| Biostats / epi / ética (`system` = Psychiatry & Behavioral) | D79 | mar 12-ene | 2 |
| Biochemistry (día doble 1: metabolismo) | D80 | mié 13-ene | 1 |
| Biochemistry (día doble 2: biología molecular + genética + farmacología general) — **cierre de Fase A** | D81 | jue 14-ene | 1 |
> En v5.14 el **NBME 30 (D72, mié 30-dic) parte MSK** (artritis delante; LES/vasculitis y hueso detrás, ya en 2027: jue 31-dic y vie 1-ene son skip), el **UWSA2 (D77, vie 8-ene) parte Psiquiatría** (psicofármacos D78 y Biostats D79 detrás) y los dos días dobles de Bioquímica van juntos al final: **D80 el mié 13-ene** y **D81 el jue 14-ene-2027**. La Fase A muere el **jue 14-ene** y **el NBME 31 (D82, vie 15-ene) cae al día siguiente, sin banco de consolidación delante**: el gate del 80% de Psych y Bioquímica tiene que estar validado el mismo 14-ene.
...
- **Milestone 3 (sistema completo timed, 20Q)**: en v5.14 son **5 días**, todos viernes sin hito de S1-S10 con el sistema ya ≥3 días abierto — **D15 (vie 9-oct, Cardiovascular: IC + shock + fármacos IC)**, **D20 (vie 16-oct, Respiratory: restrictivas + intersticiales + ocupacionales)**, **D35 (vie 6-nov, Gastrointestinal: hígado II)**, **D45 (vie 20-nov, Nervous System: tronco + pares craneales + reflejos)** y **D50 (vie 27-nov, Nervous System: EM/desmielinizantes + meningitis + NMJ + tumores SNC)**. Los otros 2 viernes sin hito de Fase A (D5 Immuno · D30 GI) caen en el 1.º-2.º día de su sistema → nivel 1, y el bloque de 20Q se hace del sistema anterior; Renal, Endo, Heme/Onc y Repro no tienen viernes propio (NBME 26, 27, 28 y 25-dic skip).
- **Milestone 4 (mixto de ≥3 sistemas)**: como **dosis diaria** en la eval de las 18:00 (10Q mixtas timed) durante toda la Fase A; **desde S11 también el viernes a las 11:00** (divergencia #6, flag `VIERNES_N4_DESDE_SEMANA = 11`): en v5.14 solo **D60 (vie 11-dic, S12, Micro/ID: Gram− bacilos + zoonosis)** = 20-30Q timed mixtos de los 9 sistemas ya cerrados (los viernes de S11 y S13 son NBME 28 y 29). En Fase B: D84 · D86 (random timed 2×40Q, intercalados con NBME 32 y 33); en Fase C: D88 · D89.
...
- **Regla de los 2 minutos y "juez, no abogado"**: 18:00 desde **D2 (mar 22-sep)** — se pueden entrenar desde el primer día sin coste.
...
|---|---|---|---|---|---|
| **UWSA1 (baseline)** | D1 | **lun 21-sep-2026** | 160 | — (cualquier valor sirve) | Baseline temprano: rompe el miedo a evaluarse y fija prioridades (§9.8). **Con la regla del 5%/mes, un baseline <45-48% haría matemáticamente inalcanzable el 70%** → es el "Goldilocks check" real, no el 15-ene |
| **NBME 25** | D10 | **vie 2-oct-2026** | 200 | **51%** | Pase seguro de la forma 25 = 64% |
| **NBME 26** | D25 | **vie 23-oct-2026** | 200 | **54%** | forma 26 = 64% |
| **NBME 27** | D40 | **vie 13-nov-2026** | 200 | **57%** (gate 1 ECFMG ≥55%) | forma 27 = 65% |
| **NBME 28** | D55 | **vie 4-dic-2026** | 200 | **61%** | forma 28 = 63% |
| **NBME 29** | D65 | **vie 18-dic-2026** | 200 | **63%** (gate 2 ≥60%) | forma 29 = 64% |
| **NBME 30 (cierre Fase A)** | D72 | **mié 30-dic-2026** | 200 | **65%** | **umbral de seguridad Palmerton ≈95% de pase**; forma 30 = la más difícil (65%) |
| **UWSA2** | D77 | **vie 8-ene-2027** | 160 | "low risk" | **El UWSA sobreestima 10-15+ puntos**: resistencia y señal secundaria; **la fecha la decide el NBME** · en v5.14 cae **dentro de la Fase A** (parte Psiquiatría: psicofármacos y Biostats quedan detrás; los dos días dobles de Bioquímica cierran la fase) |
| **NBME 31 — GO/NO-GO** | D82 | **vie 15-ene-2027** | 200 | **68%** | 68% está entre el 65% (≈95%) y el 70% (≈99%) de Palmerton; el criterio de **2 NBME consecutivos** es **más robusto** que su regla · **v5.14: cae el día siguiente al cierre de contenido (D81, jue 14-ene), sin banco de consolidación delante; abre la Fase B** |
| **NBME 32** | D83 | **lun 18-ene-2027** | 200 | **68%** | confirma el GO |
| **NBME 33** | D85 | **mié 20-ene-2027** | 200 | **68%** | confirma el GO |
| **Free 120** | D87 | **vie 22-ene-2027** | 120 | **70%** (heurística comunitaria, **no** cifra Palmerton) | Sweet spot 1-2 semanas antes ✔ (D-13 del examen del jue 4-feb; abre la Fase C) · **rendirlo en el Prometric real** = decisión pendiente (§12.6) |

**Cadencia**: los 7 hitos NBME/UWSA1 de Fase A caen cada ~3 semanas (y el UWSA2, D77, cierra la serie de Fase A) → dentro del rango operativo 1-4 semanas de Palmerton, en su extremo lento (justificado: las formas NBME son finitas, 25-33). Once de las 12 fechas no se han movido en ningún corrimiento; el UWSA1 sigue al D1 (vie 11-sep → lun 14-sep → mar 15-sep → mié 16-sep → **lun 21-sep**).
...
|---|---|---|---|
| **D77** | vie 8-ene | **UWSA2** (160 Q) — hito de la Fase A (`faseDe(77) = 'A'`), entre los dos tramos de Psiquiatría y delante de los dos días dobles de Bioquímica | Resistencia + señal secundaria |
| **D80 · D81** | mié 13-ene → jue 14-ene | Bioquímica días dobles 1 y 2 (metabolismo · biología molecular + genética + farmacología general) — **D81 = cierre de la Fase A** | Nivel 1 (últimos subtemas nuevos del plan) |
| **D82** | vie 15-ene | **NBME 31** + decisión **GO/NO-GO** — **abre la Fase B el día siguiente al cierre de contenido (sin banco de consolidación delante: consecuencia del corrimiento v5.14)** | §9.1: nunca presentarse si los dos últimos NBME no respaldan |
| **D83** | lun 18-ene | **NBME 32** + repaso FA sistemas 1-5 — Fase B | Nivel 5 |
| **D84 · D86** | mar 19-ene · jue 21-ene | Random timed **2×40Q (80 Q/día)** + revisión profunda + **sistema débil #1** según NBME — **Fase B**, intercalados con NBME 32 y 33 | **Nivel 4** (mixto de sistemas dominados) + el reparto de Melody: *"distribute more time to your weakest subjects"* |
| **D85** | mié 20-ene | **NBME 33** + repaso FA sistemas 11-14 — Fase B | Nivel 5 · **candidato natural a la maratón de resistencia** (§12.6 #4) |
| **D87** | vie 22-ene | **FREE 120 oficial** + logística del examen + cierre — **abre la Fase C** | Free 120 en la ventana correcta (D-13 del examen del jue 4-feb) |
| **D88 · D89** | lun 25-ene · mar 26-ene | Random timed 2×40Q + **sistema débil #2** (D89 con Mehlman HY del sistema) — `system = Banco intensivo` dentro del sprint | **Nivel 4** (últimos días de mixto de sistemas dominados) |
| **D90 · D91** | mié 27-ene · jue 28-ene | **uWorld incorrects (2.ª pasada) + flagged** (nunca re-hacer acertadas) + sistema débil #3 — `system = Banco intensivo` dentro del sprint | **Nivel 5** · coherente con "una sola pasada, nunca reset": los incorrects/flagged sí |
| **D92** | vie 29-ene | **AMBOSS 200 Concepts (mitad 1)** + incorrects (80 Q) — `system = Banco intensivo` | Nivel 5 · AMBOSS 200 = repaso de conceptos ya vistos, no banco nuevo |
| **sáb 30 · dom 31** | sáb 30-ene · dom 31-ene | **Libres** (entre D92 y D93): sin banco, solo Anki vencido | §8.3 · descanso |
| **D93** | lun 1-feb | **AMBOSS 200 Concepts (mitad 2)** + incorrects (80 Q) — último día de banco alojado en el sprint | Nivel 5 |
| **D94** | **mar 2-feb** | Repaso FA sistemas 6-10 + **Anki marathon** + incorrects — **TAPER D-2 · ÚLTIMA SESIÓN DE BANCO (20 Q)** | §8.3: solo Anki **maduro** + 20 Q flagged/incorrects ya vistos, sin bloque timed ni AMBOSS; cero preguntas y cero tarjetas nuevas; dormir ≥7 h ya desde hoy (`DIAS[94].franjaNota`, `USMLE_TAPER.d94`) |
| **D95 (D-1)** | **mié 3-feb** | Rapid review de First Aid + Anki + laboratorio de dudas — **TAPER D-1 · DENTRO del plan (20 Q)**: sesión MÍNIMA solo por la mañana (≤2 h); tarde de logística (`USMLE_TAPER.d95`/`.dMenos1`, `DAILY_META.descansoD1 = '2027-02-03'`) | §8.3 D-1: Anki maduro/vencido + 20 Q flagged con los mejores esquemas e imágenes, rapid review FA; cero preguntas nuevas, cero tarjetas nuevas, ningún bloque timed; **prohibido** temas densos y abrir First Aid "para ver cuánto sé"; **tarde**: permiso impreso + digital, 2 ID con el nombre exacto, bolsas Ziploc numeradas, ruta al Prometric; nada después de las 17:00 (journaling, ejercicio suave, visualización, cena con proteína); somnífero nunca por primera vez; dormir ≥7-8 h |
| **(examen)** | **jue 4-feb** | USMLE Step 1 — target v5.14, **FUERA de la ventana 25-29 ene** (`DAILY_META.examenTarget = '2027-02-04'`, `USMLE_TAPER.examen`; agendar/reprogramar Prometric y confirmar eligibility period) | Test day (§8.4): desayuno proteína + grasa, café de siempre, tutorial → +15 min de descanso, bloques 1-2 → 10' · 3-4 → 10' · 5 → almuerzo 20-30' · 6 → 10' · 7; nunca revisar entre bloques ni salir a mitad de bloque; premio post-test |

**Stress sets** (§7.5): el plan los sitúa a las **05:00 en Fases B y C** (D82 en adelante, desde el **vie 15-ene**), lo que respeta el criterio de entrada de su autor (≥60% del contenido cubierto, últimas 2-3 semanas) — con la Fase A cerrada el **14-ene** (D81), el contenido está cubierto al 100% desde el primer stress set.

**Cese de contenido nuevo**: Palmerton pide parar preguntas y tarjetas nuevas **1-2 semanas antes**. Con examen el **jue 4-feb**, eso equivale a partir del **21-28 de enero** → el plan lo adelanta a **NBME 31 (D82, vie 15-ene) en adelante**, es decir, toda la Fase B y la Fase C. El plan trata **AMBOSS 200 Concepts como repaso** de conceptos ya vistos, no como banco nuevo, y la Fase C ya sólo usa flagged. **El taper es explícito**: D94 y D95 bajan a 20 Q (flagged + Anki maduro, `TAPER_ACTIVO` en el generador); **en la v5.14 el D-1 sigue DENTRO del plan (D95 = mié 3-feb, sesión mínima + ritual), seguido del D94 (mar 2-feb), con el finde 30-31 ene libre entre D92 y D93**: el plan rebasa la ventana 25-29 ene y el examen es el jue 4-feb (target; agendar/reprogramar Prometric y confirmar eligibility period, o recortar temario: decisión de Joseph).

**Burnout (§9.12) como REGLA del plan** (divergencia #29 → §E-7, 12-sep tarde): si **2 hitos consecutivos con mínimo** quedan bajo su mínimo on-track (§12.4) **y** hay síntomas → **3-5 días con sólo Anki AM (30-45 min de tarjetas viejas) + sueño**, frenar QBank y adquisición; el corrimiento determinista +1 día hábil absorbe la pausa (no se recorta temario) y se reanuda por el gate del 80%. ⚠ En v5.14 esa pausa mueve el examen más allá del jue 4-feb: activar el protocolo implica, además, reprogramar la fecha (plan B feb-mar, mismo eligibility period). En la app: `usmleScores.gateHito` → `'ALERTA BURNOUT'` (banner en `UsmleHub`); `BURNOUT_PROTOCOLO` lleva los pasos de §9.12.

## 12.6 Divergencias que quedan abiertas

> Estas son **exactamente** las de `PALMERTON_DIVERGENCIAS_PLAN.md` (§E). Este documento **no las contradice ni las resuelve**: las hereda. **Estado al 19-sep (v5.14)**: las #2, #5 y #7 quedaron **implementadas** el 12-sep (no tocan horario, metas ni temario) y siguen vigentes remapeadas; las demás siguen a decisión de Joseph. **Ninguna decisión abierta se resolvió con el corrimiento v5.14** (la #5 mantiene su forma: "rendir el jue 4-feb vs recortar temario"; el D-1 sigue dentro del plan como D95; la novedad — NBME 31 el día siguiente al cierre de contenido — es un dato del plan, no una decisión).
...
2. ✅ **Viernes de nivel 4** (mixto de 20-30Q de sistemas dominados) desde S11 — **implementado** con el flag `VIERNES_N4_DESDE_SEMANA = 11` en `gen_usmle_v5.js` (§12.3); en v5.14 afecta solo a **D60 (vie 11-dic, S12)** (los viernes de S11 y S13 son NBME 28 y 29); contenido idéntico, sólo cambia el formato del bloque de las 11:00. *(divergencia #6)*
...
4. **Free 120 en el Prometric real de Lima + maratón de resistencia**: (a) reservar la sesión de práctica en Prometric (costo internacional citado por el cuaderno: **$155 — A VERIFICAR (16-sep)**; registro con hasta 7 días de espera); (b) la maratón de 7+ bloques **no cabe** en la tarde de un viernes de plan y los sábados son libres por régimen → única opción sin tocar franjas: **NBME 33 (D85, mié 20-ene) + 3 bloques de flagged** (≈5 h), cediendo Research/Derma/AURUM ese día. *(divergencia #21)* — ⏳ decisión Joseph.
5. ✅ **Protocolo taper / D-1 / test day** — **implementado** (§12.5) y **remapeado en v5.14**: **D94 (mar 2-feb) = D-2, última sesión de banco** y **D95 (mié 3-feb) = D-1 DENTRO del plan** (taper de 20 Q, seguidos; D95 = sesión mínima AM + ritual §8.3); **el plan rebasa la ventana 25-29 ene → examen target JUE 4-FEB-2027**, con sáb 30 y dom 31-ene libres entre D92 y D93 y test day con el plan de bloques de §8.4; código en `USMLE_TAPER`. **Dato nuevo (no decisión): el NBME 31 (D82, vie 15-ene) cae el día siguiente al cierre de contenido (D81), sin banco de consolidación delante.** ⚠ **Decisión de Joseph (19-sep; sustituye a la del 16-sep)**: **rendir el jue 4-feb** (agendar/reprogramar Prometric y **confirmar que el eligibility period cubre el 4-feb**, extendiéndolo si hace falta) **o recortar temario** para volver atrás. *(divergencia #22)*
...
8. **Si el UWSA1 del lun 21-sep sale <40%**: bajar S1-S2 (semanas del 21 y del 28-sep) a **20 Q/día** al estilo Jay/Melody. *(divergencia #2)* — ⏳ se decide el mismo lun 21-sep con el % real.
...
*Documento generado el 8-sep-2026 sobre los extractos v3 del cuaderno "STEP 1 · Palmerton Engine"; **§12 remapeado el 10-sep-2026 al régimen v5.9, el 12-sep-2026 al v5.10, el 14-sep-2026 al v5.11, el 15-sep-2026 al v5.12, el 16-sep-2026 al v5.13 y el 19-sep-2026 al v5.14**. Todas las fechas y los D# de §12 provienen del parseo de `src/lib/usmleStep1Daily.ts` (95 días, D1 = 2026-09-21 → D95 = 2027-02-03 = D-1; examen target 2027-02-04) y de `src/lib/usmleScores.ts`; no hay fechas estimadas.*
