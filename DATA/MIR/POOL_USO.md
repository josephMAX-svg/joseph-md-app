...
| **Mini-MIR D77 40Q** (vie 15-ene-2027, v5.16 — mismo día que el UWSA2 del Step 1; corrección D78 lun 18-ene; v5.15: mar 12 / mié 13-ene) | `preguntasMixtasSinUsar(40, usadas, dia.fecha /* '2027-01-15' */, { soloPlan: true })` → tabla de neto por asignatura para la baseline de D78 (cobertura medida ≈80 % del peso ProMIR: solo las 14 asignaturas del plan) | `miniMIR` | 40 |
...
## 3. Banqueo ene-abr 2027 (`src/lib/mirMantenimiento.ts`, **57 días, mar 19-ene → mié 7-abr** = 46 lun-jue + 11 viernes · v5.16, 26-sep; v5.15: 57 desde el jue 14-ene hasta el vie 2-abr (45 + 12); v5.14: 57 desde el mar 12-ene hasta el mié 31-mar; v5.13: 59 desde el vie 8-ene; v5.12: 60 desde el 7-ene; v5.11: 61 desde el 6-ene; v5.10: 62 desde el 5-ene)
...
| lun-jue `banco` (reducido, 18 días mar 19-ene → jue 11-feb = día del examen Step 1 = D18, último reducido y además jueves Tier C → decisión de Joseph si va vacío; el primer día normal es el vie 12-feb = D19, viernes 30Q) | `preguntasMixtasSinUsar(10, usadas, fecha, { nums: [dia.num, dia.num2] })` | — | `mantenimiento` |
| **TIER C EXPRESS** (último slot lun-jue de las semanas reales 1-12 desde el mar 19-ene = D1, campo `dia.tierC`, `MIR_MANT_TIER_C`; en v5.16 son 11 JUEVES (jue 21-ene = D3 … jue 1-abr = D53) y el 12.º cae en **mié 7-abr = D57**, último día del plan) | foco igual que arriba (15Q) | **10Q = `preguntasSinUsar(tierC.capId, usadas)`**; si faltan, completar con `preguntasSinUsarDeAsignatura(tierC.num, usadas)`; si sigue faltando, test del capítulo ProMIR (`capUrl(tierC.capId)`) | **entrada propia** `mantenimiento` con `asignatura = tierC.asignatura`, `capId = tierC.capId`, `qIds` = las 10 (NO se mezclan con la asignatura foco: así entran en `mirStatsPorAsignatura` y en la tabla del handoff 7-abr) |
| viernes | `preguntasSinUsarDeAsignatura(mirMantFoco(dia, mirPeorAsignatura()).num, usadas).slice(0, 30)` | — | `mantenimiento` (neto semanal) |

**Cableado en la UI el 19-sep-2026** (`MantenimientoView`, fechas v5.16 del 26-sep): foco/reducido/viernes reparten `qIds` según la tabla; los días con `dia.tierC` hay un **2.º `EvalForm`** (kind `mantenimiento`, `asignatura = tierC.asignatura`, `capId = tierC.capId`, `num = tierC.num`, `qIds` = capítulo → asignatura vía `poolConFallback`) y los chips "TIER C EXPRESS · …" y "próximo Tier C: … (M#)" (`mirMantProximoTierC`). Las 10Q Tier C **salen de `nQ`**: en los **4** jueves reducidos (M3 21-ene · M8 28-ene · M13 4-feb · M18 11-feb = día del examen Step 1, 10Q) el día es solo Tier C (sin formulario foco); en los normales (25Q) = 15Q foco + 10Q Tier C. El `EvalForm` distingue "ya registrado hoy" por asignatura cuando el kind es `mantenimiento` (dos entradas el mismo día).
