...
-- ═══════════════════════════════════════════════════════════════════════════════════════
-- 11) SEGUNDA CAPA Palmerton v3b (12-sep-2026 · gaps_v3b_research 8 y 9) — Mesa editorial persistente + RLS que faltaba + motor.
--     Copia canónica: DATA/_scripts/_migrations/research_entregables.sql (aplicada como research_entregables_palmerton_v3b;
--     verificado el 13-sep-2026 en pg_policies: research_entregables / research_manuscripts / research_citations con RLS ON +
--     policy "Allow all" [ALL/true/true]; research_engine_state con sources_ok jsonb + last_error text). Escritores: la app
--     (src/lib/researchEntregablesSync.ts, anon) · lector de la Mesa: RUTA_PUBLICACION_2027.md §9 (helper mesaMarkdown(), solo lectura).
--     research-discovery v3 (supabase/functions/research-discovery/index.ts) escribe sources_ok/last_error — NO desplegada aún (secrets OPENALEX_KEY/NCBI_KEY pendientes).
-- ═══════════════════════════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════
