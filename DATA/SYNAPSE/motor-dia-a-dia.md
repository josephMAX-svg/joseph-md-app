...
|---|---|---|
| Datos del plan | `src/lib/synapseDailyPlan.ts` | **GENERADO** — 81 días (12 semanas, **lun 14-sep → jue 3-dic-2026 · v5.10**; regenerar con `node DATA/_scripts/gen_synapse_plan.js YYYY-MM-DD`), bloques A/B/C/PC/R por día |
...
- **PC (sábado 15:00-17:00)** — **SHIP del proyecto de la semana** del vibecoding 04:15 (criterio de aceptación en `DATA/SYNAPSE/vibecoding_proyectos.json`). Desde v5.7 el generador lee el `ship` REAL de `VIBE_PROYECTOS` en vez de asumir "sábado de la semana n". Nada se arrastra a la semana siguiente.
  ...
  ⚠ **Sigue el desajuste de borde**: el **SHIP S12 = sáb 5-dic-2026** cae fuera de los 81 días del plan SYNAPSE,
  que termina el jue 3-dic-2026 (último sábado 28-nov). **Manda `VIBE_PROYECTOS.ship`**: ese sábado se ejecuta
