...
> **v5.14 · corrimiento (19-sep-2026)**: D1 = **lun 21-sep-2026** (ni el jue 17 ni el vie 18-sep se estudiaron; decimotercer corrimiento 31-ago→21-sep, 15 hábiles perdidos) →
> `node DATA/_scripts/gen_synapse_plan.js 2026-09-21` → **131 días · 19 semanas completas · lun 21-sep-2026 → vie 29-ene-2027** (110 A-units
> intactas; sem 1-18 lun-dom; sem 19 = lun 25 → vie 29-ene, de 5 días). Contado con node el 19-sep: F0 sem 1-8 (lun 21-sep → dom 15-nov, 48 A) ·
> F1 sem 9-12 (lun 16-nov → dom 13-dic, 24 A) · F2 sem 13-19 (lun 14-dic → vie 29-ene, 38 A). Los PC de sábado siguen el `ship` REAL de
> `VIBE_PROYECTOS` (v5.14: SHIP S1 sáb 26-sep (d6) … S12 sáb 12-dic (d83), los mismos sábados que en v5.11-v5.13; con D1 en lunes ya no hay sábado "de avance" antes del primer SHIP; 18 PC en total = 12 SHIP + 6 opcionales del taper, el último el sáb 23-ene (d125). ⚠ El PC del sáb 9-ene (d111) dice "Sábado 2-ene PC": texto posicional del generador, **A VERIFICAR (19-sep)**, corregir en `gen_synapse_plan.js`).
> Con D1 en lunes la **última A-unit (d131) cae el VIERNES 29-ene**, D92 del Step 1 = cierre de la última semana completa de banco (D88-D92) (v5.13: mar 26-ene = D91; v5.12: lun 25-ene; v5.11: sáb 23-ene; v5.10: vie 22-ene):
> la opción de adelantarla al vie 22-ene sigue abierta como decisión de Joseph (`PENDIENTES_JOSEPH.md`). El texto de esa A-unit ya dice "CIERRE F2 (último día del motor, vie 29-ene en v5.14)" y la cabecera del `.ts` y `synapseData.ts` también (verificado el 19-sep con node). Contexto: el Step 1 termina el mié 3-feb (D95 = D-1 dentro del plan; D94 mar 2-feb = última sesión de banco) y su examen pasa al **jue 4-feb-2027**; sáb 30 y dom 31-ene libres; desde el lun 1-feb solo Step 1.
...
> (fin del Step 1 menos la semana del examen; re-fechado a v5.11 el 14-sep, a v5.12 el 15-sep, a v5.13 el 16-sep y a v5.14 el 19-sep — 131 días, lun 21-sep → vie 29-ene —, ver arriba). Los **días 1-81 son idénticos** a v5.10 (comprobado bloque a bloque: cabecera +
...
> libre (bloque R, sin A-unit). Semana del examen (v5.14: lun 1 → mié 3-feb = D93-D95, examen jue 4-feb): sin SYNAPSE. Vibecoding: S13-S20 taper (ver CURSO_IA_04H_31AGO.md).
>
...
|---|---|---|
| Datos del plan | `src/lib/synapseDailyPlan.ts` | **GENERADO** — 131 días (19 semanas, **lun 21-sep-2026 → vie 29-ene-2027 · v5.14**; regenerar con `node DATA/_scripts/gen_synapse_plan.js YYYY-MM-DD`), bloques A/B/C/PC/R por día, `deload` en sem 17-19 |
| Vibecoding 04:15 | `src/lib/vibecodingPlan.ts` + `DATA/SYNAPSE/vibecoding_proyectos.json` | **GENERADO** — 12 proyectos S1-S12 (60 días = 12 semanas de calendario, lun 21-sep → vie 11-dic, SHIP sábados 26-sep → 12-dic) + taper S13-S20 (35 días en bloques secuenciales, lun 14-dic → mié 3-feb) = 95 días (`node DATA/_scripts/gen_vibecoding_plan.js YYYY-MM-DD`, v5.14: `2026-09-21`); `verificacion[]` por criterio; `VIBE_SHIP_LOG` horneado desde `_vibecoding_ship.json` |
...
- **PC (sábado 15:00-17:00)** — **SHIP del proyecto de la semana** del vibecoding 04:15 (criterio de aceptación en `DATA/SYNAPSE/vibecoding_proyectos.json`). Desde v5.7 el generador lee el `ship` REAL de `VIBE_PROYECTOS` en vez de asumir "sábado de la semana n". Nada se arrastra a la semana siguiente.
  ...
  ✔ **Re-verificado el 14-sep (v5.11), el 15-sep (v5.12), el 16-sep (v5.13) y el 19-sep (v5.14)**: los SHIP siguen en los mismos sábados — en v5.14 los bloques del vibecoding vuelven a ser lun→vie y el primer sábado del plan ya es SHIP:
  sáb 26-sep (d6) = SHIP S1 … sáb 12-dic (d83) = SHIP S12; 18 PC en total (12 SHIP + 6 opcionales del taper; el sáb 23-ene, d125, es el último PC; el PC del sáb 9-ene, d111, dice "Sábado 2-ene": texto posicional, A VERIFICAR 19-sep). Sábados de sem 13-16 = "PC opcional · taper" (30': `verify_vibecoding.js --sensores` + retro);
...
- **F0 · La Escuela de Anthropic = semanas 1-8** (48 bloques A contados con node el 19-sep · **lun 21-sep → dom 15-nov-2026**, v5.14; en v5.13 eran 45, en v5.12 46 y en v5.11 47): Academy + AI Fluency
...
- **F2 · Academy restante + prep CCA-F = semanas 13-19** (38 bloques A · **lun 14-dic-2026 → vie 29-ene-2027**, taper del Step 1; v5.14; en v5.13 eran 41 hasta el mar 26-ene, en v5.12 40 hasta el lun 25-ene y en v5.11 39 hasta el sáb 23-ene):
  ...
  Academy (12-sep): la última A-unit (vie 29-ene, d131) lo deja como **A VERIFICAR** (real:false, honesto).
