...
> **v5.16 · corrimiento RÍGIDO (26-sep-2026)**: D1 = **lun 28-sep-2026** (ni el mié 23, ni el jue 24 ni el vie 25-sep se estudiaron; decimoquinto corrimiento 31-ago→28-sep, 20 hábiles perdidos; **no se toca ningún contenido: solo corren los días y el fin se AMPLÍA**) →
> `node DATA/_scripts/gen_synapse_plan.js 2026-09-28` → **131 días · 19 semanas · lun 28-sep-2026 → vie 5-feb-2027** (110 A-units
> intactas; **sem 1 vuelve a ser completa = lun 28-sep → dom 4-oct, 7 días**; sem 2-18 lun-dom; sem 19 = lun 1-feb → vie 5-feb, de 5 días). Contado el 26-sep: F0 sem 1-8 (lun 28-sep → dom 22-nov, 48 A) ·
> F1 sem 9-12 (lun 23-nov → dom 20-dic, 24 A) · F2 sem 13-19 (lun 21-dic → vie 5-feb, 38 A). Los PC de sábado siguen el `ship` REAL de
> `VIBE_PROYECTOS` (v5.16: SHIP S1 sáb 3-oct (d6) … S12 sáb 19-dic (d83), los MISMOS sábados que en v5.15 porque los bloques del vibecoding vuelven a ser lun→vie; ya no hay sábado "de avance" antes del primer SHIP; **18 PC en total = 12 SHIP + 6 opcionales del taper** (sáb 26-dic d90 · 2-ene d97 · 9-ene d104 · 16-ene d111 · 23-ene d118 · 30-ene d125), el último el sáb 30-ene (d125); el sáb 6-feb ya queda fuera del plan.)
> La **última A-unit (d131) cae el VIERNES 5-feb**, D92 del Step 1 = último día del sprint final de banco (D88-D92) (v5.15: lun 1-feb = D91; v5.14: vie 29-ene = D92; v5.13: mar 26-ene = D91; v5.12: lun 25-ene; v5.11: sáb 23-ene; v5.10: vie 22-ene):
> la opción de adelantarla sigue abierta como decisión de Joseph (`PENDIENTES_JOSEPH.md`). El texto de esa A-unit ya dice "CIERRE F2 (último día del motor, vie 5-feb en v5.16)" y la cabecera del `.ts` también (verificado el 26-sep). Contexto: el Step 1 termina el mié 10-feb (D95 = D-1 real; D94 mar 9-feb = última sesión de banco; ya sin finde libre entre medio) y su examen pasa al **jue 11-feb-2027**; desde el lun 8-feb solo Step 1.
...
> (fin del Step 1 menos la semana del examen; re-fechado a v5.11 el 14-sep, a v5.12 el 15-sep, a v5.13 el 16-sep, a v5.14 el 19-sep, a v5.15 el 22-sep y a v5.16 el 26-sep — 131 días, lun 28-sep → vie 5-feb —, ver arriba). Los **días 1-81 son idénticos** a v5.10 (comprobado bloque a bloque: cabecera +
...
> **F2 · sem 13-19 (47 días en v5.16; 50 en v5.15) = Anthropic Academy restante + prep CCA-F** a 30': Building with the Claude API (12 módulos) ·
...
> libre (bloque R, sin A-unit). Semana del examen (v5.16: lun 8 → mié 10-feb = D93-D95, examen jue 11-feb; v5.15: mar 2 → vie 5-feb, examen lun 8-feb): sin SYNAPSE. Vibecoding: S13-S20 taper (ver CURSO_IA_04H_31AGO.md).
>
...
|---|---|---|
| Datos del plan | `src/lib/synapseDailyPlan.ts` | **GENERADO** — 131 días (19 semanas, **lun 28-sep-2026 → vie 5-feb-2027 · v5.16**; v5.15: 132 días, mié 23-sep → lun 1-feb; regenerar con `node DATA/_scripts/gen_synapse_plan.js YYYY-MM-DD`), bloques A/B/C/PC/R por día, `deload` en sem 17-19 |
| Vibecoding 04:15 | `src/lib/vibecodingPlan.ts` + `DATA/SYNAPSE/vibecoding_proyectos.json` | **GENERADO** — 12 proyectos S1-S12 (60 días = 12 semanas de calendario lun→vie: lun 28-sep → vie 18-dic, SHIP sábados 3-oct → 19-dic) + taper S13-S20 (35 días en bloques secuenciales, lun 21-dic → mié 10-feb) = 95 días (`node DATA/_scripts/gen_vibecoding_plan.js YYYY-MM-DD`, v5.16: `2026-09-28`; v5.15: `2026-09-23`, bloques mié→mar); `verificacion[]` por criterio; `VIBE_SHIP_LOG` horneado desde `_vibecoding_ship.json` |
...
- **PC (sábado 15:00-17:00)** — **SHIP del proyecto de la semana** del vibecoding 04:15 (criterio de aceptación en `DATA/SYNAPSE/vibecoding_proyectos.json`). Desde v5.7 el generador lee el `ship` REAL de `VIBE_PROYECTOS` en vez de asumir "sábado de la semana n". Nada se arrastra a la semana siguiente.
  ...
  ✔ **Re-verificado el 14-sep (v5.11), el 15-sep (v5.12), el 16-sep (v5.13), el 19-sep (v5.14), el 22-sep (v5.15) y el 26-sep (v5.16)**: en v5.16 los SHIP caen en los MISMOS sábados que en v5.15 (3-oct → 19-dic) porque los bloques del vibecoding vuelven a ser lun→vie, y el primer sábado del plan ya es SHIP:
  sáb 3-oct (d6) = SHIP S1 … sáb 19-dic (d83) = SHIP S12; 18 PC en total (12 SHIP + 6 opcionales del taper: sáb 26-dic (d90, S13) · 2-ene (d97, S13) · 9-ene (d104, S14) · 16-ene (d111, S15) · 23-ene (d118, S17) · 30-ene (d125, S18); el sáb 30-ene, d125, es el último PC). *(v5.15: sáb 26-sep (d4) de avance, SHIP 3-oct (d11) → 19-dic (d88), 19 PC.)* Sábados de sem 13-16 = "PC opcional · taper" (30': `verify_vibecoding.js --sensores` + retro);
...
- **F0 · La Escuela de Anthropic = semanas 1-8** (48 bloques A contados el 26-sep · **lun 28-sep → dom 22-nov-2026**, v5.16 — la semana 1 vuelve a ser completa; en v5.15 eran 46 (mié 23-sep → dom 15-nov), en v5.14 48, en v5.13 45, en v5.12 46 y en v5.11 47): Academy + AI Fluency
...
- **F1 · stack del vibecoding = semanas 9-12** (24 bloques A · **lun 23-nov → dom 20-dic-2026**; v5.15: lun 16-nov → dom 13-dic): sem 9 Claude Code core (memoria · skills · hooks · subagentes; docs + Academy) · sem 10 MCP · headless · Agent SDK · plugins/workflows · sem 11 Supabase (RLS · select · triggers · webhooks · Edge Functions) · sem 12 n8n + Claude Code in Action + best practices. Temarios extraídos con WebFetch el 5-sep-2026 (`curricula/claude-code-docs.md`, `academy-*.md`, `supabase-docs.md`, `n8n-docs.md`; 106/106 URLs OK en check_links). CS50P/Automate quedan como biblioteca de consulta (Python se lee, no se memoriza).
- **F0 no se reinicia**: sus 48 A-units (v5.16; 46 en v5.15) llevan `audit:true` (chip "auditar: ✓ si ya hecho") porque F0 y la minifase corrieron jun-ago con progreso solo en localStorage. Día 1 = auditoría de 5' en "My courses" de la Academy.
...
- **F2 · Academy restante + prep CCA-F = semanas 13-19** (38 bloques A · **lun 21-dic-2026 → vie 5-feb-2027**, taper del Step 1; v5.16; en v5.15 eran 40 hasta el lun 1-feb, en v5.14 38 hasta el vie 29-ene, en v5.13 41 hasta el mar 26-ene, en v5.12 40 hasta el lun 25-ene y en v5.11 39 hasta el sáb 23-ene):
  ...
  Academy (12-sep): la última A-unit (vie 5-feb, d131 en v5.16; v5.15: lun 1-feb, d132) lo deja como **A VERIFICAR** (real:false, honesto).
