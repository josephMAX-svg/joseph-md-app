# 🧠 FRANJA 04:15–05:45 (v5.2 · serie desde lun 31-ago-2026 · D1 efectivo MIÉ 23-SEP-2026, v5.15) — IA VIBECODING + ANKI AM

> **v5.15 (22-sep-2026) · corrimiento RÍGIDO:** ni el lun 21 ni el mar 22 de septiembre se estudiaron → D1 efectivo = **mié 23-sep-2026**
> (decimocuarto corrimiento, 31-ago→23-sep; 17 hábiles perdidos). Regla de este corrimiento: **no se toca NINGÚN tema ni contenido — solo corren los días, y donde había un fin clavado se AMPLÍAN días**; el
> desfase se absorbe alargando el final. La franja, el reparto 45'+45' y las metas NO cambian; solo se
> re-fecharon los planes: **vibecoding S1-S12 = 12 bloques de 5 hábiles que YA NO coinciden con las semanas de calendario (van de MIÉRCOLES a MARTES: mié 23-sep → mar 15-dic-2026;
> el SHIP cae el sábado siguiente al cierre de cada bloque: 3-oct → 19-dic, una semana más tarde que en v5.11-v5.14) + taper
> S13-S20 = bloques SECUENCIALES de `pasos.length` hábiles (mié 16-dic → vie 5-feb-2027; 95 días en total = los del Step 1, d == D#; ningún paso perdido)** y
> **SYNAPSE 132 días · 19 semanas (mié 23-sep-2026 → lun 1-feb-2027; 110 A-units intactas: F0 46 · F1 24 · F2 40; F0 sem 1-8 · F1 sem 9-12 · F2 sem 13-19, deload sem 17-19; la **semana 1 es corta**: mié 23 → dom 27-sep, 5 días)**. ⚠ **El Step 1 termina el vie 5-feb (D95 = D-1 dentro del plan; D94 jue 4-feb = última sesión de banco) → examen target LUN 8-FEB-2027**; sáb 6 y dom 7-feb libres entre D95 y el examen (solo Anki vencido). El Anki AM sigue hasta el vie 5-feb (sesión mínima) y el finde solo lo vencido. Fechas leídas de `vibecodingPlan.ts` / `synapseDailyPlan.ts` /
> `usmleStep1Daily.ts` el 22-sep, no estimadas. La **última A-unit de SYNAPSE (d132) cae el LUNES 1-feb = D91 del Step 1** (dentro del sprint final de banco D88-D92): la opción de adelantarla sigue abierta como **decisión de Joseph** (`PENDIENTES_JOSEPH.md`).
> *(v5.14, 19-sep: D1 lun 21-sep · vibecoding lun 21-sep → vie 11-dic, SHIP 26-sep → 12-dic · taper 14-dic → 3-feb · SYNAPSE lun 21-sep → vie 29-ene, 131 días, F0 48 · F1 24 · F2 38, última A-unit vie 29-ene = D92 · examen jue 4-feb.)*
...
- **Currículo (v5.10-b, RE-SECUENCIADO el 12-sep con lo ya construido; re-fechado a v5.15 el 22-sep)**: `DATA/SYNAPSE/VIBECODING_12_PROYECTOS.md` — 12 proyectos S1-S12
  (**mié 23-sep → mar 15-dic-2026**, 5 pasos hábiles cada uno = un bloque mié→mar que YA NO coincide con la semana de calendario; SHIP en los sábados **3-oct → 19-dic**) ordenados por
  ...
  **S1** APEX end-to-end (redeploy n8n + 1 APEX USMLE íntegro en Anki y vault + ruteo por subtema) · **S2** Anki sync + KPI
  ...
  tablas sin RLS · **S7** motor de preguntas ENCAPS desde el stock del banco propio (proyecto con flag `deload`: en v5.15 corre **mié 4 → mar 10-nov**, un bloque DESPUÉS de la
  deload post-NBME 26 (D25 mar 27-oct), que es S6 `rls-datos-tesis` (mié 28-oct → mar 3-nov) y va sin flag; la otra semana post-NBME es S12 (mié 9 → mar 15-dic, tras el NBME 28 del mar 8-dic) — **decisión de Joseph**: mover el flag a S6/S12 o dejarlo en S7) · **S8** pool MIR (cuadernillos 2022-2026: clasificación verificada + UI 10Q) ·
...
- **Taper S13-S20 (mié 16-dic-2026 → vie 5-feb-2027, 35 días = D61-D95; el día d del vibecoding = D# del Step 1, 95 en total; desde v5.11 son bloques SECUENCIALES de `pasos.length` hábiles, ningún paso perdido)**:
  S13 16-22 dic · S14 23-30 dic (25-dic libre) · S15 4-7 ene (4 pasos; 31-dic y 1-ene libres) · S16 8-12 ene (3 pasos) = **mantenimiento ≤15'/día** con flag deload (sensores verdes: `node DATA/_scripts/verify_vibecoding.js
  --sensores` · 1 mejora pequeña · retro) y el resto del bloque = journal + lectura de docs; S17 13-19 ene · S18 20-26 ene · S19 27-ene → 2-feb · S20 3-5 feb (Fases B-C)
  = **deload total**: journal 5' + audio, PC sáb/dom opcional (30' máx, solo VERDE); S20 = cierre del plan (mié 3 · jue 4 · vie 5-feb = D93-D95,
  jue 4-feb = D94 última sesión de banco del Step 1; vie 5-feb = D95 = D-1 (journal 5'); sáb 6 y dom 7-feb libres; lun 8-feb EXAMEN). Ningún proyecto nuevo; la decisión de febrero (IA vs ENCAPS intensivo) sigue intacta.
...
- **SYNAPSE 12:30 sem 13-19 (lun 14-dic → lun 1-feb)**: Anthropic Academy restante + prep CCA-F (F2 del motor, 50 días, 40 A-units; sem 17-19
  deload desde el lun 11-ene; la última A-unit cae el lun 1-feb, D91 del Step 1 — A DECIDIR si se adelanta). La página oficial del examen CCA-F no aparece enlazada en academy.claude.com ni en anthropic.skilljar.com
  (verificado 12-sep): **A VERIFICAR** antes de fijar fecha (post-Step 1, feb-2027).

## 05:00–05:45 · 🇺🇸 USMLE — ANKI AM (L-V, hasta el vie 5-feb = D95 = D-1, sesión mínima; sáb 6 y dom 7-feb solo vencido; examen lun 8-feb)
...
- **Fase A (sep-ene, D1-D81 = mié 23-sep-2026 → lun 18-ene-2027)**: 45' pasada principal FSRS. El bloque de 07:15 queda para el repaso anclado
  D-1/D-3/D-7 + free recall + Anki restante.
- **Fases B-C (ene-feb, B = D82-D86 mar 19→lun 25-ene · C = D87-D95 mar 26-ene→vie 5-feb-2027)**: 05:00-05:12 🔥 **STRESS SET** (10Q uWorld random en 12 min — confiar en
...
- Al terminar el Step 1 (examen lun 8-feb-2027), esta media franja vuelve a IA o a ENCAPS intensivo (se
  decide en la reestructuración de febrero).

## Decisión sobre las academias de Business (27-ago)

- **LIVIANO** — única academia activa hasta enero (17:15–18:00, plan de 90 días en la app: **mié 23-sep-2026 → vie 29-ene-2027**, v5.15; sus 16 casos corren una semana (caso 1 vie 2-oct = D8) y el caso integral (vie 29-ene, D90) vuelve a caer DESPUÉS del repaso (mar 26-ene, D87), del capstone (mié 27-ene, D88) y de la trimestral II (jue 28-ene, D89) — ver `DATA/BUSINESS/LIVIANO_ACADEMIA.md`).
