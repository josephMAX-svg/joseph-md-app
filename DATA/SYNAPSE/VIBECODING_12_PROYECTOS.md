...
- **Deload**: Semanas DELOAD de los frentes secundarios (PROTOCOLO_MODO_MINIMO §4, fechas intactas, carga 50%): post-NBME 26 (vie 30-oct) = lun 2 → vie 6-nov-2026 y post-NBME 28 (vie 11-dic) = lun 14 → vie 18-dic-2026. Con el orden v5.10-b el proyecto con `deload: true` es S7 (motor de preguntas ENCAPS), que en la v5.16 corre lun 9 → vie 13-nov: con D1 en lunes los bloques de 5 hábiles coinciden con las semanas de calendario, así que la semana deload post-NBME 26 (2-6 nov) es exactamente S6 y el proyecto con el flag (S7) cae la semana siguiente — decisión pendiente de Joseph: intercambiar S6↔S7 o mover el flag `deload` a S6 (el orden por riesgo no se toca sin su OK; el flag sigue en S7). La semana post-NBME 28 (14-18 dic) = S12 completa (proyecto normal) + S13 taper desde el lun 21-dic (≤15'/día).
- **Taper S13-S20**: S13-S16 (21-dic → 15-ene, bloques secuenciales): MANTENIMIENTO ≤15'/día con flag deload (sensores verdes: node DATA/_scripts/verify_vibecoding.js --sensores · 1 mejora pequeña · retro) y el resto del bloque 04:15 = journal + lectura de docs (sin teclado pesado). S17-S20 (18-ene → 10-feb, cierre de Fase A + Fases B-C del Step 1): DELOAD TOTAL del vibecoding = journal 5' + audio (bloque B del SYNAPSE); PC sáb/dom OPCIONAL (30' máx, solo en día VERDE). S20 = cierre del plan: lun 8 → mié 10-feb (D93-D95; mar 9-feb = D94 última sesión de banco, mié 10-feb = D95 = D-1 real), jue 11-feb EXAMEN (v5.16). Ningún proyecto nuevo después de S12; la decisión de febrero (IA vs ENCAPS intensivo) sigue intacta.
...
|---|---|---|---|---|---|
| S1 | APEX end-to-end: redeploy n8n + 1 APEX USMLE íntegro en Anki y vault + ruteo por subtema | yocpmd | lun 28-sep → vie 02-oct | sáb 03-oct | — |
| S2 | Anki: sync + KPI '1ª review del día' — telemetría v2 en producción | yocpmd | lun 05-oct → vie 09-oct | sáb 10-oct | — |
| S3 | Espejo Supabase del progreso (plan_checks) + exportar/importar | yocpmd | lun 12-oct → vie 16-oct | sáb 17-oct | — |
| S4 | verify_vibecoding + revisión semanal cerrada (10 métricas con dato real) | yocpmd | lun 19-oct → vie 23-oct | sáb 24-oct | — |
| S5 | Puente VITALS → revisión (sueño y agua cruzados con % eval) | yocpmd | lun 26-oct → vie 30-oct | sáb 31-oct | — |
| S6 | RLS en datos_tesis y kappa_piloto + auditoría de tablas sin RLS | yocpmd | lun 02-nov → vie 06-nov | sáb 07-nov | — |
| S7 | Motor de preguntas ENCAPS semiautomático (stock del banco propio) — semana DELOAD | preguntas | lun 09-nov → vie 13-nov | sáb 14-nov | **sí (50%)** |
| S8 | Pool MIR: clasificación verificada de los cuadernillos 2022-2026 + UI de 10Q | preguntas | lun 16-nov → vie 20-nov | sáb 21-nov | — |
| S9 | Migrador de overlays de hitos del Calendar (USMLE + Research) tras cada corrimiento | yocpmd | lun 23-nov → vie 27-nov | sáb 28-nov | — |
| S10 | Bot WhatsApp → OCR → ficha LIVIANO (n8n Webhook + visión) | bots | lun 30-nov → vie 04-dic | sáb 05-dic | — |
| S11 | Pipeline de contenido de marcas (brief → 3 guiones + títulos + thumbnail) con la API de Claude | contenido | lun 07-dic → vie 11-dic | sáb 12-dic | — |
| S12 | Capstone: README del sistema YoCPMD + demo 5' + retro de 12 semanas | yocpmd | lun 14-dic → vie 18-dic | sáb 19-dic | — |
| S13 | Mantenimiento 1: sensores verdes tras la semana deload post-NBME 28 | taper · mantenimiento | lun 21-dic → lun 28-dic (15'/día) | sáb 02-ene (opcional 30') | **sí (mantenimiento)** |
| S14 | Mantenimiento 2: revisión semanal automática y export | taper · mantenimiento | mar 29-dic → mié 06-ene (15'/día) | sáb 09-ene (opcional 30') | **sí (mantenimiento)** |
| S15 | Mantenimiento 3: primera semana de enero (4 hábiles · NBME 30) | taper · mantenimiento | jue 07-ene → mar 12-ene (15'/día) | sáb 16-ene (opcional 30') | **sí (mantenimiento)** |
| S16 | Mantenimiento 4: sensores verdes antes del UWSA2 y del cierre de Fase A | taper · mantenimiento | mié 13-ene → vie 15-ene (15'/día) | sáb 16-ene (opcional 30') | **sí (mantenimiento)** |
| S17 | Deload total 1: journal 5' + audio (cierre de Fase A → GO/NO-GO) | taper · deload | lun 18-ene → vie 22-ene (5'/día) | opcional / no | **sí (deload)** |
| S18 | Deload total 2: journal 5' + audio (semana NBME 32/33 + Free 120) | taper · deload | lun 25-ene → vie 29-ene (5'/día) | opcional / no | **sí (deload)** |
| S19 | Deload total 3: journal 5' + audio (Fase C, última semana con SYNAPSE) | taper · deload | lun 01-feb → vie 05-feb (5'/día) | opcional / no | **sí (deload)** |
| S20 | Cierre del plan: journal 5' (lun 8, mar 9 y mié 10-feb), jue 11-feb EXAMEN | taper · deload | lun 08-feb → mié 10-feb (5'/día) | opcional / no | **sí (deload)** |

## S1 · APEX end-to-end: redeploy n8n + 1 APEX USMLE íntegro en Anki y vault + ruteo por subtema

- **Semana**: lun 28-sep → vie 02-oct · **SHIP**: sábado sáb 03-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S1] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 28-09 · 5' objetivo → spec del día**: Inventario de lo hecho: correr node scripts/test_parser_multilinea.js (verde desde el 5-sep) y leer n8n_nodos_nuevos.md + n8n_flow_4_destinos.json para ubicar el nodo Code del parser. Objetivo del día: saber EXACTAMENTE qué fichero pegar y dónde; nada de reescribir el parser.
  - **Mar 29-09 · construir con Claude Code**: Redeploy: pegar n8n_parser_v2_3.js íntegro en el nodo Code de n8n (copiar el fichero, no editar en la UI), guardar, activar; disparar 1 APEX de prueba de USMLE (Ctrl+Shift+A, is_test=true) y ver la ejecución verde en Executions.
  - **Mié 30-09 · construir con Claude Code**: Ruteo por subtema: comprobar que la tarjeta cayó en APEX::USMLE::<sistema> (ensure_anki_subdecks.py / _mapping_slim.json) y la nota en la carpeta correcta de 01_USMLE; si cae en el deck raíz o en 99_INBOX, corregir el mapping y repetir el envío.
  - **Jue 01-10 · verificar en vivo (test/URL/dato)**: Verificar en vivo: abrir Anki (REVERSO, CASO_CLINICO y FISIO_EXPANDIDA completos) y el vault (secciones renderizadas); escribir DATA/USMLE/_apex_e2e_prueba.json con noteId, subdeck, ruta de la nota y hash del parser (certutil -hashfile).
  - **Vie 02-10 · doc + commit + preparar SHIP**: Commit [S1] en joseph-md-app (evidencia + doc del redeploy con fecha y hash; opcional: git init LOCAL en D:/agente_estudio con .gitignore — decisión de Joseph) + node DATA/_scripts/verify_vibecoding.js 1 + entrada del journal (node DATA/_scripts/journal_hoy.js). Lo que quede (T1/T2/T3 Palmerton = P1-6) NO entra: es otra semana.
...
- **Semana**: lun 05-oct → vie 09-oct · **SHIP**: sábado sáb 10-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S2] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 05-10 · 5' objetivo → spec del día**: Correr node DATA/_scripts/anki_telemetria.js con Anki abierto (hace sync antes de leer) y comparar due/backlog/revisadas/primeraReview con la pantalla Stats de Anki; anotar discrepancias (patrón APEX::USMLE* vs AnKing). Objetivo del día: números que cuadran ±5%.
  - **Mar 06-10 · construir con Claude Code**: Retención y config: validar retencion30 (review-level, getReviewsOfCards) y que getDeckConfig devuelve 10 nuevas/día y desired retention 0.90 (si no, corregir en Anki, no en el script).
  - **Mié 07-10 · construir con Claude Code**: Automatizar: schtasks /create /tn "JMD Anki telemetria" /tr "node D:\joseph-md-app\DATA\_scripts\anki_telemetria.js --registrar-cerrado" /sc daily /st 21:05 (o hook SessionEnd de Claude Code); comprobar que escribe sin ventana abierta y que deja huella si Anki está cerrado.
  - **Jue 08-10 · verificar en vivo (test/URL/dato)**: KPI en Home: CockpitStatusBar lee primeraReview / primeraReviewEstado de jmd-anki-telemetria y colorea el instrumento ANKI (verde/ámbar/rojo); verificar en vivo con el one-liner que imprime el script.
  - **Vie 09-10 · doc + commit + preparar SHIP**: Commit [S2] + node DATA/_scripts/verify_vibecoding.js 2 + journal. Regla 'Anki finde = due × 20 s' ya está en DATA/SYNC_ANKI_OBSIDIAN_APP.md: enlazarla desde el KPI.
...
- **Semana**: lun 12-oct → vie 16-oct · **SHIP**: sábado sáb 17-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S3] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 12-10 · 5' objetivo → spec del día**: Leer plan_checks.sql y studyProgressSync.ts (push del diff en saveDone, pull en el primer loadDone, migración única, export/import); aplicar la migración CON Joseph (apply_migration, backup previo) y comprobar la policy en pg_policies. Objetivo del día: tabla viva.
  - **Mar 13-10 · construir con Claude Code**: Probar el sync en vivo: marcar/desmarcar 3 días en la app → select * from plan_checks (execute_sql) refleja altas y bajas; revisar el log de errores del cliente (nunca debe lanzar).
  - **Mié 14-10 · construir con Claude Code**: Segundo navegador (o móvil): el pull fusiona remoto + local; la migración única sube el JSON existente una sola vez (jmd-study-progress-migrated-v1). Corregir lo que no cuadre en studyProgressSync.ts.
  - **Jue 15-10 · verificar en vivo (test/URL/dato)**: UI: botón Exportar/Importar progreso (portapapeles) en Home si aún no está expuesto; gen_revision_semanal.js lee plan_checks (métricas 6, 7, 9) con fallback al export.
  - **Vie 16-10 · doc + commit + preparar SHIP**: Commit [S3] + node DATA/_scripts/verify_vibecoding.js 3 + journal.
...
- **Semana**: lun 19-oct → vie 23-oct · **SHIP**: sábado sáb 24-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S4] …` en `D:/joseph-md-app`
...
- **Objetivo**: Que la revisión semanal del sáb 24-oct (la que cierra S4, v5.16) salga con ≥8/10 métricas reales y que la métrica 7 lea el último verify del proyecto de la semana, no el auto-reporte.
- **Entregable verificable**: DATA/USMLE/REVISIONES/S04_2026-10-24.md con ≥8/10 métricas reales + _vibecoding_ship.json con S1-S4 + gen_revision_semanal.js leyendo verify/scores/plan_checks + commit [S4].
...
- **Criterio de aceptación (definition of done)** — `node DATA/_scripts/verify_vibecoding.js 4`:
  - [ ] node DATA/_scripts/gen_revision_semanal.js genera DATA/USMLE/REVISIONES/S04_2026-10-24.md con ≤2 métricas 'sin dato' _(verify: fichero)_
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 19-10 · 5' objetivo → spec del día**: Correr node DATA/_scripts/gen_revision_semanal.js y listar cada métrica 'sin dato'; correr node DATA/_scripts/verify_vibecoding.js 1, 2 y 3 y leer _vibecoding_ship.json. Objetivo del día: lista de las 3 conexiones más baratas.
  - **Mar 20-10 · construir con Claude Code**: gen_revision_semanal.js: métrica 7 = último verify del proyecto de la semana (criterios_ok/total + shipped) en lugar de '5/5 días = shipped'; métrica 1 desde usmle_daily_scores / jmd-usmle-scores (usmleScores.ts).
  - **Mié 21-10 · construir con Claude Code**: Métricas 6 y 9 desde plan_checks (S3) con fallback al export de localStorage; métrica 3 desde la telemetría v2 (primeraReview incluido).
  - **Jue 22-10 · verificar en vivo (test/URL/dato)**: Generar S04 en seco, leerla completa y corregir la plantilla (DATA/REVISION_SEMANAL.md) donde no ayude a decidir; contar métricas reales (≥8/10).
  - **Vie 23-10 · doc + commit + preparar SHIP**: Automatizar el pre-relleno (viernes 21:00, schtasks o /schedule) + commit [S4] + node DATA/_scripts/verify_vibecoding.js 4 + journal.
- **SHIP**: SHIP S4: sábado 24-oct 07:15 revisión S04 con ≥8/10 métricas reales y métrica 7 verificada por script (verify 4 → 4/4).

## S5 · Puente VITALS → revisión (sueño y agua cruzados con % eval)

- **Semana**: lun 26-oct → vie 30-oct · **SHIP**: sábado sáb 31-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S5] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 26-10 · 5' objetivo → spec del día**: Consultar mv_wellness_logs (tipo sueno/agua) con la anon key: si RLS bloquea, definir la policy de lectura para el usuario médico o usar el JSON de la revisión como puente. Objetivo del día: 7 noches leídas por script.
  - **Mar 27-10 · construir con Claude Code**: gen_revision_semanal.js: tabla sueño×eval por fecha (join con usmle_daily_scores de usmleScores.ts) + contador de noches <7 h y <6 h.
  - **Mié 28-10 · construir con Claude Code**: CockpitStatusBar: 'Sueño anoche: X h' (último log o JSON) con try/catch y '—' si no hay dato.
  - **Jue 29-10 · verificar en vivo (test/URL/dato)**: TodayMission: si el sueño de anoche <6 h → chip 'sugerido: ÁMBAR' junto al selector de modo (PROTOCOLO_MODO_MINIMO); Joseph decide.
  - **Vie 30-10 · doc + commit + preparar SHIP**: Fuente de datos documentada en PROTOCOLO_MODO_MINIMO.md (sección Fuentes) + commit [S5] + verify 5 + journal.
...
- **Semana**: lun 02-nov → vie 06-nov · **SHIP**: sábado sáb 07-nov (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S6] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 02-11 · 5' objetivo → spec del día**: Inventario: quién lee/escribe datos_tesis y kappa_piloto (grep en agente_estudio + SECURITY_AUDIT_PHASE_A_C.sql) y listado de TODAS las tablas con relrowsecurity=false (execute_sql sobre pg_class); decidir política por tabla. Objetivo del día: _auditoria_rls.md borrador.
  - **Mar 03-11 · construir con Claude Code**: Escribir la migración: alter table … enable row level security + policies (solo service_role + rol médico); probar en rama de Supabase (create_branch) o en seco. Si Joseph ya la aprobó en D1-D2, saltar a la prueba.
  - **Mié 04-11 · construir con Claude Code**: Mover el acceso del pipeline a SUPABASE_SERVICE_ROLE_KEY en .env (gitignored); comprobar que el pipeline sigue insertando.
  - **Jue 05-11 · verificar en vivo (test/URL/dato)**: DATA/_scripts/test_rls_anon.js: con la anon key de src/lib/supabase.ts, SELECT sobre datos_tesis y kappa_piloto → espera 0 filas/401 e imprime "OK"; correr get_advisors (security) y pegar la salida en _auditoria_rls.md.
  - **Vie 06-11 · doc + commit + preparar SHIP**: Aplicar con Joseph (apply_migration, backup previo) + SECURITY_AUDIT.md + commit [S6] + node DATA/_scripts/verify_vibecoding.js 6 + journal.
...
- **Semana**: lun 09-nov → vie 13-nov · **SHIP**: sábado sáb 14-nov (PC SYNAPSE 15:00-17:00) · rotación `preguntas` · commits `[S7] …` en `D:/joseph-md-app`
- **A quién sirve**: ENCAPS — la hora de banqueo 16:15 depende de que el chat genere preguntas con el mapa (pronóstico v3 + TRACKING_ERRORES) y registre la ronda; hay 18 sets / 2.052 preguntas del banco propio, gen_encaps_minisim.js (--banco/--eval/--sim100) y _etiquetas_examenes_reales_v3.json ya construidos: el motor debe SERVIR del stock por subtema del día y registrar la ronda, generando preguntas nuevas solo cuando el stock del subtema se agota. Semana deload post-NBME 26 = lun 2 → vie 6-nov (S6; el flag `deload` sigue en S7 = 9-13 nov, decisión pendiente de Joseph): 2 días de 45' + 3 de ≤15'.
...
- **Pasos diarios (45' cada uno; deload: mar/jue 15')**:
  - **Lun 09-11 · 5' objetivo → spec del día**: (45') Escribir SKILL.md: lee PRONOSTICO v3 + rotación del día + PERFIL_CONOCIMIENTO → sirve 5 viñetas del stock propio (gen_encaps_minisim.js --banco) en formato pregunta→solución interactiva, sin clave hasta que Joseph responda; genera nuevas SOLO si el subtema no tiene stock.
  - **Mar 10-11 · construir con Claude Code (15')**: (15' deload) Revisar el formato de salida con 1 viñeta; nada más.
  - **Mié 11-11 · construir con Claude Code**: (45') Registro: la skill apenda la ronda a _registro_resoluciones.json (esquema README), vuelca RONDAS/<codigo>_<fecha>.json y anota los ids servidos en _stock_usado.json.
  - **Jue 12-11 · verificar en vivo (test/URL/dato) (15')**: (15' deload) Descanso activo: leer 1 ronda anterior y anotar 1 mejora.
  - **Vie 13-11 · doc + commit + preparar SHIP (15')**: (15') 1 ronda real de 5Q en la hora ENCAPS (16:15) registrada + commit [S7] + node DATA/_scripts/verify_vibecoding.js 7 + journal.
...
- **Semana**: lun 16-nov → vie 20-nov · **SHIP**: sábado sáb 21-nov (PC SYNAPSE 15:00-17:00) · rotación `preguntas` · commits `[S8] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 16-11 · 5' objetivo → spec del día**: Correr node DATA/_scripts/gen_mir_pool.js --parse (si aún no) y leer _parse_errores.json + _stats.json; comprobar en 10 preguntas que la clave del JSON coincide con la plantilla definitiva del PDF. Objetivo del día: pool parseado y fiable.
  - **Mar 17-11 · construir con Claude Code**: mir_pool_clasificar.js: clasificar por asignatura/tema (claude -p --json-schema con el temario de mirDailyPlan.ts como lista cerrada) → _clasificacion.json; empezar por 2026 y extender a 2022-2025.
  - **Mié 18-11 · construir con Claude Code**: Verificación: muestra aleatoria de 30 preguntas comprobada A MANO contra el PDF (enunciado, clave, asignatura) → _verificacion.md con % y las correcciones aplicadas.
  - **Jue 19-11 · verificar en vivo (test/URL/dato)**: UI: MirTodayPlan sirve 10Q ciegas del tema del día desde el pool (sin clave hasta responder) y registra la ronda en jmd-mir-eval-log; verificar en vivo.
  - **Vie 20-11 · doc + commit + preparar SHIP**: Commit [S8] + node DATA/_scripts/verify_vibecoding.js 8 + journal.
...
- **Semana**: lun 23-nov → vie 27-nov · **SHIP**: sábado sáb 28-nov (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S9] …` en `D:/joseph-md-app`
...
- **Criterio de aceptación (definition of done)** — `node DATA/_scripts/verify_vibecoding.js 9`:
  - [ ] node DATA/_scripts/gen_hitos_overlays.js 2026-09-28 imprime '12 hitos' y escribe DATA/USMLE/_hitos_overlays.json con fecha = la del .ts para cada uno _(verify: test)_
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 23-11 · 5' objetivo → spec del día**: Leer gen_research_calendar.js (--print/--check/--set, hash por (fecha, título, descripción)) y listar los hitos y su D# desde usmleStep1Daily.ts (DIAS con 🎯); tabla esperada para START=2026-09-28 comparada con CALENDARIO_5_MESES.md. Objetivo del día: tabla de hitos + contrato del script.
  - **Mar 24-11 · construir con Claude Code**: gen_hitos_overlays.js: mismo esquema que el de Research (DATA/USMLE/_hitos_overlays.json con ids + hash + accion); lee el .ts, calcula fechas, sin llamadas al Calendar.
  - **Mié 25-11 · construir con Claude Code**: Descripción del overlay = protocolo test-day generado desde PALMERTON_POR_MATERIA §F (texto fijo + hora de cada break).
  - **Jue 26-11 · verificar en vivo (test/URL/dato)**: Diff en seco: con el MCP (list_events/search_events 'NBME'|'UWSA') comparar fechas y listar solo cambios; corregir falsos positivos.
  - **Vie 27-11 · doc + commit + preparar SHIP**: Doc + commit [S9] + node DATA/_scripts/verify_vibecoding.js 9 + journal. Ejecutar delete/create solo tras confirmación de Joseph (y solo si hubo corrimiento).
...
- **Semana**: lun 30-nov → vie 04-dic · **SHIP**: sábado sáb 05-dic (PC SYNAPSE 15:00-17:00) · rotación `bots` · commits `[S10] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 30-11 · 5' objetivo → spec del día**: Diseño: WhatsApp Cloud API → Webhook n8n → descarga de media → visión (prompt de extracción a JSON) → validación → Supabase → respuesta. Credenciales de WhatsApp: A VERIFICAR con Joseph (cuenta Meta Business). Objetivo del día: diagrama + esquema de la ficha.
  - **Mar 01-12 · construir con Claude Code**: n8n: Webhook node (POST, path propio) + verificación de firma + descarga de la imagen; probar con curl y una imagen local.
  - **Mié 02-12 · construir con Claude Code**: OCR/visión: prompt de extracción con esquema JSON fijo (campos de la ficha) + 2 ejemplos; validar con un nodo Code (campos obligatorios, unidades).
  - **Jue 03-12 · verificar en vivo (test/URL/dato)**: Guardar en Supabase (tabla liviano_fichas, RLS; migración aplicada con Joseph) + respuesta automática; 3 pruebas con imágenes reales y medir latencia.
  - **Vie 04-12 · doc + commit + preparar SHIP**: Exportar el workflow a DATA/BUSINESS/n8n_liviano_ocr.json + doc + commit [S10] + node DATA/_scripts/verify_vibecoding.js 10 + journal.
...
- **Semana**: lun 07-dic → vie 11-dic · **SHIP**: sábado sáb 12-dic (PC SYNAPSE 15:00-17:00) · rotación `contenido` · commits `[S11] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 07-12 · 5' objetivo → spec del día**: Brief por marca (LIVIANO): 3 pilares + tono + 3 temas de la academia; escribir gen_contenido.js que arma el prompt y llama a la API (o claude -p --json-schema) con salida JSON y modo --dry-run. Objetivo del día: dry-run imprimiendo la estructura.
  - **Mar 08-12 · construir con Claude Code**: Guiones: 3 guiones de 45 s (hook-cuerpo-CTA) + 5 títulos; guardar en DATA/BUSINESS/CONTENIDO/liviano/<fecha>/guiones.md.
  - **Mié 09-12 · construir con Claude Code**: Thumbnail: prompt de imagen + 1 imagen generada con la herramienta disponible; marcadores de corte para clips.
  - **Jue 10-12 · verificar en vivo (test/URL/dato)**: Revisión humana (claims clínicos con fuente de la academia) + publicar 1 pieza.
  - **Vie 11-12 · doc + commit + preparar SHIP**: Anotar alcance a 48 h + commit [S11] + node DATA/_scripts/verify_vibecoding.js 11 + journal.
...
- **Semana**: lun 14-dic → vie 18-dic · **SHIP**: sábado sáb 19-dic (PC SYNAPSE 15:00-17:00) · rotación `yocpmd` · commits `[S12] …` en `D:/joseph-md-app`
...
- **Pasos diarios (45' cada uno)**:
  - **Lun 14-12 · 5' objetivo → spec del día**: README_SISTEMA.md: inventario de DATA/_scripts (generadores, remap, telemetría, revisión, verify), hooks y skills; orden del pipeline de corrimiento. Objetivo del día: índice completo, sin prosa.
  - **Mar 15-12 · construir con Claude Code**: Demo 5': grabar (OBS/Loom) Home → Estudio → revisión semanal; subir y enlazar desde el README.
  - **Mié 16-12 · construir con Claude Code**: Retro: correr node DATA/_scripts/verify_vibecoding.js para S1-S11 y rellenar la tabla de VIBECODING_12_PROYECTOS.md con estado + 1 lección cada uno; qué se automatiza en el taper.
  - **Jue 17-12 · verificar en vivo (test/URL/dato)**: Decisión CCA-F: revisar los 5 dominios (Agentic Architecture 27% · Claude Code 20% · Tool Design & MCP 18% · Prompt Engineering 20% · Context Management 15%, según CALIDAD/Synapse.md) contra lo hecho; fijar fecha post-Step 1 (feb-2027) o no.
  - **Vie 18-12 · doc + commit + preparar SHIP**: Commit final [S12] + tag vibecoding-s12 + node DATA/_scripts/verify_vibecoding.js 12 + journal (cierre).
...
### S13 · Mantenimiento 1: sensores verdes tras la semana deload post-NBME 28 · MANTENIMIENTO (15'/día)

- **Semana**: lun 21-dic → lun 28-dic (5 días hábiles) · Step 1: S13 · lun 21 → lun 28-dic (vie 25-dic libre; el NBME 29 cae el lun 28-dic = D65; la deload post-NBME 28 fue S12, 14-18 dic)
...
- **Días**:
  - **Lun 21-12 · sensores verdes (≤15')**: Sensores (≤15'): node DATA/_scripts/verify_vibecoding.js --sensores → todo verde o anotar el rojo; nada más de teclado. Resto: journal 5' + lectura de docs.
  - **Mar 22-12 · 1 mejora pequeña (≤15')**: 1 mejora pequeña (≤15') al sensor que dio rojo el lunes (o ninguna si todo verde); commit [S13].
  - **Mié 23-12 · journal + lectura de docs**: Journal 5' + lectura de docs (15'): 1 página de Claude Code docs pendiente del F1/F2 de las 12:30; sin construir.
  - **Jue 24-12 · retro semanal**: Retro semanal (≤15'): qué se rompió, qué sensor mintió, 1 línea por frente en el journal.
  - **Lun 28-12 · doc + commit + preparar SHIP**: Commit [S13] de lo que haya + journal + node DATA/_scripts/verify_vibecoding.js --sensores (deja constancia en _vibecoding_ship.json).
...
- **Semana**: mar 29-dic → mié 06-ene (5 días hábiles) · Step 1: S14 · mar 29-dic → mié 6-ene (31-dic y 1-ene libres)
...
- **Días**:
  - **Mar 29-12 · sensores verdes (≤15')**: Sensores (≤15'): verify --sensores; comprobar que la tarea programada del viernes 21:00 dejó DATA/USMLE/REVISIONES/S13_*.md.
  - **Mié 30-12 · 1 mejora pequeña (≤15')**: 1 mejora pequeña (≤15'): la métrica que peor se lea en la revisión; commit [S14].
  - **Lun 04-01 · journal + lectura de docs**: Journal 5' + lectura de docs (15'); sin construir.
  - **Mar 05-01 · retro semanal**: Retro semanal (≤15') en el journal.
  - **Mié 06-01 · doc + commit + preparar SHIP**: Commit [S14] + journal + verify --sensores.
- **Sábado/domingo**: Sábado PC (opcional, 30'): verify --sensores + retro.

### S15 · Mantenimiento 3: primera semana de enero (4 hábiles · NBME 30) · MANTENIMIENTO (15'/día)

- **Semana**: jue 07-ene → mar 12-ene (4 días hábiles) · Step 1: S15 · jue 7 → mar 12-ene (4 hábiles; el NBME 30 cae el vie 8-ene = D72)
- **Objetivo**: Sensores verdes con 4 días hábiles (jue 7, vie 8, lun 11 y mar 12-ene); los feriados 31-dic y 1-ene cayeron en S14 y el vie 25-dic en S13.
- **Sensores**: `node DATA/_scripts/verify_vibecoding.js --sensores`
- **Días**:
  - **Jue 07-01 · sensores verdes (≤15')**: Sensores (≤15'): verify --sensores; journal 5'.
  - **Vie 08-01 · 1 mejora pequeña (≤15')**: 1 mejora pequeña (≤15') o ninguna; commit [S15].
  - **Lun 11-01 · journal + lectura de docs**: Journal 5' + lectura de docs (15').
  - **Mar 12-01 · retro semanal**: Retro semanal (≤15') + commit [S15] + verify --sensores (el bloque S15 = jue 7, vie 8, lun 11 y mar 12-ene; NBME 30 el vie 8-ene: el journal solo anota sueño y modo).
- **Sábado/domingo**: Sábado 16-ene PC (opcional, 30'; mismo sábado que el de S16): verify --sensores + retro; si descansas, marca ✓ igual (S15 termina el mar 12-ene: su PC es el sábado siguiente).

### S16 · Mantenimiento 4: sensores verdes antes del UWSA2 y del cierre de Fase A · MANTENIMIENTO (15'/día)

- **Semana**: mié 13-ene → vie 15-ene (3 días hábiles) · Step 1: S16 · mié 13 → vie 15-ene · UWSA2 vie 15-ene (D77) · Fase A sigue hasta el jue 21-ene (D81)
- **Objetivo**: Llegar al UWSA2 (vie 15-ene, último paso de este bloque) y al cierre de Fase A (jue 21-ene) con todos los sensores verdes y el journal al día; 3 días hábiles (mié 13 → vie 15-ene).
- **Sensores**: `node DATA/_scripts/verify_vibecoding.js --sensores` · `node DATA/_scripts/anki_telemetria.js`
- **Días**:
  - **Mié 13-01 · sensores verdes (≤15')**: Sensores (≤15'): verify --sensores + telemetría Anki (backlog = 0 antes del UWSA2 y la Fase B); journal 5'.
  - **Jue 14-01 · 1 mejora pequeña (≤15')**: 1 mejora pequeña (≤15') o ninguna; commit [S16].
  - **Vie 15-01 · retro semanal**: Retro de S13-S16 (≤15') en el journal (UWSA2 hoy, vie 15-ene: el journal anota solo cómo dormiste): qué se mantiene en deload total, qué se apaga hasta febrero. Commit [S16] + verify --sensores.
- **Sábado/domingo**: Sábado 16-ene PC (opcional, 30'): verify --sensores + retro; desde aquí, deload total.

### S17 · Deload total 1: journal 5' + audio (cierre de Fase A → GO/NO-GO) · DELOAD (5'/día)

- **Semana**: lun 18-ene → vie 22-ene (5 días hábiles) · Step 1: S17 · lun 18 → vie 22-ene · cierre de contenido jue 21-ene (D81) · NBME 31 vie 22-ene (D82, GO/NO-GO) abre la Fase B
...
- **Días**:
  - **Lun 18-01 · journal 5' + audio (deload total)**: Journal 5' (objetivo del día del Step 1 · sueño · modo) + audio B (lun 18-ene: post-UWSA2 del vie 15, sin juicios). Nada más.
  - **Mar 19-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (mar 19-ene: D79, bioestadística + ética).
  - **Mié 20-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (mié 20-ene: Bioquímica, 1.er día doble, D80).
  - **Jue 21-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (jue 21-ene: cierre de contenido / Fase A, D81).
  - **Vie 22-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (NBME 31 hoy, vie 22-ene: GO/NO-GO; el journal solo registra sueño y modo, sin juicios).
- **Sábado/domingo**: PC sáb/dom OPCIONAL (30' máx, solo en día VERDE): verify --sensores; si no, nada.

### S18 · Deload total 2: journal 5' + audio (semana NBME 32/33 + Free 120) · DELOAD (5'/día)

- **Semana**: lun 25-ene → vie 29-ene (5 días hábiles) · Step 1: S18 · lun 25 → vie 29-ene · NBME 32 lun 25 (D83) · NBME 33 mié 27 (D85) · Free 120 vie 29-ene (D87)
- **Objetivo**: Igual que S17. Tres hitos en cinco días (NBME 32, NBME 33 y Free 120): el journal solo registra sueño y modo, sin juicios.
- **Días**:
  - **Lun 25-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (NBME 32 hoy, lun 25-ene).
  - **Mar 26-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (mar 26-ene: random timed, D84).
  - **Mié 27-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (NBME 33 hoy, mié 27-ene).
  - **Jue 28-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (jue 28-ene: random timed, D86).
  - **Vie 29-01 · journal 5' + audio (deload total)**: Journal 5' + audio B (Free 120 hoy, vie 29-ene).
...
- **Semana**: lun 01-feb → vie 05-feb (5 días hábiles) · Step 1: S19 · Fase C (lun 1 → vie 5-feb) · banco dentro del sprint D88-D92 · última A-unit del SYNAPSE vie 5-feb
- **Objetivo**: Igual que S17-S18. El SYNAPSE de las 12:30 cierra el vie 5-feb (F2 · CCA-F prep, última A-unit); desde el lun 8-feb solo Step 1.
- **Días**:
  - **Lun 01-02 · journal 5' + audio (deload total)**: Journal 5' + audio B (lun 1-feb).
  - **Mar 02-02 · journal 5' + audio (deload total)**: Journal 5' + audio B (mar 2-feb).
  - **Mié 03-02 · journal 5' + audio (deload total)**: Journal 5' + audio B (mié 3-feb).
  - **Jue 04-02 · journal 5' + audio (deload total)**: Journal 5' + audio B (jue 4-feb).
  - **Vie 05-02 · journal 5' + audio (deload total)**: Journal 5' + audio B (vie 5-feb: última A-unit del SYNAPSE) · cierre del journal S13-S19: 3 líneas para la reestructuración de febrero (IA vs ENCAPS intensivo).
- **Sábado/domingo**: Sáb 6 / dom 7-feb: NADA de IA (último fin de semana antes del examen del jue 11-feb). PC = no.

### S20 · Cierre del plan: journal 5' (lun 8, mar 9 y mié 10-feb), jue 11-feb EXAMEN · DELOAD (5'/día)

- **Semana**: lun 08-feb → mié 10-feb (3 días hábiles) · Step 1: S20 · cierre del plan: lun 8 → mié 10-feb (D93-D95; mar 9-feb = D94 última sesión de banco · mié 10-feb = D95, último día del plan y D-1 real) · jue 11-feb EXAMEN
- **Objetivo**: Solo 5' de journal a las 04:15 los tres últimos días del plan (lun 8, mar 9 y mié 10-feb = D95 = D-1 real); el jue 11-feb es el Step 1. Cero IA.
- **Días**:
  - **Lun 08-02 · journal 5' + audio (deload total)**: Journal 5' (sueño · modo · 1 frase de identidad). Nada más.
  - **Mar 09-02 · journal 5' + audio (deload total)**: Journal 5'.
  - **Mié 10-02 · journal 5' + audio (deload total)**: Journal 5' (D95 = mié 10-feb, último día del plan y D-1 real): cierre del journal del ciclo · nada de IA · dormir temprano · ritual de D-1 · jueves 11-feb = EXAMEN.
- **Sábado/domingo**: Sin PC: el sáb 13-feb ya es post-examen (descanso). La decisión de febrero decide qué vuelve a la franja 04:15.
