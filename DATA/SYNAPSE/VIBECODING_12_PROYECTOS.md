...
|---|---|---|---|---|---|
| S1 | Fix parser APEX (P0-2 / P0-3) + test multilínea | yocpmd | vie 11-sep → jue 17-sep | sáb 19-sep | — |
| S2 | Telemetría AnkiConnect en producción (due / backlog / retención) | yocpmd | vie 18-sep → jue 24-sep | sáb 26-sep | — |
| S3 | Captura diaria de scores USMLE → Supabase (pre-test / 30Q / eval + tipo de error) | preguntas | vie 25-sep → jue 01-oct | sáb 03-oct | — |
| S4 | gen_revision_semanal.js en producción (10 métricas con dato real) | yocpmd | vie 02-oct → jue 08-oct | sáb 10-oct | — |
| S5 | remap_todo.js: corrimiento de 1 comando + verificación automática | yocpmd | vie 09-oct → jue 15-oct | sáb 17-oct | — |
| S6 | RLS en datos_tesis y kappa_piloto (P0 de seguridad abierto desde el 2-jul) | yocpmd | vie 16-oct → jue 22-oct | sáb 24-oct | — |
| S7 | Puente VITALS → rendimiento (sueño y agua cruzados con % eval) | yocpmd | vie 23-oct → jue 29-oct | sáb 31-oct | — |
| S8 | Motor de preguntas ENCAPS semiautomático v0 (skill /pregunta-encaps) — semana DELOAD | preguntas | vie 30-oct → jue 05-nov | sáb 07-nov | **sí (50%)** |
| S9 | Migrador de overlays de hitos del Calendar (UWSA/NBME) tras cada corrimiento | yocpmd | vie 06-nov → jue 12-nov | sáb 14-nov | — |
| S10 | Bot WhatsApp → OCR → ficha LIVIANO (n8n Webhook + visión) | bots | vie 13-nov → jue 19-nov | sáb 21-nov | — |
| S11 | Pipeline de contenido de marcas (brief → 3 guiones + títulos + thumbnail) con la API de Claude | contenido | vie 20-nov → jue 26-nov | sáb 28-nov | — |
| S12 | Capstone: README del sistema YoCPMD + demo 5' + retro de 12 semanas | yocpmd | vie 27-nov → jue 03-dic | sáb 05-dic | — |

## S1 · Fix parser APEX (P0-2 / P0-3) + test multilínea

- **Semana**: vie 11-sep → jue 17-sep · **SHIP**: sábado sáb 19-sep (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 11-09 · doc + commit + preparar SHIP**: Reproducir el bug: pegar un APEX con REVERSO de 3 líneas por node scripts/node_parsear_tarjeta_v2_3.js y ver el truncado. Escribir scripts/test_parser_multilinea.js con 3 fixtures que HOY fallan (rojo). Objetivo del día: test rojo reproducible.
  - **Lun 14-09 · 5' objetivo → spec del día**: P0-2: cambiar el lookahead en node_parsear_tarjeta_v2_3.js:27 y n8n_parser_v2_3.js:56 a (?=\n[A-Z_]+:|\n═|(?![\s\S])) (sin '$' bajo flag m). Correr el test → verde.
  - **Mar 15-09 · construir con Claude Code**: P0-3: node_crear_nota_v2_3.js renderiza caso_clinico y fisio_expandida en la nota Obsidian (sección condicional si viene vacía). Test de la nota con fixture.
  - **Mié 16-09 · construir con Claude Code**: Verificar en vivo: pegar n8n_parser_v2_3.js en el nodo Code de n8n (redeploy), enviar 1 APEX de prueba (Ctrl+Shift+A, is_test) → abrir Anki y el vault y comprobar que llega íntegro.
  - **Jue 17-09 · verificar en vivo (test/URL/dato)**: Commit en agente_estudio + cerrar P0-2/P0-3 en DATA/ENCAPS/AUDITORIA_AGENTE_ESTUDIO_2026-07-02.md (fecha) + nota synapse-journal. Preparar el SHIP del sábado: lista de lo que falta (T1/T2/T3 Palmerton = P1-6, queda para otra semana).
...
- **Semana**: vie 18-sep → jue 24-sep · **SHIP**: sábado sáb 26-sep (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 18-09 · doc + commit + preparar SHIP**: Correr node DATA/_scripts/anki_telemetria.js con Anki abierto. Comparar due/backlog/revisadas con la pantalla Stats de Anki y anotar discrepancias (deck pattern APEX::USMLE* vs AnKing).
  - **Lun 21-09 · 5' objetivo → spec del día**: Retención real: usar cardReviews de los últimos 30 días (reviewType=1) y calcular 1 − Again/total; validar que getDeckConfig devuelve 10 nuevas/día y desired retention 0.9 (si no, corregir en Anki, no en el script).
  - **Mar 22-09 · construir con Claude Code**: Automatizar: tarea programada de Windows (schtasks /create … 21:05 L-D) o hook SessionEnd de Claude Code que ejecute el script; comprobar que escribe sin ventana abierta.
  - **Mié 23-09 · construir con Claude Code**: KPI en Home: pegar el one-liner localStorage que imprime el script y verificar en vivo que CockpitStatusBar lo muestra; opcional: servir el JSON desde public/ para no pegar a mano.
  - **Jue 24-09 · verificar en vivo (test/URL/dato)**: Regla 'Anki finde = due × 20 s' en DATA/SYNC_ANKI_OBSIDIAN_APP.md (ya escrita) + commit + nota synapse-journal.
...
- **Semana**: vie 25-sep → jue 01-oct · **SHIP**: sábado sáb 03-oct (PC SYNAPSE 15:00-17:00) · rotación `preguntas`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 25-09 · doc + commit + preparar SHIP**: Definir el esquema: tabla usmle_daily_scores(fecha date pk, pretest_10 int, q30_pct numeric, eval_pct numeric, error_dominante text, ccsn int, nota text) con RLS; escribir la migración en supabase-schema.sql (NO aplicar aún). Alternativa: study_sim_scores con examen='USMLE'.
  - **Lun 28-09 · 5' objetivo → spec del día**: UI: en UsmleTodayPlan añadir 3 inputs numéricos + selector knowledge/transfer/proceso + contador CCSN; guardar en localStorage 'jmd-usmle-scores' (try/catch) al instante.
  - **Mar 29-09 · construir con Claude Code**: Espejo: upsert en Supabase (anon + RLS por usuario) con reintento; lectura de los últimos 7 días para ReadinessBar (media móvil + distancia al mínimo on-track de la tabla Parte V).
  - **Mié 30-09 · construir con Claude Code**: Verificar en vivo (web): registrar los 3 valores de hoy, recargar, comprobar persistencia y la fila en Supabase (MCP execute_sql SELECT).
  - **Jue 01-10 · verificar en vivo (test/URL/dato)**: Conectar gen_revision_semanal.js (fuente usmle_daily_scores) + commit + nota synapse-journal.
...
- **Semana**: vie 02-oct → jue 08-oct · **SHIP**: sábado sáb 10-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 02-10 · doc + commit + preparar SHIP**: Correr node DATA/_scripts/gen_revision_semanal.js y listar cada métrica que sale 'sin dato'; elegir las 3 más baratas de conectar.
  - **Lun 05-10 · 5' objetivo → spec del día**: Conectar fuentes: scores USMLE (S3), telemetría Anki (S2), % ciego ENCAPS del viernes desde TRACKING_ERRORES/_registro_resoluciones.json (rondas de la semana).
  - **Mar 06-10 · construir con Claude Code**: Export de localStorage sin fricción: skill /export-ls (imprime el snippet de consola) o botón en la app que descarga jmd-*.json a DATA/USMLE/REVISIONES/_localstorage_export.json.
  - **Mié 07-10 · construir con Claude Code**: Generar la revisión de esta semana en seco, leerla completa, corregir la plantilla (DATA/REVISION_SEMANAL.md) donde no ayude a decidir.
  - **Jue 08-10 · verificar en vivo (test/URL/dato)**: Automatizar: viernes 21:00 el script pre-genera (schtasks o /schedule); sábado 07:15 Joseph rellena 20'. Commit + nota synapse-journal.
...
- **Semana**: vie 09-oct → jue 15-oct · **SHIP**: sábado sáb 17-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
- **A quién sirve**: TODOS los planes — ya van 8 corrimientos (31-ago→11-sep, régimen v5.9) y cada uno exige 5 scripts a mano + execute_sql + docs; un error de fecha rompe el Home de todos los frentes
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 09-10 · doc + commit + preparar SHIP**: Escribir remap_todo.js: encadena los 5 generadores con child_process, acepta --dry-run (trabaja sobre una copia temporal de src/lib) y resume fechas inicio→fin por plan.
  - **Lun 12-10 · 5' objetivo → spec del día**: verificar_planes.js: lee cada src/lib/*DailyPlan.ts y usmleStep1Daily.ts (regex fecha:"20\d\d-\d\d-\d\d"), falla si hay sáb/dom, 25-dic/31-dic/1-ene, duplicados o fechas no crecientes; imprime nº de días vs esperado (v5.9: USMLE 95 · MIR 78 · LIVIANO 90 · SYNAPSE 82 · VIBE 60 · AURUM 130 · Business 123 · Derma 70 · Research 42+67 · ENCAPS 98).
  - **Mar 13-10 · construir con Claude Code**: Skill /remap <fecha> (.claude/skills/remap/SKILL.md, disable-model-invocation: true): ejecuta el dry-run, muestra el diff y pide confirmación; hook PostToolUse opcional que corre verificar_planes.js al editar src/lib/*Plan.ts.
  - **Mié 14-10 · construir con Claude Code**: Prueba negativa: inyectar un sábado en una copia y ver que verificar_planes.js falla; prueba positiva con la fecha real.
  - **Jue 15-10 · verificar en vivo (test/URL/dato)**: Doc breve en DATA/_scripts (cabecera del script) + memoria de Claude (regla: usar remap_todo.js) + commit + nota synapse-journal.
...
- **Semana**: vie 16-oct → jue 22-oct · **SHIP**: sábado sáb 24-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 16-10 · doc + commit + preparar SHIP**: Inventario: quién lee/escribe datos_tesis y kappa_piloto (grep en agente_estudio + SECURITY_AUDIT_PHASE_A_C.sql ya existente); decidir política: solo service_role (pipeline) + rol médico.
  - **Lun 19-10 · 5' objetivo → spec del día**: Escribir la migración: alter table … enable row level security + policies; probar en una rama de Supabase (create_branch) o con --dry-run en local.
  - **Mar 20-10 · construir con Claude Code**: Mover el acceso del pipeline a SUPABASE_SERVICE_ROLE_KEY en .env (gitignored); comprobar que el pipeline sigue insertando.
  - **Mié 21-10 · construir con Claude Code**: scripts/test_rls_anon.js: con la anon key, SELECT → espera 0 filas/401; correr get_advisors y adjuntar salida.
  - **Jue 22-10 · verificar en vivo (test/URL/dato)**: Aplicar con Joseph (apply_migration, backup previo) + SECURITY_AUDIT.md + commit + nota synapse-journal.
...
- **Semana**: vie 23-oct → jue 29-oct · **SHIP**: sábado sáb 31-oct (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 23-10 · doc + commit + preparar SHIP**: Consultar mv_wellness_logs (tipo sueno/agua) con la anon key: si RLS bloquea, definir la policy de lectura para el usuario médico (is_medico) o usar el JSON de la revisión como puente.
  - **Lun 26-10 · 5' objetivo → spec del día**: gen_revision_semanal.js: tabla sueño×eval por fecha (join con usmle_daily_scores de S3) + contador de noches <7h y <6h.
  - **Mar 27-10 · construir con Claude Code**: CockpitStatusBar: 'Sueño anoche: X h' (lectura del último log o del JSON) con try/catch y '—' si no hay dato.
  - **Mié 28-10 · construir con Claude Code**: TodayMission: si el sueño de anoche <6h → chip 'sugerido: ÁMBAR' junto al selector de modo (PROTOCOLO_MODO_MINIMO); Joseph decide.
  - **Jue 29-10 · verificar en vivo (test/URL/dato)**: Doc: fuente de datos en PROTOCOLO_MODO_MINIMO.md (sección Fuentes) + commit + nota synapse-journal.
...
- **Semana**: vie 30-oct → jue 05-nov · **SHIP**: sábado sáb 07-nov (PC SYNAPSE 15:00-17:00) · rotación `preguntas`
...
- **Pasos diarios (45' cada uno; deload: mar/jue 15')**:
  - **Vie 30-10 · doc + commit + preparar SHIP**: (45') Escribir SKILL.md: lee PRONOSTICO v3 + rotación del día + PERFIL_CONOCIMIENTO → genera 5 viñetas ciegas, formato pregunta→solución interactiva, sin clave hasta que Joseph responda.
  - **Lun 02-11 · 5' objetivo → spec del día (15')**: (15' deload) Revisar el formato de salida con 1 viñeta; nada más.
  - **Mar 03-11 · construir con Claude Code**: (45') Registro: la skill apenda la ronda a _registro_resoluciones.json (esquema README) y vuelca RONDAS/<codigo>_<fecha>.json.
  - **Mié 04-11 · construir con Claude Code (15')**: (15' deload) Descanso activo: leer 1 ronda anterior y anotar 1 mejora.
  - **Jue 05-11 · verificar en vivo (test/URL/dato) (15')**: (15') 1 ronda real de 5Q en la hora ENCAPS (16:15) registrada + commit + nota synapse-journal.
...
- **Semana**: vie 06-nov → jue 12-nov · **SHIP**: sábado sáb 14-nov (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
- **A quién sirve**: USMLE — los 12 hitos (UWSA1-2 · NBME 25-33 · Free 120) son overlays naranjas del Calendar — 9 en viernes y 3 fuera (NBME 30 mié 30-dic-2026 · NBME 32 lun 18-ene-2027 · NBME 33 mié 20-ene-2027); cada corrimiento los desalinea del .ts y hoy se recrean a mano (recurrenceData del MCP está roto para UPDATE → delete + create)
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 06-11 · doc + commit + preparar SHIP**: Listar los hitos y su D# desde usmleStep1Daily.ts (DIAS con palm/hito) y la regla 'hito = viernes de su semana'; escribir la tabla esperada para START=2026-09-11 (v5.9) y comparar con CALENDARIO_5_MESES.md.
  - **Lun 09-11 · 5' objetivo → spec del día**: gen_hitos_overlays.js: lee el .ts, calcula fechas, emite JSON {hito, fecha, hora, descripcion, accion:'delete+create'}; sin llamadas al Calendar.
  - **Mar 10-11 · construir con Claude Code**: Descripción del overlay = protocolo test-day generado desde PALMERTON_POR_MATERIA §F (texto fijo + hora de cada break).
  - **Mié 11-11 · construir con Claude Code**: Diff en seco: con el MCP (list_events/search_events 'NBME'|'UWSA') comparar fechas y listar solo cambios; corregir falsos positivos.
  - **Jue 12-11 · verificar en vivo (test/URL/dato)**: Doc + commit + nota synapse-journal. Ejecutar delete/create solo tras confirmación de Joseph (y solo si hubo corrimiento).
...
- **Semana**: vie 13-nov → jue 19-nov · **SHIP**: sábado sáb 21-nov (PC SYNAPSE 15:00-17:00) · rotación `bots`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 13-11 · doc + commit + preparar SHIP**: Diseño: WhatsApp Cloud API → Webhook n8n → descarga de media → visión (prompt de extracción a JSON) → validación → Supabase → respuesta. Credenciales de WhatsApp: A VERIFICAR con Joseph (cuenta Meta Business).
  - **Lun 16-11 · 5' objetivo → spec del día**: n8n: Webhook node (POST, path propio) + verificación de firma + descarga de la imagen; probar con curl y una imagen local.
  - **Mar 17-11 · construir con Claude Code**: OCR/visión: prompt de extracción con esquema JSON fijo (campos de la ficha) + 2 ejemplos; validar con un nodo Code (campos obligatorios, unidades).
  - **Mié 18-11 · construir con Claude Code**: Guardar en Supabase (tabla liviano_fichas, RLS) + respuesta automática; 3 pruebas con imágenes reales y medir latencia.
  - **Jue 19-11 · verificar en vivo (test/URL/dato)**: Exportar el workflow (JSON) a DATA/BUSINESS/ + doc + commit + nota synapse-journal.
...
- **Semana**: vie 20-nov → jue 26-nov · **SHIP**: sábado sáb 28-nov (PC SYNAPSE 15:00-17:00) · rotación `contenido`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 20-11 · doc + commit + preparar SHIP**: Brief por marca (LIVIANO): 3 pilares + tono + 3 temas de la academia; escribir gen_contenido.js que arma el prompt y llama a la API (o claude -p --json-schema) con salida JSON.
  - **Lun 23-11 · 5' objetivo → spec del día**: Guiones: 3 guiones de 45 s (hook-cuerpo-CTA) + 5 títulos; guardar en DATA/BUSINESS/CONTENIDO/liviano/<fecha>/guiones.md.
  - **Mar 24-11 · construir con Claude Code**: Thumbnail: prompt de imagen + 1 imagen generada con la herramienta disponible; marcadores de corte para clips.
  - **Mié 25-11 · construir con Claude Code**: Revisión humana (claims clínicos con fuente) + publicar 1 pieza.
  - **Jue 26-11 · verificar en vivo (test/URL/dato)**: Anotar alcance a 48 h + commit + nota synapse-journal.
...
- **Semana**: vie 27-nov → jue 03-dic · **SHIP**: sábado sáb 05-dic (PC SYNAPSE 15:00-17:00) · rotación `yocpmd`
...
- **Pasos diarios (45' cada uno)**:
  - **Vie 27-11 · doc + commit + preparar SHIP**: README_SISTEMA.md: inventario de DATA/_scripts (generadores, remap, telemetría, revisión), hooks y skills; orden del pipeline de corrimiento.
  - **Lun 30-11 · 5' objetivo → spec del día**: Demo 5': grabar (OBS/Loom) Home → Estudio → revisión semanal; subir y enlazar.
  - **Mar 01-12 · construir con Claude Code**: Retro: tabla de los 12 proyectos con estado + 1 lección cada uno; qué se automatiza en F2.
  - **Mié 02-12 · construir con Claude Code**: Decisión CCA-F: revisar dominios (Agentic Architecture 27% · Claude Code 20% · Tool Design & MCP 18% · Prompt Engineering 20% · Context Management 15%) contra lo hecho; fijar fecha o no.
  - **Jue 03-12 · verificar en vivo (test/URL/dato)**: Commit final + tag vibecoding-s12 + nota synapse-journal (cierre).
