 ...
 * D1 = JUE 2026-09-17 → D95 = LUN 2027-02-01 (v5.13: 31-ago→16-sep no estudiados = 13 hábiles perdidos).
 * EXAMEN: target MAR 2-FEB-2027 (fuera de la ventana original 25-29 ene desde la v5.12). D94 vie 29-ene = última sesión
 * de banco; sáb 30 y dom 31 libres; D95 lun 1-feb = D-1 DENTRO del plan (sesión mínima AM + ritual). Joseph debe agendar/reprogramar el Prometric y
 * confirmar el eligibility period; desde aquí cada día no estudiado mueve el examen un hábil más (o exige recortar).
 * v5.10-v5.13: el UWSA1 es el único hito que cambia de fecha en cada corrimiento (vie 11 → lun 14 → mar 15 → mié 16 → jue 17-sep) para
 * seguir siendo el D1 (baseline el primer día, como prescribe Palmerton). Los otros 11 hitos conservan su fecha y solo
 * cambian de D#. El contenido arranca el vie 18-sep (D2). NADA se fusionó ni se recortó: el temario sale 1:1 y el
 ...
 *
 * FASES (v5.13, recalculadas sobre las fechas reales): A · Contenido por sistemas D1-D81 (17-sep→mar 12-ene, ~40Q
 ...
 *            corrimiento; el UWSA2 vie 8-ene = D79 cae DENTRO de la Fase A y el cierre de Bioquímica pasa al mar 12-ene = D81)
 *        B · Banco intensivo D82-D84 (13-ene→15-ene, random timed + sistema débil + NBME 31 GO/NO-GO)
 *        C · Sprint final D85-D95 (18-ene→1-feb, NBME 32/33 + Free 120 + taper; D86 19-ene, D88 21-ene, D90 25-ene, D91 26-ene, D92 27-ene y
 *            D93 28-ene siguen siendo días de banco — random timed, incorrects 2ª pasada + AMBOSS 200 mitades 1 y 2 — alojados dentro del sprint)
 * HITOS: UWSA1 jue 17-sep (baseline, D1) · NBME 25/26/27/28/29 en viernes cada ~3 semanas · NBME 30 mié 30-dic ·
 ...
 * v5.10-Palmerton (12-sep-2026, vigente en v5.13): cada día lleva nivelUW (1-5 = los 5 niveles de maestría UWorld) y qDia (Q objetivo);
 ...
 *  · VIERNES DE NIVEL 4 (#6): desde S11 los viernes sin hito de Fase A que serían nivel 3 pasan a nivel 4 (v5.13: solo D62 vie 11-dic;
 *    el vie 27-nov D52 es 2º día de Heme y queda en nivel 1)
 ...
 *  · TAPER (#22): D94 (vie 29-ene, D-2 = última sesión de banco) y D95 (lun 1-feb, D-1 dentro del plan) = 20Q flagged + Anki maduro, cero contenido nuevo (franjaNota);
 *    v5.13: sáb 30 y dom 31-ene libres entre D94 y D95 (solo Anki vencido); D95 = sesión mínima AM + ritual de test-day → USMLE_TAPER;
 *    examen target LUN 1-FEB-2027 (fuera de la ventana 25-29 ene).
...
export const DAILY_META = {
  step: 1, inicio: '2026-09-17', fin: '2027-02-01', totalDias: 95, // v5.13 (16-sep): Step 1 = bloque principal · D95 = lun 1-feb = D-1 dentro del plan · examen target mar 2-feb-2027
  ...
  /** Ventana original (25-29 ene), superada desde la v5.12; v5.13: el plan termina el lun 1-feb. */
  examenVentana: '2027-01-25 → 2027-01-29 (superada desde v5.12; v5.13: D95 = lun 1-feb)',
  /** Target de examen v5.13 (16-sep): mar 2-feb-2027, primer hábil tras el D95. Agendar/reprogramar Prometric y confirmar eligibility period (decisión de Joseph: o esto, o recortar). */
  examenTarget: '2027-02-02',
  /** D-1 = lun 1-feb = D95, DENTRO del plan (sesión mínima AM + ritual de test-day; USMLE_TAPER.dMenos1). Sáb 30 y dom 31 libres. */
  descansoD1: '2027-02-01',
...
/** Los 5 niveles de maestría UWorld (Palmerton, "UWorld Complete Guide: The Five Levels of Mastery to 260+") mapeados a las fases A/B/C del plan v5.13. */
export const USMLE_NIVELES: NivelUWInfo[] = [
  ...
  { nivel: 3, nombre: 'Sistema completo · timed', fase: 'A', formato: 'Bloques de 10-20Q de TODO el sistema · timed (sin la "ventaja injusta" de saber el subtema)', qDia: 'Palmerton 40-50Q/día → plan: 40Q (10 pre-test + 20Q sistema + 10 tutor)', umbral: '80% en 20Q timed consecutivas del sistema', dondeVive: 'VIERNES sin NBME/UWSA a las 11:00 hasta S10 (v5.13: D22 Renal · D32 GI · D47 Neuro; los viernes D7 Cardio, D17 Resp y D37 Endo caen en 1º-2º día de sistema y quedan en nivel 1): 20Q del sistema en curso, o del anterior si el sistema lleva <3 días · desde S11 el viernes pasa a nivel 4', color: '#6BB8B0' },
  { nivel: 4, nombre: 'Sistemas mixtos · timed', fase: 'A (dosis diaria + viernes desde S11) → B', formato: 'Bloques de 20-30Q mezclando ≥3 sistemas dominados + el nuevo (saltar entre especialidades bajo presión)', qDia: 'Palmerton 50-70Q/día → plan: viernes N4 = 40Q (10 pre-test + 30Q mixtos timed) · Fase B: 2×40Q (80Q)', umbral: '80% en bloques mixtos de 20Q timed de ≥3 sistemas', dondeVive: '18:00 EVAL (10Q mixta timed) toda la Fase A como dosis diaria · VIERNES sin hito desde S11 (v5.13: solo D62 vie 11-dic; el D52 vie 27-nov es 2º día de Heme → nivel 1) = 20-30Q mixtos timed a las 11:00 en vez de sistema único (DIAS[].franjaNota) · Fase B D82-D83 + D86 y D88 (random timed 2×40Q + sistema débil)', color: '#C8A96A' },
  { nivel: 5, nombre: 'Mixto completo 40Q · timed', fase: 'B → C (+ todos los hitos)', formato: 'Bloques de 40Q random · timed 60 min (90 s/Q) = simulación exacta del examen', qDia: 'Palmerton 80-100Q/día (máx. 2 bloques de 40) · hitos: UWSA 160Q · NBME 200Q · Free 120', umbral: '80% sostenido (90% para 260+) · pase seguro = NBME ≥65% (≈95%) / ≥70% (≈99%)', dondeVive: '05:00 STRESS SET 10Q/12min (Fases B-C) · NBME 31 D84 (GO/NO-GO) · D90 y D91 (incorrects 2ª pasada + sistema débil #3), D92 y D93 (AMBOSS 200 mitades 1-2, banco dentro de la semana del sprint) · Fase C (NBME 32/33 + Free 120 + taper D94-D95) · hitos = formato nivel 5 como MEDICIÓN, no como progresión', color: '#C56A5A' },
];
...
/** Fase del plan por número de día (v5.13): A contenido D1-81 (17-sep→12-ene; UWSA2 D79 dentro) · B banco D82-84 (13→15-ene) · C sprint D85-95 (18-ene→1-feb). */
export function faseDe(d: number): 'A' | 'B' | 'C' { return d <= 81 ? 'A' : d <= 84 ? 'B' : 'C'; }
...
/** Semana del plan (S1 = semana L-V del D1; v5.13: D1 = jue 17-sep-2026, S1 = 14-18 sep). Fuera de rango → 0. */
...
/**
 * TAPER y D-1 (Palmerton §8.3 · DIVERGENCIAS §E-5, implementada 12-sep-2026 · v5.13 16-sep: D94 vie 29-ene = última sesión de banco,
 * sáb 30 y dom 31 libres, D95 lun 1-feb = D-1 DENTRO del plan (sesión mínima + ritual); examen target MAR 2-FEB-2027).
...
export const USMLE_TAPER = {
  ...
  cierre: 'Desde NBME 31 (D84, vie 15-ene): cero preguntas nuevas y cero tarjetas nuevas (Palmerton: cesar 1-2 semanas antes); solo incorrects/flagged + AMBOSS 200 como repaso de conceptos ya vistos · no repetir NBME ya hechos.',
  d94: { d: 94, fecha: '2027-01-29', rol: 'D-2 · última sesión de banco (vie 29-ene)', resumen: 'Solo Anki MADURO + 20Q flagged/incorrects ya vistos · repaso First Aid de esquemas (sistemas 6-10) · sáb 30 y dom 31 libres (solo Anki vencido) · dormir ≥7 h' },
  d95: { d: 95, fecha: '2027-02-01', rol: 'D-1 · lun 1-feb (dentro del plan · v5.13)', resumen: 'Sesión MÍNIMA solo por la mañana (≤2 h): Anki maduro/vencido + 20Q flagged con los mejores esquemas e imágenes · rapid review FA · tarde: permiso impreso + digital, 2 ID con el nombre EXACTO del permiso, bolsas Ziploc numeradas, ruta al Prometric · nada después de las 17:00' },
  dMenos1: {
    fecha: '2027-02-01', rol: 'D-1 · lun 1-feb · = D95 (dentro del plan · v5.13; el finde 30-31 ene queda libre)',
    pasos: [
  ...
  examen: {
    fecha: '2027-02-02', rol: 'EXAMEN · mar 2-feb-2027 (target v5.13: fuera de la ventana 25-29 ene → agendar/reprogramar Prometric y confirmar eligibility period)',
    pasos: [
