 ...
 * D1 = MIÉ 2026-09-16 → D95 = VIE 2027-01-29 (v5.12: 31-ago→15-sep no estudiados = 12 hábiles perdidos).
 * EXAMEN: el plan ocupa ya el vie 29-ene (último día de la ventana 25-29 ene) → target LUN 1-FEB-2027, FUERA de la
 * ventana: sáb 30 = D-2 y dom 31-ene = D-1 fuera del plan (descanso). Joseph debe agendar/reprogramar el Prometric y
 * confirmar el eligibility period; desde aquí cada día no estudiado mueve el examen un hábil más (o exige recortar).
 * v5.10-v5.12: el UWSA1 es el único hito que cambia de fecha en cada corrimiento (vie 11 → lun 14 → mar 15 → mié 16-sep) para
 * seguir siendo el D1 (baseline el primer día, como prescribe Palmerton). Los otros 11 hitos conservan su fecha y solo
 * cambian de D#. El contenido arranca el jue 17-sep (D2). NADA se fusionó ni se recortó: el temario sale 1:1 y el
 ...
 *
 * FASES (v5.12, recalculadas sobre las fechas reales): A · Contenido por sistemas D1-D81 (16-sep→lun 11-ene, ~40Q
 ...
 *            corrimiento; el UWSA2 vie 8-ene = D80 cae DENTRO de la Fase A y el cierre de Bioquímica pasa al lun 11-ene = D81)
 *        B · Banco intensivo D82-D85 (12-ene→15-ene, random timed + sistema débil + NBME 31 GO/NO-GO)
 *        C · Sprint final D86-D95 (18-ene→29-ene, NBME 32/33 + Free 120 + taper; D87 19-ene, D89 21-ene, D91 25-ene, D92 26-ene y
 *            D93 27-ene siguen siendo días de banco — random timed, incorrects 2ª pasada + AMBOSS 200 mitades 1 y 2 — alojados dentro del sprint)
 * HITOS: UWSA1 mié 16-sep (baseline, D1) · NBME 25/26/27/28/29 en viernes cada ~3 semanas · NBME 30 mié 30-dic ·
 ...
 * v5.10-Palmerton (12-sep-2026, vigente en v5.12): cada día lleva nivelUW (1-5 = los 5 niveles de maestría UWorld) y qDia (Q objetivo);
 ...
 *  · VIERNES DE NIVEL 4 (#6): desde S11 (vie 27-nov, D53) los viernes sin hito de Fase A que serían nivel 3 pasan a nivel 4
 ...
 *  · TAPER (#22): D94 (jue 28-ene, D-4) y D95 (vie 29-ene, D-3 = última sesión) = 20Q flagged + Anki maduro, cero contenido nuevo (franjaNota);
 *    v5.12: sáb 30 (D-2) y dom 31-ene (D-1) quedan FUERA del plan (solo Anki vencido + ritual de test-day el domingo) → USMLE_TAPER;
...
export const DAILY_META = {
  step: 1, inicio: '2026-09-16', fin: '2027-01-29', totalDias: 95, // v5.12 (15-sep): Step 1 = bloque principal · el plan llena la ventana 25-29 ene → examen target lun 1-feb-2027
  ...
  /** Ventana original (25-29 ene). v5.12: el plan ya la ocupa entera (D95 = vie 29) → el target queda FUERA de ella. */
  examenVentana: '2027-01-25 → 2027-01-29 (superada en v5.12: D95 = vie 29-ene)',
  /** Target de examen v5.12 (15-sep): lun 1-feb-2027, primer hábil tras el D95. Reprogramar/agendar Prometric y confirmar eligibility period (decisión de Joseph: o esto, o recortar). */
  examenTarget: '2027-02-01',
  /** D-1 FUERA del plan (dom 31-ene; el sáb 30 = D-2 también libre de banco): solo Anki vencido + ritual de test-day (USMLE_TAPER.dMenos1). */
  descansoD1: '2027-01-31',
...
/** Los 5 niveles de maestría UWorld (Palmerton, "UWorld Complete Guide: The Five Levels of Mastery to 260+") mapeados a las fases A/B/C del plan v5.12. */
export const USMLE_NIVELES: NivelUWInfo[] = [
  ...
  { nivel: 3, nombre: 'Sistema completo · timed', fase: 'A', formato: 'Bloques de 10-20Q de TODO el sistema · timed (sin la "ventaja injusta" de saber el subtema)', qDia: 'Palmerton 40-50Q/día → plan: 40Q (10 pre-test + 20Q sistema + 10 tutor)', umbral: '80% en 20Q timed consecutivas del sistema', dondeVive: 'VIERNES sin NBME/UWSA a las 11:00 hasta S10 (v5.12: D8 Cardio · D33 GI · D48 Neuro; los viernes D18 Resp, D23 Renal y D38 Endo caen en 1º-2º día de sistema y quedan en nivel 1): 20Q del sistema en curso, o del anterior si el sistema lleva <3 días · desde S11 el viernes pasa a nivel 4', color: '#6BB8B0' },
  { nivel: 4, nombre: 'Sistemas mixtos · timed', fase: 'A (dosis diaria + viernes desde S11) → B', formato: 'Bloques de 20-30Q mezclando ≥3 sistemas dominados + el nuevo (saltar entre especialidades bajo presión)', qDia: 'Palmerton 50-70Q/día → plan: viernes N4 = 40Q (10 pre-test + 30Q mixtos timed) · Fase B: 2×40Q (80Q)', umbral: '80% en bloques mixtos de 20Q timed de ≥3 sistemas', dondeVive: '18:00 EVAL (10Q mixta timed) toda la Fase A como dosis diaria · VIERNES sin hito desde S11 (D53, vie 27-nov: ≥6 sistemas cerrados) = 20-30Q mixtos timed a las 11:00 en vez de sistema único (DIAS[].franjaNota) · Fase B D82-D84 + D87 (random timed 2×40Q + sistema débil)', color: '#C8A96A' },
  { nivel: 5, nombre: 'Mixto completo 40Q · timed', fase: 'B → C (+ todos los hitos)', formato: 'Bloques de 40Q random · timed 60 min (90 s/Q) = simulación exacta del examen', qDia: 'Palmerton 80-100Q/día (máx. 2 bloques de 40) · hitos: UWSA 160Q · NBME 200Q · Free 120', umbral: '80% sostenido (90% para 260+) · pase seguro = NBME ≥65% (≈95%) / ≥70% (≈99%)', dondeVive: '05:00 STRESS SET 10Q/12min (Fases B-C) · NBME 31 D85 (GO/NO-GO) · D89 y D91 (incorrects 2ª pasada + sistema débil #3), D92 y D93 (AMBOSS 200 mitades 1-2, banco dentro de la semana del sprint) · Fase C (NBME 32/33 + Free 120 + taper D94-D95) · hitos = formato nivel 5 como MEDICIÓN, no como progresión', color: '#C56A5A' },
];
...
/** Fase del plan por número de día (v5.12): A contenido D1-81 (16-sep→11-ene; UWSA2 D80 dentro) · B banco D82-85 (12→15-ene) · C sprint D86-95 (18→29-ene). */
export function faseDe(d: number): 'A' | 'B' | 'C' { return d <= 81 ? 'A' : d <= 85 ? 'B' : 'C'; }
...
/** Semana del plan (S1 = semana L-V del D1; v5.12: D1 = mié 16-sep-2026, S1 = 14-18 sep). Fuera de rango → 0. */
...
/**
 * TAPER y D-1 (Palmerton §8.3 · DIVERGENCIAS §E-5, implementada 12-sep-2026 · v5.12 15-sep: el plan termina el vie 29-ene = D95 =
 * última sesión (D-3); sáb 30 = D-2 y dom 31-ene = D-1 fuera del plan; examen target LUN 1-FEB-2027, ya fuera de la ventana 25-29).
...
export const USMLE_TAPER = {
  ...
  cierre: 'Desde NBME 31 (D85, vie 15-ene): cero preguntas nuevas y cero tarjetas nuevas (Palmerton: cesar 1-2 semanas antes); solo incorrects/flagged + AMBOSS 200 como repaso de conceptos ya vistos · no repetir NBME ya hechos.',
  d94: { d: 94, fecha: '2027-01-28', rol: 'D-4 · penúltima sesión', resumen: 'Solo Anki MADURO + 20Q flagged/incorrects ya vistos · repaso First Aid de esquemas (sistemas 6-10) · dormir ≥7 h' },
  d95: { d: 95, fecha: '2027-01-29', rol: 'D-3 · última sesión del plan (vie 29-ene)', resumen: 'Sesión ligera: Anki maduro + 20Q flagged con los mejores esquemas e imágenes · rapid review FA · tarde: permiso impreso + digital, 2 ID con el nombre EXACTO del permiso, bolsas Ziploc numeradas, ruta al Prometric · sáb 30 y dom 31 = descanso fuera del plan' },
  dMenos1: {
    fecha: '2027-01-31', rol: 'D-1 · dom 31-ene · FUERA DEL PLAN (descanso; el sáb 30 = D-2 también sin banco: solo Anki vencido)',
    pasos: [
  ...
  examen: {
    fecha: '2027-02-01', rol: 'EXAMEN · lun 1-feb-2027 (target v5.12: FUERA de la ventana 25-29 ene → agendar/reprogramar Prometric y confirmar eligibility period)',
    pasos: [
