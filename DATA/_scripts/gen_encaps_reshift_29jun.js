// gen_encaps_reshift_29jun.js — ÚLTIMA reprogramación: 26-28 jun (vie+finde) no se estudió → D1=29-jun.
...
// Salida: _encaps_reshift_29jun.sql. node DATA/_scripts/gen_encaps_reshift_29jun.js
...
(async () => {
  ...
  const L = ['-- ENCAPS reshift +1 → D1=29-jun · EXTENDIDO (no comprime · examen corre). Restaura 40 días de contenido.', 'BEGIN;'];
  L.push("DROP TABLE IF EXISTS study_schedule_reshift29_backup;");
  L.push("CREATE TABLE study_schedule_reshift29_backup AS SELECT * FROM study_schedule WHERE examen='ENCAPS';");
  ...
  L.push('COMMIT;');
  fs.writeFileSync(path.join(__dirname, '_encaps_reshift_29jun.sql'), L.join('\n'), 'utf8');
