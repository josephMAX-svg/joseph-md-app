// gen_encaps_remap_17jun.sql.js — ENCAPS v9-fecha (16-jun): arranca 17-jun, EXAMEN FIJO 20-ago.
// 45 temas L-V (17-jun→18-ago), 9 sábados de simulacros + 1 día-examen (19-ago) = 35 simulacros.
// Preserva el contenido de cada tema (videos/nts/material): UPDATE por codigo, solo dia/fecha.
...
if(topics.length!==45||sims.length!==35)throw new Error('counts '+topics.length+'/'+sims.length);

const START='2026-06-17', EXAM='2026-08-20';
...
if(ti!==45)throw new Error('temas='+ti);
if(si!==35)throw new Error('sims='+si);

const L=['-- ENCAPS 17-jun (examen FIJO 20-ago) · backup study_schedule_backup_v4_20260613','BEGIN;'];
...
L.push('COMMIT;');
fs.writeFileSync(path.join(__dirname,'_encaps_remap_17jun.sql'),L.join('\n'),'utf8');
const tr=rows.filter(x=>x.role==='topic');
console.log(`OK total=${dia} temas=${ti}(${tr[0].fecha}…${tr[tr.length-1].fecha}) sims=${si} examen=${ex.fecha}(${ex.wd}) diasExamen=${[...DIAS_EXAMEN]}`);
console.log('D1=17-jun · TOTAL encapsPlan.ts =', dia);
