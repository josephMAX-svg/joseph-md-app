// gen_encaps_remap_24jun.sql.js — ENCAPS (20-23 jun no se estudió → arranca 24-jun, miércoles).
// EXAMEN FIJO 20-ago. 24-jun→20-ago = 50 días hábiles (1 menos que 23-jun) → para conservar
// los 45 temas + 35 simulacros + examen, se añade una 5ª fusión afín: IV-1+IV-2
// (Tipos de estudio ➕ Validez de estudios — mismo núcleo metodológico: diseño y validez/sesgos).
// → 40 días-tema. día1 = mié 24-jun (1er tema; sin simulacro diagnóstico de sábado-día1).
// Fusiones idempotentes (las 4 previas son no-op; sólo IV-1+IV-2 es nueva). Merges preservan
...
// 45 temas → 40 entradas: III-4+III-7, IV-6+IV-7, V-7+V-10, IV-3+IV-5 (previas) y AHORA IV-1+IV-2 (nueva).
const topics=['II-3','II-1','II-2','I-4','II-4','II-6','V-1','II-11','I-1','I-2','I-3','II-5','II-7','II-8','II-9','II-10','III-1','III-3','III-9','V-2','V-6','IV-1+IV-2','I-5+I-6','I-7','I-8','I-9','I-11+I-12','II-12','II-13','III-2','III-5','III-6+III-10','III-8','IV-3+IV-5','IV-4','IV-6+IV-7','V-3','V-7+V-10','I-10','III-4+III-7'];
const sims=[];
...
if(topics.length!==40||sims.length!==35)throw new Error('counts '+topics.length+'/'+sims.length);

const START='2026-06-24', EXAM='2026-08-20';
...
if(ti!==40)throw new Error('temas='+ti);
if(si!==35)throw new Error('sims='+si);

const L=['-- ENCAPS 24-jun (examen FIJO 20-ago) · backup study_schedule_backup_24jun','BEGIN;'];
// 1) Fusiones afines (idempotentes). Las 4 previas son no-op (sus fuentes ya no existen). IV-1+IV-2 NUEVA.
...
// 1b) NUEVA 5ª fusión IV-1+IV-2 (preserva videos+theomed+material_comp+nts)
L.push("UPDATE study_schedule t SET codigo='IV-1+IV-2', subtema = COALESCE(t.subtema,'') || ' ➕ ' || COALESCE(s.subtema,''), videos = COALESCE(t.videos,'[]'::jsonb) || COALESCE(s.videos,'[]'::jsonb), theomed = COALESCE(t.theomed,'[]'::jsonb) || COALESCE(s.theomed,'[]'::jsonb), material_comp = COALESCE(t.material_comp,'[]'::jsonb) || COALESCE(s.material_comp,'[]'::jsonb), nts = NULLIF(TRIM(BOTH ' +' FROM COALESCE(t.nts,'') || ' + ' || COALESCE(s.nts,'')),''), updated_at=now() FROM (SELECT subtema, videos, theomed, material_comp, nts FROM study_schedule WHERE examen='ENCAPS' AND codigo='IV-2') s WHERE t.examen='ENCAPS' AND t.codigo='IV-1';");
L.push("DELETE FROM study_schedule WHERE examen='ENCAPS' AND codigo='IV-2';");
...
L.push('COMMIT;');
fs.writeFileSync(path.join(__dirname,'_encaps_remap_24jun.sql'),L.join('\n'),'utf8');
...
console.log('D1=24-jun · TOTAL encapsPlan.ts =', dia);
