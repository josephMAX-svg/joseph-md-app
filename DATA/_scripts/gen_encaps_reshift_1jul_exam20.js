// gen_encaps_reshift_1jul_exam20.js — D1=1-jul · EXAMEN FIJO 20-ago (corrección: NO 27-ago).
...
// Sanity: NUNCA descarta un tema (deep_prime). node DATA/_scripts/gen_encaps_reshift_1jul_exam20.js
...
const START = '2026-07-01', EXAM = '2026-08-20', DX = '2026-08-19';
...
(async () => {
  ...
  const L = ['-- ENCAPS D1=1-jul · EXAMEN FIJO 20-ago · extiende estudio / reduce repaso. backup study_schedule_examen20_backup', 'BEGIN;'];
  L.push("DROP TABLE IF EXISTS study_schedule_examen20_backup;");
  L.push("CREATE TABLE study_schedule_examen20_backup AS SELECT * FROM study_schedule WHERE examen='ENCAPS';");
  ...
  L.push('COMMIT;');
  fs.writeFileSync(path.join(__dirname, '_encaps_reshift_1jul_exam20.sql'), L.join('\n'), 'utf8');
