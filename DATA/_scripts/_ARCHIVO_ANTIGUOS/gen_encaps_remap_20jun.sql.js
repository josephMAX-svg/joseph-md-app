// gen_encaps_remap_20jun.sql.js — ENCAPS (problema el 19 → arranca 20-jun, sábado).
// EXAMEN FIJO 20-ago. 20-jun→20-ago = 53 días hábiles (1 menos que 19-jun) → para conservar
// los 45 temas + 35 simulacros + examen, se FUSIONAN 3 pares afines: III-4+III-7, IV-6+IV-7 (ya estaban)
// y AHORA V-7+V-10 (NTS Referencia ➕ SIS/AUS — ambos Gestión/normativa-detalle, MEDIA). 42 días-tema.
// día1 = sáb 20-jun = simulacro (diagnóstico); los temas empiezan lun 22-jun. Merges idempotentes.
...
// 45 temas → 42 entradas: III-4+III-7, IV-6+IV-7 y V-7+V-10 fusionados (afines, mismo bloque)
...
if(topics.length!==42||sims.length!==35)throw new Error('counts '+topics.length+'/'+sims.length);

const START='2026-06-20', EXAM='2026-08-20';
const DIAS_EXAMEN=new Set(['2026-08-19']);
const satCounts=[2,3,3,3,3,3,3,3,3]; // 9 sábados = 26 (el 1º es día1=20-jun)
...
if(ti!==42)throw new Error('temas='+ti);
if(si!==35)throw new Error('sims='+si);

const L=['-- ENCAPS 20-jun (examen FIJO 20-ago) · backup study_schedule_backup_v4_20260613','BEGIN;'];
// 1) Fusiones afines (idempotentes). III-4+III-7 e IV-6+IV-7 ya aplicadas (no-op). V-7+V-10 NUEVA (20-jun).
...
L.push('COMMIT;');
fs.writeFileSync(path.join(__dirname,'_encaps_remap_20jun.sql'),L.join('\n'),'utf8');
const tr=rows.filter(x=>x.role==='topic');
console.log(`OK total=${dia} temas=${ti}(${tr[0].fecha}…${tr[tr.length-1].fecha}) sims=${si} examen=${ex.fecha}(${ex.wd}) dia1=${rows[0].fecha}(${rows[0].role})`);
console.log('D1=20-jun · TOTAL encapsPlan.ts =', dia);
