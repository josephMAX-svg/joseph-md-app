/**
 * remap_inicio_26jun.js — 25-jun tampoco se estudió → todo arranca MAÑANA 26-jun.
 ...
 * Uso: node DATA/_scripts/remap_inicio_26jun.js
 */
...
// 1) USMLE daily (72)
{const f='src/lib/usmleStep1Daily.ts';if(countFechas(f,'export const DIAS').length!==72)throw new Error('USMLE!=72');const nd=calNoSun('2026-06-26',72);replaceFechas(f,'export const DIAS',nd);setMeta(f,'export const DAILY_META','2026-06-26',nd[71]);console.log('USMLE ✓ '+nd[0]+'→'+nd[71]);}
// 2) MIR (78)
{const f='src/lib/mirDailyPlan.ts';if(countFechas(f,'export const MIR_DIAS').length!==78)throw new Error('MIR!=78');const nd=calNoSun('2026-06-26',78);replaceFechas(f,'export const MIR_DIAS',nd);setMeta(f,'export const MIR_DAILY_META','2026-06-26',nd[77]);console.log('MIR ✓ '+nd[0]+'→'+nd[77]);}
// 3) USMLE plan UNIDADES (5)
{const f='src/lib/usmleStep1Plan.ts';if(countFechas(f,'export const UNIDADES').length!==5)throw new Error('UNID!=5');replaceFechas(f,'export const UNIDADES',calNoSun('2026-06-26',5));setMeta(f,'export const PLAN_META','2026-06-26',null);console.log('USMLE UNIDADES ✓');}
// 4) Research (42) — slots research (alterna con Derma, ancla 10-jun → 24-jun = research)
{const f='src/lib/researchDailyPlan.ts';if(countFechas(f,'export const DIAS').length!==42)throw new Error('RES!=42');const nd=slots('research','2026-06-26',42);replaceFechas(f,'export const DIAS',nd);setMeta(f,'export const DAILY_META',nd[0],nd[41]);console.log('Research ✓ '+nd[0]+'→'+nd[41]);}
// 5) Derma (70) — slots derma (alterna con Research, ancla 10-jun → 25-jun = derma)
{const f='src/lib/dermaDailyPlan.ts';if(countFechas(f,'DERMA_DIAS: DiaDerma[] = [').length!==70)throw new Error('DERMA!=70');const nd=slots('derma','2026-06-26',70);replaceFechas(f,'DERMA_DIAS: DiaDerma[] = [',nd);setMeta(f,'export const DERMA_DAILY_META',nd[0],nd[69]);console.log('Derma ✓ '+nd[0]+'→'+nd[69]);}
// 6) Business — reconstruido desde 24-jun
{const f='src/lib/businessStudyPlan.ts';const p=path.join(ROOT,f);let s=fs.readFileSync(p,'utf8');const marker='export const BIZ_DIAS: DiaBiz[] = [';const i=s.indexOf(marker);const bs=i+marker.length;const j=s.indexOf('];',bs);const parts=s.slice(bs,j).split('},{').map((pp,idx,arr)=>{let e=pp;if(idx>0)e='{'+e;if(idx<arr.length-1)e=e+'}';return e.trim();});const work=parts.filter(e=>!/materia:"DESCANSO"/.test(e));if(work.length!==84)throw new Error('BIZ work='+work.length);const out=[];let cur='2026-06-26',wi=0,d=1;while(wi<work.length){if(isSun(cur))out.push(`{d:${d},fecha:"${cur}",wd:"Dom",materia:"DESCANSO",lectura:"DÍA LIBRE TOTAL (domingo). Sin actividad — descanso real; el sueño/descanso consolida lo estudiado (Walker). Las notas se re-leen el lunes en 10 min.",accion:"",min:0,libroN:null,yt:null}`);else out.push(work[wi++].replace(/^\{d:\d+,fecha:"[^"]*",wd:"[^"]*",/,`{d:${d},fecha:"${cur}",wd:"${wdOf(cur)}",`));d++;cur=addDays(cur,1);}const total=out.length,fin=out[out.length-1].match(/fecha:"([^"]*)"/)[1];s=s.slice(0,bs)+out.join(',')+s.slice(j);s=s.replace(/inicio: '2026-06-\d\d', fin: '[^']*', totalDias: \d+, \/\/ v\d[^\n]*/,`inicio: '2026-06-26', fin: '${fin}', totalDias: ${total}, // v9 (25-jun): reconstruido vie 26-jun (25-jun tampoco se estudió) · TODOS los domingos LIBRES · 84 trabajo + descansos dominicales`);fs.writeFileSync(p,s,'utf8');console.log('Business ✓ 24-jun→'+fin+' total='+total);}

console.log('\nOK — remap 26-jun (5 planes). ENCAPS, Synapse y AURUM aparte.');
