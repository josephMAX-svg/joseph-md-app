// gen_encaps_reshift_26jun.js — ÚLTIMA reprogramación: 25-jun tampoco se estudió → D1=26-jun.
...
// Salida: _encaps_reshift_26jun.sql. node DATA/_scripts/gen_encaps_reshift_26jun.js
...
(async () => {
  ...
  const L = ['-- ENCAPS reshift +1 → D1=26-jun · EXTENDIDO (no comprime · examen corre). Restaura 40 días de contenido.', 'BEGIN;'];
  L.push("DROP TABLE IF EXISTS study_schedule_reshift26_backup;");
  L.push("CREATE TABLE study_schedule_reshift26_backup AS SELECT * FROM study_schedule WHERE examen='ENCAPS';");
  ...
  L.push('COMMIT;');
  fs.writeFileSync(path.join(__dirname, '_encaps_reshift_26jun.sql'), L.join('\n'), 'utf8');
