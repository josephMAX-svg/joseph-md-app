# 📊 SISTEMA DE TRACKING DE RESPUESTAS — multi-examen (ENCAPS · USMLE · MIR · DERMA) · v3.1 (12-sep-2026 · comandos re-fechados el 26-sep-2026 al régimen v5.16)
...
Aplicar: `node DATA/_scripts/gen_encaps_mantenimiento_2027.js 2026-09-28 --override DATA/ENCAPS/TRACKING_ERRORES/SEMANAS/override_<lunes>.json` (D1 vigente = lun 28-sep-2026, régimen v5.16 / `MANTENIMIENTO_2027-1 v6.14`; la siembra base del 26-sep fue `… 2026-09-28 --bk study_schedule_bk_0926` y cada re-siembra toma un backup fechado nuevo) → revisar el SQL → `execute_sql` → pegar la verificación que imprime el script (`select modo, tipo, count(*) …` → 75 banqueo1h + 17 mini_sim). El checkpoint de fin de enero (v3 §6 regla 3) sale de esta misma serie: no se reconstruye a mano.
...
3. **Backup fechado y no pisable:** `study_schedule_bk_<YYYYMMDD>` por defecto; el SQL aborta (`RAISE EXCEPTION`) si esa tabla ya existe. Segunda corrida el mismo día → `--bk study_schedule_bk_<YYYYMMDD>b`; `--bk-reemplazar` solo si de verdad se quiere sobrescribir. Los backups viejos (`bk_0703 … bk_0922`) no se borran desde los scripts.
