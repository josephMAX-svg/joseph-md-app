# 🇵🇪 PROTOCOLO DE LA HORA ENCAPS — MANTENIMIENTO 2027-I (16:15–17:15 L-V · D1 lun 14-sep-2026 → vie 29-ene-2027 · 97 días)
...
> 97 días · backup `study_schedule_bk_0912`; los `bk_0910` (v5.9), `bk_0909` (v5.8) y `bk_0908` (v5.7) siguen intactos). Siembra: `DATA/_scripts/gen_encaps_mantenimiento_2027.js`
> (configuración compartida en `_encaps_ciclo_v3.js`). Versión **v3.6 (12-sep-2026, régimen v5.10)**: (a) §1 alineado con la regla REAL de la eval anclada (fallos ≥2 → código CALIENTE para el override del viernes; nada se desplaza esa semana; lunes = fallos del mini-sim) y con el JSON `horarios` de `study_metrics.extra` (`_migrations/study_metrics_horarios.sql`, aplicado el 12-sep); (b) §1-bis **pre-test de arranque** (lun 14 + mar 15-sep, 40Q reales, línea base ciega por crítico); (c) §5 siembra segura (`DELETE … AND modo='MANTENIMIENTO'`, backup fechado que aborta si existe, `SEMANAS/overrides_acumulado.json`, `PERFIL_CONOCIMIENTO.md` generado). Mantiene de v3.5: re-fechado a D1 = lun 14-sep (el 11 de septiembre tampoco se estudió; el ciclo pasó de 98 a 97 días · **18 mini-sims de viernes**: se cae solo el del vie 11-sep, ya pasado · metas, franjas y fecha de examen intactas). El fin del ciclo NO se mueve (29-ene-2027): el mantenimiento es banqueo puro y se acorta por delante, no por detrás — no hay temario que "dejar atrás". Mantiene de v3.1-v3.4: sub-ejes por sesión,
> secundario de cola larga, receta fija del mini-sim, cierre de 1 línea y **lista negra del 2026-II**.
>
> **Efecto exacto del corrimiento a v5.10** (demostrado fila a fila contra el SQL v5.9, no estimado): las **79 filas de banqueo son IDÉNTICAS** — mismo código, sub-eje, secundario **y misma fecha** (la rotación lun-jue ya arrancaba el lun 14-sep en v5.9); solo baja 1 su `dia`. Los **18 mini_sim** conservan su viernes y cambian de `dia`, pero **su cola larga rota una semana entera**: al desaparecer el mini-sim #1 del vie 11-sep (II-2 + I-10), la pareja de cada viernes pasa al viernes siguiente (18-sep = II-2 + I-10, antes V-6 + II-6 … 29-ene = II-2 + I-10, antes V-6 + II-6). **II-8 sigue en 4** (`extra.de = 4`, heredado de v5.9). Es el primer corrimiento que no mueve ninguna sesión de banqueo: el único día que el plan pierde es un viernes ya pasado.
...
### 1-bis) Semana 1 = PRE-TEST DE ARRANQUE (lun 14 y mar 15-sep-2026): línea base ciega por crítico

| Día | 16:15 | 16:30–17:10 | 17:10 |
|---|---|---|---|
| **Lun 14 (D1)** | sin eval | **`BANCO_PROPIO/pretest_arranque_2026-09-14.html` — parte 1: 20Q = 5Q × II-3 · I-3 · V-2 · III-5**, modo examen (72 s/Q = 24 min, sin material, solución al final), confianza 1-3 obligatoria | export JSON → `node DATA/_scripts/gen_encaps_minisim.js --registrar <export.json> --append` |
| **Mar 15** | eval anclada II-3 (`eval_2026-09-15.html`, como estaba) | **`pretest_arranque_2026-09-15.html` — parte 2: 20Q = 5Q × II-5 · I-4 · IV-1+IV-2 · II-4** | ídem |
| Mié 16 → vie 18 | eval I-3 · eval V-2 | banco V-2 · banco III-5 (sin cambio) | mini-sim #5 el viernes (sin cambio) |

- Qué es: **40Q ciegas = 5Q × 8 críticos v3**, tomadas de los **ítems reales con clave oficial 2024-2A · 2025-1A · 2025-2** (36/40; II-3 lleva 4 reales + 1 nuevo porque solo existen 5 reales y uno está en la eval del martes; III-5 lleva 2 reales + 3 nuevos porque 13 reales ya están en el banco del jueves 17). **Nunca el 2026-II** (lista negra intacta). Generado con `node DATA/_scripts/gen_encaps_minisim.js --pretest-arranque` (detalle de la selección en `_meta.seleccion` de cada JSON).
- **Sustituye el CONTENIDO, no el horario**: `banco_2026-09-14` (II-3 · esquema_intervalos) y `banco_2026-09-15` (I-3 · tipos_vigilancia) quedan marcados `_meta.sustituido_por` (su HTML muestra el aviso y no arranca); sus ítems no se resuelven y vuelven al pool. Las filas de `study_schedule` no cambian (la app sigue mostrando II-3 e I-3 con su sub-eje: el pre-test cubre esos dos códigos ese mismo día).
...
| Cola larga | **5-6Q** repartidas entre los **2 códigos** que marca el viernes (`extra.cola_larga`) | 18 viernes (36 slots) → II-2 e I-10 caen 3×, los otros 15 códigos 2× (contado sobre el SQL v5.10) |
...
### 3.1 Ciclo base (se repite **5 veces = 79 slots lun-jue**; el contador NO se reinicia en feriados)
...
I-3 y V-2 caen **2× por ciclo** (10 sesiones cada uno); el resto **5 cada uno**, salvo **II-8 con 4** (`extra.de = 4`): con 79 slots el 5.º ciclo se queda a un jueves del final, y el slot que se cae es precisamente el último II-8. Contado sobre `_encaps_mantenimiento_2027.sql`, no estimado. ↩ = ALTA con flag de rebote. IV tiene 2 slots fijos por ciclo (lección 2026-II: 4→12).

### 3.2 Sub-eje por sesión (rotación circular por INSTANCIA del código; la app lo muestra como «◈ Sub-eje de hoy n/total»)

| Código | Sub-ejes en orden (la sesión k del código toma el sub-eje ((k−1) mod total)+1) | Reparto REAL en los 97 días (contado del SQL; idéntico al de v5.9) |
|---|---|---|
...
| **II-8 ↩ (4)** | paquetes ENT + tamizaje (HEARTS) · metas HTA/DM (HbA1c, 150 min/sem) | **2·2** — es el único código con 4 sesiones (bajó de 5 a 4 en v5.9; v5.10 no toca ninguna fila de banqueo) |
...
En 97 días cada código cae **6-8 veces** entre lun-jue (79 slots) y viernes (36 slots), contado del SQL v5.10: II-2 · I-10 caen 8× · V-6 · II-6 · II-10 · I-5+I-6 · II-EMG · I-OCC · III-3 · I-11+I-12 · V-1 caen 7× · V-3 · III-1 · III-9 · II-9 · II-7 · I-1 caen 6× (V-6 y II-6 pierden una aparición respecto a v5.9: eran las del mini-sim del 29-ene, que ahora cierra con II-2 + I-10). II-EMG e I-OCC existen en `encapsCobertura.ts` desde el re-tier v3 (tier MEDIA, temario mínimo con marcas "A VERIFICAR").
...
|---|---|---|---|---|---|
| 1 (14-sep) | **D1** · II-3 · esquema_intervalos (+II-2) → **contenido = PRE-TEST parte 1** (§1-bis) | I-3 · tipos_vigilancia (+I-10) → **contenido = PRE-TEST parte 2** (§1-bis) | V-2 · planeamiento (+V-6) | III-5 · pertinencia_barreras (+II-6) | SIM #5 · II-2 + I-10 |
| 2 (21-sep) | II-5 · nts_adolescente (+II-10) | I-4 · dengue (+I-5+I-6) | IV-1 · metodo_cientifico (+II-EMG) | II-4 · suplementacion_cifras (+I-OCC) | SIM #10 · V-6 + II-6 |
| 3 (28-sep) | II-1 · prenatal_emergencias (+III-3) | I-3 · notificacion (+I-11+I-12) | V-2 · clima_calidad (+V-1) | III-8 · ley_27815 (+V-3) | SIM #15 · II-10 + I-5+I-6 |
| 4 (05-oct) | II-11 · prep_dual_ptmi (+III-1) | IV-6 · imryd (+III-9) | V-7 · farmacovigilancia_urm (+II-9) | II-8 · paquetes_tamizaje (+II-7) | SIM #20 · II-EMG + I-OCC |
| 5 (12-oct) | II-3 · novedades_gestante (+I-1) | I-3 · historia_natural (+II-2) | V-2 · residuos_bioseguridad (+I-10) | III-5 · medicina_tradicional (+V-6) | SIM #25 · III-3 + I-11+I-12 |
| 6 (19-oct) | II-5 · mci_curso_vida (+II-6) | I-4 · rabia (+II-10) | IV-1 · tipos_estudio (+I-5+I-6) | II-4 · anemia_dx_tto (+II-EMG) | SIM #30 · V-1 + V-3 |
| 7 (26-oct) | II-1 · parto_lactancia (+I-OCC) | I-3 · causalidad_hill (+III-3) | V-2 · planeamiento (+I-11+I-12) | III-8 · historia_clinica (+V-1) | SIM #35 · III-1 + III-9 |
| 8 (02-nov) | II-11 · sifilis_sindromico (+V-3) | IV-6 · etica_publicacion (+III-1) | V-7 · esenciales_sismed (+III-9) | II-8 · metas_hta_dm (+II-9) | SIM #40 · II-9 + II-7 |
| 9 (09-nov) | II-3 · esavi_kit (+II-7) | I-3 · sala_situacional_asis (+I-1) | V-2 · clima_calidad (+II-2) | III-5 · inclusion_migrantes (+I-10) | SIM #45 · I-1 + II-2 |
| 10 (16-nov) | II-5 · nts_adolescente (+V-6) | I-4 · epp_precauciones (+II-6) | IV-1 · muestreo (+II-10) | II-4 · plan_multisectorial (+I-5+I-6) | SIM #50 · I-10 + V-6 |
| 11 (23-nov) | II-1 · prenatal_emergencias (+II-EMG) | I-3 · brote_bloqueo (+I-OCC) | V-2 · residuos_bioseguridad (+III-3) | III-8 · ley_27815 (+I-11+I-12) | SIM #55 · II-6 + II-10 |
| 12 (30-nov) | II-11 · prep_dual_ptmi (+V-1) | IV-6 · estadistica_descriptiva (+V-3) | V-7 · contrataciones_digemid (+III-1) | II-8 · paquetes_tamizaje (+III-9) | SIM #60 · I-5+I-6 + II-EMG |
| 13 (07-dic) | II-3 · cadena_frio (+II-9) | I-3 · tlp (+II-7) | V-2 · planeamiento (+I-1) | III-5 · pertinencia_barreras (+II-2) | SIM #65 · I-OCC + III-3 |
| 14 (14-dic) | II-5 · mci_curso_vida (+I-10) | I-4 · malaria_zoonosis (+V-6) | IV-1 · instrumentos (+II-6) | II-4 · suplementacion_cifras (+II-10) | SIM #70 · I-11+I-12 + V-1 |
| 15 (21-dic) | II-1 · parto_lactancia (+I-5+I-6) | I-3 · desastres (+II-EMG) | V-2 · clima_calidad (+I-OCC) | III-8 · historia_clinica (+III-3) | — (25-dic feriado) |
| 16 (28-dic) | II-11 · sifilis_sindromico (+I-11+I-12) | IV-6 · imryd (+V-1) | V-7 · farmacovigilancia_urm (+V-3) | — (31-dic) | — (1-ene) |
| 17 (04-ene) | II-8 · metas_hta_dm (+III-1) | II-3 · esquema_intervalos (+III-9) | I-3 · tipos_vigilancia (+II-9) | V-2 · residuos_bioseguridad (+II-7) | SIM #82 · V-3 + III-1 |
| 18 (11-ene) | III-5 · medicina_tradicional (+I-1) | II-5 · nts_adolescente (+II-2) | I-4 · dengue (+I-10) | IV-1 · metodo_cientifico (+V-6) | SIM #87 · III-9 + II-9 |
| 19 (18-ene) | II-4 · anemia_dx_tto (+II-6) | II-1 · prenatal_emergencias (+II-10) | I-3 · notificacion (+I-5+I-6) | V-2 · planeamiento (+II-EMG) | SIM #92 · II-7 + I-1 |
| 20 (25-ene) | III-8 · ley_27815 (+I-OCC) | II-11 · prep_dual_ptmi (+III-3) | IV-6 · etica_publicacion (+I-11+I-12) | V-7 · esenciales_sismed (+V-1) | **D97** · SIM #97 · II-2 + I-10 |

⚠ **Lo que cambió respecto a v5.9:** el mini-sim #1 del vie 11-sep (ya pasado) desaparece, así que la semana 1 = Sem A completa y arranca directamente con banqueo (el D1 es un lunes, II-3). Las 79 sesiones lun-jue **no cambian ni de fecha ni de contenido** (solo baja 1 su D#: la semana 18 sigue abriendo con el II-8 del lun 4-ene). La rotación de cola larga de los viernes vuelve a empezar en el primer viernes vivo (18-sep = II-2 + I-10), por lo que cada viernes hereda la pareja que en v5.9 tenía el viernes anterior; el plan sigue cerrando el vie 29-ene, ahora con el mini-sim #97 (II-2 + I-10).

Esta tabla se ha releído del SQL sembrado el 12-sep (`DATA/_scripts/_encaps_mantenimiento_2027.sql`, D1 = lun 14-sep-2026, régimen v5.10, 97 filas) parseando fila a fila con `node` — no es una estimación. Un **override semanal** (§5) sustituye solo la semana indicada (máx. 2 slots) y queda anotado en `extra.override`. Cada día sin estudiar = +1 hábil: `node DATA/_scripts/gen_encaps_mantenimiento_2027.js <nuevo D1>` → revisar el SQL → `execute_sql` (la tabla se desplaza entera por delante; los sub-ejes y secundarios siguen su contador y el 29-ene no se mueve). **Siembra segura desde el 12-sep:** el backup se llama `study_schedule_bk_<YYYYMMDD>` por defecto y el SQL **aborta si ya existe** (mismo día → `--bk study_schedule_bk_<YYYYMMDD>b`); el `DELETE` va filtrado por `modo = 'MANTENIMIENTO'` (las filas INTENSIVO de febrero no se tocan); el generador **lee siempre `TRACKING_ERRORES/SEMANAS/overrides_acumulado.json`** (todas las semanas ya re-ponderadas) y el `--override` de la semana nueva se AÑADE encima, nunca sustituye a las anteriores; al final imprime la verificación (`select modo, tipo, count(*) …` → 79 banqueo1h + 18 mini_sim) para pegarla tras `execute_sql`.
...
- **Cierre de sesión (1 línea, obligatorio, 17:10)** — se apenda a `TRACKING_ERRORES/_registro_resoluciones.json` (esquema v3 multi-examen, `README_SISTEMA_TRACKING.md`):
  ...
  node DATA/_scripts/gen_encaps_semana.js --cerrar "ENCAPS|banco_dia|2026-09-14|II-3|n=22|seg=15|dud=3|CONCEPTO:1,OLVIDO:2,CCSN:1|t=68|sub=esquema_intervalos" --sql
