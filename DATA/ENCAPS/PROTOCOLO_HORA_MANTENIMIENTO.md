# 🇵🇪 PROTOCOLO DE LA HORA ENCAPS — MANTENIMIENTO 2027-I (16:15–17:15 L-V · D1 mié 23-sep-2026 → mar 2-feb-2027 · 92 días)
...
> 92 días · régimen `MANTENIMIENTO_2027-1 v6.13`, backup `study_schedule_bk_0922`; los `bk_0919` (v5.14), `bk_0916` (v5.13), `bk_0915` (v5.12), `bk_0914` (v5.11), `bk_0912` (v5.10), `bk_0910` (v5.9), `bk_0909` (v5.8) y `bk_0908` (v5.7) siguen intactos). Siembra: `DATA/_scripts/gen_encaps_mantenimiento_2027.js`
> (configuración compartida en `_encaps_ciclo_v3.js`). Versión **v3.11 (22-sep-2026, régimen v5.15)**: re-fechado a **D1 = mié 23-sep-2026** (tampoco se estudiaron el lun 21 ni el mar 22-sep; decimocuarto corrimiento 31-ago→23-sep, 17 hábiles perdidos). **CORRIMIENTO RÍGIDO** (regla nueva de Joseph): solo corren los días — ningún tema se toca, no se fusiona ni se recorta nada, y donde había un fin clavado se **amplían días**. Por eso el ciclo **mantiene sus 92 días** (75 `banqueo1h` + 17 `mini_sim`, **0 sesiones perdidas**) y su cierre se corre del vie 29-ene al **mar 2-feb-2027**. El pre-test de arranque (§1-bis) pasa a **mié 23 + jue 24-sep**, y la rotación lun-jue corre dos hábiles entera (el D1 sigue siendo II-3 · esquema_intervalos, ahora en miércoles); **los 17 viernes de mini-sim NO se mueven** (el primero sigue siendo el vie 25-sep y el último el vie 29-ene) y sus parejas de cola larga se quedan donde estaban. ⚠ Consecuencia del corrimiento en el Step 1: su D95 es el **vie 5-feb = D-1 dentro del plan**, el D94 jue 4-feb la última sesión de banco, el finde 6-7 feb queda libre y el **examen Step 1 target pasa al lun 8-feb-2027**; los días 93-96 de la cuenta ENCAPS (mié 3 → lun 8-feb) son el cierre del Step 1 y su examen → la fase intensiva ENCAPS arranca **después** del examen: propuesta **mar 9-feb (día 97)** o **lun 15-feb (día 101)** (`FASE_INTENSIVA_2027-I.md`; decisión de Joseph). Mantiene de v3.10 (19-sep, v5.14), de v3.9 (16-sep, v5.13), de v3.8 (15-sep, v5.12), v3.7 (14-sep, v5.11) y de **v3.6 (12-sep-2026, régimen v5.10)**: (a) §1 alineado con la regla REAL de la eval anclada (fallos ≥2 → código CALIENTE para el override del viernes; nada se desplaza esa semana; lunes = fallos del mini-sim) y con el JSON `horarios` de `study_metrics.extra` (`_migrations/study_metrics_horarios.sql`, aplicado el 12-sep); (b) §1-bis **pre-test de arranque** (40Q, hoy **39 reales + 1 nuevo**, línea base ciega por crítico; en v3.6 caía lun 14 + mar 15, en v3.7 mar 15 + mié 16, en v3.8 mié 16 + jue 17, en v3.9 jue 17 + lun 21, en v3.10 lun 21 + mar 22, ahora **mié 23 + jue 24**); (c) §5 siembra segura (`DELETE … AND modo='MANTENIMIENTO'`, backup fechado que aborta si existe, `SEMANAS/overrides_acumulado.json`, `PERFIL_CONOCIMIENTO.md` generado). Mantiene de v3.5 la receta de los mini-sims de viernes (siguen **17**, sin moverse en v5.15: el primero es el vie 25-sep y el último el vie 29-ene; las parejas de cola larga no cambian, §2) · metas, franjas y fecha de examen intactas. ⚠ **Regla invertida en v5.15:** el fin del ciclo **SÍ se mueve** (del vie 29-ene al **mar 2-feb-2027**). Con el corrimiento RÍGIDO el mantenimiento ya no se acorta por delante: conserva sus 92 sesiones y se amplía por detrás, así que **no se pierde ninguna sesión** (hasta v5.14 cada corrimiento se comía el último slot de la rotación). Mantiene de v3.1-v3.4: sub-ejes por sesión,
> secundario de cola larga, receta fija del mini-sim, cierre de 1 línea y **lista negra del 2026-II**.
>
> **Efecto exacto del corrimiento a v5.15** (contado fila a fila sobre el SQL sembrado el 22-sep con `node`, no estimado): esta vez el patrón es el más limpio de los catorce. (1) **Los 17 mini_sim no se mueven ni cambian de pareja**: siguen en sus viernes (25-sep … 29-ene) con las mismas colas largas (II-2 + I-10 abre, II-7 + I-1 cierra) y con 34 slots los 17 códigos caen exactamente 2×; solo baja su número de día (`dia` 3, 8, 13 … 90 en vez de 5, 10, 15 … 92) porque el D1 se corrió dos hábiles. No hizo falta `--realinear`. (2) Las **75 sesiones lun-jue conservan código, sub-eje y secundario en el mismo ORDEN y corren dos días hábiles** (el D1 = II-3 · esquema_intervalos pasa del lun 21 al **mié 23-sep**; I-3 · tipos_vigilancia del mar 22 al **jue 24**; V-2 · planeamiento al lun 28; III-5 · pertinencia_barreras al mar 29, y así hasta el final). (3) **No se cae ningún slot**: como el cierre se amplía del vie 29-ene al **mar 2-feb**, la rotación entra entera (la última sesión lun-jue sigue siendo V-2 · planeamiento, ahora el **mar 2-feb**) y el reparto por código es idéntico al de v5.14 — I-3 10 · V-2 10 · I-4/II-1/II-3/II-4/II-5/III-5/IV-1+IV-2 5 · II-8/II-11/III-8/IV-6+IV-7/V-7+V-10 4 — y la demanda sigue en **1 786Q**. Las cinco instancias perdidas en corrimientos anteriores (II-8 v5.9 · V-7+V-10 v5.11 · IV-6+IV-7 v5.12 · II-11 v5.13 · III-8 v5.14) **no se recuperan solas**: siguen con `extra.de = 4` (A VERIFICAR con Joseph si se reponen en la intensiva). Las sesiones que desaparecen del calendario son las del lun 21 y el mar 22-sep.
>
...
### 1-bis) Semana 1 = PRE-TEST DE ARRANQUE (mié 23 y jue 24-sep-2026): línea base ciega por crítico

| Día | 16:15 | 16:30–17:10 | 17:10 |
|---|---|---|---|
| **Mié 23 (D1)** | `eval_2026-09-23` (D1 del régimen, sin sesión anterior: 5Q de fallos previos, críticos primero) | **`BANCO_PROPIO/pretest_arranque_2026-09-23.html` — parte 1: 20Q = 5Q × II-3 · V-2 · III-5 · I-3**, modo examen (72 s/Q = 24 min, sin material, solución al final), confianza 1-3 obligatoria | export JSON → `node DATA/_scripts/gen_encaps_minisim.js --registrar <export.json> --append` |
| **Jue 24 (D2)** | eval anclada (`eval_2026-09-24.html`: 5Q de II-3, 3 cifras + 2 viñetas) | **`pretest_arranque_2026-09-24.html` — parte 2: 20Q = 5Q × II-4 · IV-1+IV-2 · I-4 · II-5** (con D1 en miércoles tampoco lo parte ningún viernes ni fin de semana) | ídem |
| **Vie 25 (D3)** | — (el mini-sim ocupa las 16:15) | primera mini-sim del ciclo (`minisim_2026-09-25`, `dia` 3, II-2 + I-10; **no se movió** en este corrimiento, ítems intactos) | cierre semanal + pre-generar la semana del 28-sep (`--semana 2026-09-28`; abre el lun 28 con V-2 · planeamiento (+V-6) y sigue el mar 29 con III-5 · pertinencia_barreras (+II-6)) |

- Qué es: **40Q ciegas = 5Q × 8 críticos v3**, tomadas de los **ítems reales con clave oficial 2024-2A · 2025-1A · 2025-2** — en v5.15 son **39/40 reales + 1 nuevo** (`_meta.conteos` de `pretest_arranque_2026-09-23.json` = 19 reales y `…_2026-09-24.json` = 20; en v5.14 eran 40/40, en v5.12 39 + 1 nuevo, en v5.11 40/40). **Nunca el 2026-II** (lista negra intacta). Generado el 22-sep con `node DATA/_scripts/gen_encaps_minisim.js --pretest-arranque` (detalle de la selección en `_meta.seleccion` y `_meta.conteos` de cada JSON).
- **Sustituye el CONTENIDO, no el horario**: los bancos del mié 23 (D1 · II-3 · esquema_intervalos) y del jue 24 (D2 · I-3 · tipos_vigilancia) existen pero quedan marcados `_meta.sustituido_por` (sus ítems no se consumen y vuelven al pool): el contenido de esos dos días es el pre-test (`pretest_arranque_2026-09-21.*`, `pretest_arranque_2026-09-22.*` y `eval_2026-09-22.*` de v5.14 ya no existen). Las filas de `study_schedule` no cambian (la app sigue mostrando II-3 e I-3 con su sub-eje: el pre-test cubre esos códigos ese mismo día).
- Para qué: **`resumen_por_subtema` nace con n = 5 en los 8 críticos** antes del primer cierre semanal (vie 25-sep): el **override de la semana del 28-sep ya se calcula con línea base real en los 8** (en v5.13 el pre-test quedaba partido por el viernes y solo 4 críticos la tenían; con D1 en miércoles el pre-test vuelve a caber en dos días seguidos). La ronda entra como tipo `pretest` con `preguntas[]` y `gen_encaps_semana.js` la reparte por código (por eso `--append`, no la línea de 1 renglón).
- Umbral orientativo del pre-test: **≥70 % ciego** en un crítico = ya en crucero; **<40 %** = ese código pide set nuevo esa misma semana (`set_<codigo>_2.json`). Los fallos del pre-test alimentan la eval del jue 24 y los ≥5Q de fallos previos de la mini-sim del vie 25-sep. ⚠ **Stock (inventario del 22-sep):** al quedar los bancos del 23 y 24-sep marcados `sustituido_por`, sus ítems vuelven al pool y III-5 conserva **20 disponibles** (16 de `pertinencia_barreras`), así que la sesión III-5 del **mar 29-sep** está cubierta; el déficit global de III-5 sigue siendo alto (`_inventario_banco_por_codigo.json`, `_meta.totales.deficit_codigos`) → reponer `set_III-5_2.json` antes de `medicina_tradicional` (mar 27-oct).
...
| Cola larga | **5-6Q** repartidas entre los **2 códigos** que marca el viernes (`extra.cola_larga`) | 17 viernes (34 slots) → los 17 códigos caen exactamente 2× (contado sobre el SQL v5.15; **los viernes no se movieron en este corrimiento**: II-2 + I-10 sigue abriendo el 25-sep y II-7 + I-1 cerrando el 29-ene, igual que en v5.14) |
...
### 3.1 Ciclo base (se repite **5 veces = 75 slots lun-jue** en v5.15; el contador NO se reinicia en feriados)
...
I-3 y V-2 caen **2× por ciclo** (10 sesiones cada uno); el resto **5 cada uno**, salvo **II-8, V-7+V-10, IV-6+IV-7, II-11 y III-8 con 4** (`extra.de = 4`): con 75 slots el 5.º ciclo se queda a cinco slots del final, y los que se caen son los cuatro de la Sem D más el jueves de la Sem C — el III-8 (perdido en v5.14), el II-11 (v5.13), el IV-6+IV-7 (v5.12), el V-7 (v5.11) y el II-8 (v5.9) — **en v5.15 no se cae ninguno más**, porque el cierre se amplió al mar 2-feb; el último slot lun-jue (**mar 2-feb**) es V-2 · planeamiento. Contado sobre `_encaps_mantenimiento_2027.sql`, no estimado. ↩ = ALTA con flag de rebote. IV tiene 2 slots fijos por ciclo (lección 2026-II: 4→12).

### 3.2 Sub-eje por sesión (rotación circular por INSTANCIA del código; la app lo muestra como «◈ Sub-eje de hoy n/total»)

| Código | Sub-ejes en orden (la sesión k del código toma el sub-eje ((k−1) mod total)+1) | Reparto REAL en los 92 días (contado del SQL v5.15; **idéntico al de v5.14**: el corrimiento rígido no cambió ningún reparto) |
|---|---|---|
...
En 92 días cada código cae **6-7 veces** entre lun-jue (75 slots) y viernes (34 slots), contado del SQL v5.15 (cifras **idénticas a las de v5.14**): II-2 · I-10 · V-6 · II-6 · II-10 · I-5+I-6 · II-EMG caen 7× · I-OCC · III-3 · I-11+I-12 · V-1 · V-3 · III-1 · III-9 · II-9 · II-7 · I-1 caen 6× (II-2 e I-10 habían perdido su aparición al desaparecer el viernes 18-sep en v5.14; I-OCC la del secundario de la sesión III-8 que se cayó entonces; III-3 la había perdido en v5.13, I-11+I-12 en v5.12 y V-1 en v5.11). II-EMG e I-OCC existen en `encapsCobertura.ts` desde el re-tier v3 (tier MEDIA, temario mínimo con marcas "A VERIFICAR").
...
|---|---|---|---|---|---|
| 1 (21-sep) | — (no estudiado) | — (no estudiado) | **D1** · II-3 · esquema_intervalos (+II-2) → **contenido = PRE-TEST parte 1** (§1-bis) | **D2** · I-3 · tipos_vigilancia (+I-10) → **contenido = PRE-TEST parte 2** (§1-bis) | SIM #3 · II-2 + I-10 |
| 2 (28-sep) | V-2 · planeamiento (+V-6) | III-5 · pertinencia_barreras (+II-6) | II-5 · nts_adolescente (+II-10) | I-4 · dengue (+I-5+I-6) | SIM #8 · V-6 + II-6 |
| 3 (05-oct) | IV-1 · metodo_cientifico (+II-EMG) | II-4 · suplementacion_cifras (+I-OCC) | II-1 · prenatal_emergencias (+III-3) | I-3 · notificacion (+I-11+I-12) | SIM #13 · II-10 + I-5+I-6 |
| 4 (12-oct) | V-2 · clima_calidad (+V-1) | III-8 · ley_27815 (+V-3) | II-11 · prep_dual_ptmi (+III-1) | IV-6 · imryd (+III-9) | SIM #18 · II-EMG + I-OCC |
| 5 (19-oct) | V-7 · farmacovigilancia_urm (+II-9) | II-8 · paquetes_tamizaje (+II-7) | II-3 · novedades_gestante (+I-1) | I-3 · historia_natural (+II-2) | SIM #23 · III-3 + I-11+I-12 |
| 6 (26-oct) | V-2 · residuos_bioseguridad (+I-10) | III-5 · medicina_tradicional (+V-6) | II-5 · mci_curso_vida (+II-6) | I-4 · rabia (+II-10) | SIM #28 · V-1 + V-3 |
| 7 (02-nov) | IV-1 · tipos_estudio (+I-5+I-6) | II-4 · anemia_dx_tto (+II-EMG) | II-1 · parto_lactancia (+I-OCC) | I-3 · causalidad_hill (+III-3) | SIM #33 · III-1 + III-9 |
| 8 (09-nov) | V-2 · planeamiento (+I-11+I-12) | III-8 · historia_clinica (+V-1) | II-11 · sifilis_sindromico (+V-3) | IV-6 · etica_publicacion (+III-1) | SIM #38 · II-9 + II-7 |
| 9 (16-nov) | V-7 · esenciales_sismed (+III-9) | II-8 · metas_hta_dm (+II-9) | II-3 · esavi_kit (+II-7) | I-3 · sala_situacional_asis (+I-1) | SIM #43 · I-1 + II-2 |
| 10 (23-nov) | V-2 · clima_calidad (+II-2) | III-5 · inclusion_migrantes (+I-10) | II-5 · nts_adolescente (+V-6) | I-4 · epp_precauciones (+II-6) | SIM #48 · I-10 + V-6 |
| 11 (30-nov) | IV-1 · muestreo (+II-10) | II-4 · plan_multisectorial (+I-5+I-6) | II-1 · prenatal_emergencias (+II-EMG) | I-3 · brote_bloqueo (+I-OCC) | SIM #53 · II-6 + II-10 |
| 12 (07-dic) | V-2 · residuos_bioseguridad (+III-3) | III-8 · ley_27815 (+I-11+I-12) | II-11 · prep_dual_ptmi (+V-1) | IV-6 · estadistica_descriptiva (+V-3) | SIM #58 · I-5+I-6 + II-EMG |
| 13 (14-dic) | V-7 · contrataciones_digemid (+III-1) | II-8 · paquetes_tamizaje (+III-9) | II-3 · cadena_frio (+II-9) | I-3 · tlp (+II-7) | SIM #63 · I-OCC + III-3 |
| 14 (21-dic) | V-2 · planeamiento (+I-1) | III-5 · pertinencia_barreras (+II-2) | II-5 · mci_curso_vida (+I-10) | I-4 · malaria_zoonosis (+V-6) | — (25-dic feriado) |
| 15 (28-dic) | IV-1 · instrumentos (+II-6) | II-4 · suplementacion_cifras (+II-10) | II-1 · parto_lactancia (+I-5+I-6) | — (31-dic) | — (1-ene) |
| 16 (04-ene) | I-3 · desastres (+II-EMG) | V-2 · clima_calidad (+I-OCC) | III-8 · historia_clinica (+III-3) | II-11 · sifilis_sindromico (+I-11+I-12) | SIM #75 · I-11+I-12 + V-1 |
| 17 (11-ene) | IV-6 · imryd (+V-1) | V-7 · farmacovigilancia_urm (+V-3) | II-8 · metas_hta_dm (+III-1) | II-3 · esquema_intervalos (+III-9) | SIM #80 · V-3 + III-1 |
| 18 (18-ene) | I-3 · tipos_vigilancia (+II-9) | V-2 · residuos_bioseguridad (+II-7) | III-5 · medicina_tradicional (+I-1) | II-5 · nts_adolescente (+II-2) | SIM #85 · III-9 + II-9 |
| 19 (25-ene) | I-4 · dengue (+I-10) | IV-1 · metodo_cientifico (+V-6) | II-4 · anemia_dx_tto (+II-6) | II-1 · prenatal_emergencias (+II-10) | SIM #90 · II-7 + I-1 |
| 20 (01-feb) | I-3 · notificacion (+I-5+I-6) | **D92** · V-2 · planeamiento (+II-EMG) | — | — | — |

⚠ **Lo que cambió respecto a v5.14 (corrimiento RÍGIDO):** el lun 21 y el mar 22-sep dejan de tener sesión y el D1 pasa al **mié 23-sep**, así que la semana 1 arranca a media semana (mié-jue de pre-test + el viernes de mini-sim) y las semanas de la tabla ya **no** coinciden con las del Step 1. Los **17 mini-sims no se mueven ni cambian de pareja** (II-2 + I-10 abre el 25-sep, ahora con `dia` 3; el 29-ene cierra con II-7 + I-1, `dia` 90; cada código de cola larga sigue cayendo 2× en viernes). Las **75 sesiones lun-jue conservan código, sub-eje y secundario en el mismo orden y corren dos hábiles**, y como el cierre se amplía del vie 29-ene al **mar 2-feb** (semana 20 de la tabla) **no se cae ningún slot**: el reparto por código es idéntico al de v5.14 y la demanda sigue en 1 786Q.

Esta tabla se ha releído del SQL sembrado el 22-sep (`DATA/_scripts/_encaps_mantenimiento_2027.sql`, D1 = mié 23-sep-2026, régimen v5.15 / `MANTENIMIENTO_2027-1 v6.13`, 92 filas = 75 `banqueo1h` + 17 `mini_sim`) parseando fila a fila con `node` — no es una estimación. Un **override semanal** (§5) sustituye solo la semana indicada (máx. 2 slots) y queda anotado en `extra.override`. Cada día sin estudiar = +1 hábil: `node DATA/_scripts/gen_encaps_mantenimiento_2027.js <nuevo D1>` → revisar el SQL → `execute_sql` (**v5.15, corrimiento RÍGIDO:** la tabla entera se desplaza y el CIERRE se amplía con ella — ya no se recorta por delante, así que no se pierde ninguna sesión; los sub-ejes y secundarios siguen su contador). **Siembra segura desde el 12-sep:** el backup se llama `study_schedule_bk_<YYYYMMDD>` por defecto y el SQL **aborta si ya existe** (mismo día → `--bk study_schedule_bk_<YYYYMMDD>b`); el `DELETE` va filtrado por `modo = 'MANTENIMIENTO'` (las filas INTENSIVO de febrero no se tocan); el generador **lee siempre `TRACKING_ERRORES/SEMANAS/overrides_acumulado.json`** (todas las semanas ya re-ponderadas) y el `--override` de la semana nueva se AÑADE encima, nunca sustituye a las anteriores; al final imprime la verificación (`select modo, tipo, count(*) …` → 75 banqueo1h + 17 mini_sim) para pegarla tras `execute_sql`.
...
> Queda **RESERVADO ÍNTEGRAMENTE como pre-test diagnóstico** del arranque de la fase intensiva (v5.15: el primer viernes DESPUÉS del examen Step 1 del lun 8-feb-2027 — **vie 12-feb-2027**, o vie 19-feb si Joseph arranca la intensiva el lun 15; se genera el jue 4-feb. `PRETEST_2026-II.md`).
...
- **Convergencia app ↔ registro (v5.14 · 19-sep, vigente en v5.15)**: el CIERRE DE SESIÓN de la app escribe `study_progress` (fuente `app:cierre`, `errores_por_tipo` trae la ronda v3 completa). El viernes, ANTES del cierre semanal, se trae todo al registro con
  ```
