# 📅 SEGMENTOS DEL CALENDAR — RÉGIMEN v5.12 (corrimiento aplicado en vivo 15-sep-2026)
> **📌 Nota v5.12 (mar 15-sep-2026).** Este documento **ya refleja el corrimiento a D1 = miércoles 16-sep-2026** (el martes 15 de septiembre tampoco se estudió; es el **11.º corrimiento** desde el 31-ago: **12 días hábiles perdidos**, 31-ago → 15-sep). El nombre del fichero conserva `V5_6` **solo por los enlaces existentes**: lo citan como autoridad varias descripciones de series del Calendar y otros docs del repo, y renombrarlo los rompería. El contenido es v5.12 en todas sus secciones vivas (§0, §0.1, §5, §5.1, §6, §14); §7-§13 son registros históricos y conservan sus cifras de entonces. Última pasada sobre el Calendar en vivo: **15-sep-2026, 28 eventos tocados: 1 evento movido de fecha (el UWSA1) + 11 overlays USMLE (solo `description`) + 1 overlay NUEVO (🎯 EXAMEN lun 1-feb-2027, `oinh139dsnbuma9r3kfu56dhkc`) + 23 series (solo `description`)** (§14). Los 12 overlays 🔬 RESEARCH **no cambian** en v5.12 (el mié 16-sep ya era el D1 de Research; `gen_research_calendar.js --check` = 0 según el orquestador).
> ⚠ **La regla permanente de Joseph: NO se fusiona ni se recorta contenido — ni un tema ni un subtema se deja atrás.** El desfase se absorbe **alargando el final del plan**, nunca comprimiendo días. **CONSECUENCIA NUEVA de v5.12: el plan USMLE de 95 días ocupa ahora TODA la ventana de examen (D95 = vie 29-ene-2027 = última sesión, D-3) → el examen SALE de la ventana 25-29 ene: TARGET LUN 1-FEB-2027** (`DAILY_META.examenTarget = '2027-02-01'`, `examenVentana` marcada como superada, `descansoD1 = '2027-01-31'`). El **sáb 30 (D-2) y el dom 31-ene (D-1) quedan FUERA del plan** como descanso pre-examen (solo Anki vencido + ritual de test-day el domingo; `USMLE_TAPER.dMenos1`). **Joseph debe agendar/reprogramar el Prometric y confirmar que su eligibility period cubre el 1-feb** (si no: extenderlo, o decidir recortar temario — decisión suya, registrada en `DATA/PENDIENTES_JOSEPH.md`). **Desde aquí cada día no estudiado mueve el examen un día hábil más (o exige recortar).** Los 2 "días dobles" de Bioquímica que ya existían en v5.7 se conservan tal cual (D79 jue 7-ene y D81 lun 11-ene; el UWSA2 del vie 8-ene = D80 cae entre ambos, DENTRO de la Fase A).
> ⚡ **El UWSA1 volvió a cambiar de fecha (es el único hito que lo hace en cada corrimiento: vie 11 → lun 14 → mar 15 → mié 16-sep).** Como el D1 debe ser la línea base (Palmerton), **el evento suelto `o1gla7846uae4tgngvc4q45osg` se movió del mar 15 al mié 16-sep 09:00-13:00 (misma hora, misma zona `America/Lima`) y sigue siendo el D1**. Los otros 11 overlays USMLE conservan su fecha y solo cambian de D# (todos −1). El contenido (Fundamentos · Pathoma 1-2) arranca el **jue 17-sep = D2**.
> Fuente: Google Calendar `josephsototocas@gmail.com` · zona `America/Lima` · estructura L-V/sáb/dom extraída el 05-sep (166 eventos), reverificada en vivo el 08-sep, reescrita el 09-sep (28 descripciones), el 10-sep (24), el 12-sep (28 + UWSA1 movido), el 14-sep (40) y **reescrita el 15-sep con `get_event` + `update_event` (la respuesta del `update_event` devuelve el recurso completo y se comparó campo a campo) en los 27 eventos tocados + 1 `create_event`** (12 overlays USMLE + 23 series + el overlay del examen). En las 23 series se cambió **solo `description`**; en el UWSA1 `start`/`end` + `description`. Horas del resto, títulos, RRULE, `colorId` y `transparency` verificados intactos. Se verificó además con `list_events` que el **mié 16-sep** tiene los 13 bloques de estudio + el overlay UWSA1 (29 eventos ese día: el miércoles solo hay BAILE 19:00-20:30, sin GYM), que el **mar 15-sep ya no tiene ningún overlay** (29 eventos, todos series: el martes ALISTARSE/VIAJE van a las 18:30/18:45 y hay BAILE+GYM), que el **vie 29-ene-2027 = D95** sigue teniendo las 7 series con `UNTIL` (28 eventos, es su último día) y que el **lun 1-feb-2027** tiene el overlay del examen + el overlay 🔬 CR-9 y **ya no tiene ninguna de las 7 series USMLE/ENCAPS** (23 eventos; siguen apareciendo las series secundarias: vibecoding, SYNAPSE, Research↔Derma, AURUM, MIR ×2, LIVIANO, GYM+BAILE — ver §14.3).
> **Autoridad de CUÁNDO y CÓMO** se ejecuta cada segmento (L-V + sábado/domingo). El **QUÉ** (tema/misión del día) lo mandan la app YoCPMD y sus planes: `src/lib/usmleStep1Daily.ts` (**95 días**, D1 = 2026-09-16 → D95 = 2027-01-29; examen target 2027-02-01), Supabase `study_schedule` (ENCAPS **95 días**, `encapsPlan.ts`), `mirDailyPlan.ts` + `mirMantenimiento.ts`, `researchDailyPlan.ts`/`researchDailyPlan2027.ts`/`dermaDailyPlan.ts`/`dermaCiclo2.ts`, `aurumDailyPlan.ts`, `livianoStudyPlan.ts`, `synapseDailyPlan.ts`, `vibecodingPlan.ts`. Los overlays 🔬 RESEARCH los gobierna `DATA/_scripts/gen_research_calendar.js` (estado en `DATA/RESEARCH/_calendar_overlays.json`).
...
## 0 · Reglas del régimen v5.12 (las que el Calendar materializa)
| Regla | Valor |
|---|---|
| **D1** | **miércoles 16-sep-2026** (31-ago→15-sep no estudiados = **12 hábiles perdidos, 11 corrimientos acumulados**. v5.11 tenía D1 = mar 15-sep: el 15 de septiembre no se estudió y el plan corrió +1 día hábil) |
| **D1 = hito** | El D1 **es el UWSA1** (baseline, 09:00-13:00), **movido del mar 15 al mié 16-sep** — el único hito que cambia de fecha en cada corrimiento. El contenido arranca el **jue 17-sep = D2** (Pathoma 1-2). Los otros 11 hitos siguen **anclados por fecha** desde v5.8 |
| **Fases USMLE** | **A · contenido D1-D81** (16-sep→**lun 11-ene**; el cierre de Bioquímica = D81) · **B · banco D82-D85** (mar 12-ene→vie 15-ene, **4 días**: 3 de banco + NBME 31) · **C · sprint D86-D95** (lun 18-ene→**vie 29-ene**). **El UWSA2 (vie 8-ene = D80) cae DENTRO de la Fase A**, entre los 2 días dobles de Bioquímica (D79 jue 7-ene · D81 lun 11-ene). **No se fusionó ni se recortó ningún día de contenido**: el multiconjunto de temas es idéntico al de v5.11 y el desfase se absorbió al final. **D87 (mar 19-ene)** = random timed 2×40Q + sistema débil #2 (Mehlman); **D89 (jue 21-ene) y D91 (lun 25-ene)** = uWorld incorrects 2.ª pasada + sistema débil #3; **D92 (mar 26-ene) y D93 (mié 27-ene)** = uWorld incorrects + AMBOSS 200 mitades 1 y 2 (días de banco alojados dentro del sprint); **D94 (jue 28-ene) = D-4** repaso FA sistemas 6-10 + Anki marathon (taper: solo Anki maduro + 20Q flagged) · **D95 (vie 29-ene) = D-3 = ÚLTIMA SESIÓN** rapid review FA (sesión ligera + tarde de logística: permiso, 2 ID, Ziploc, ruta) · **sáb 30 = D-2 y dom 31-ene = D-1 FUERA del plan** (descanso; solo Anki vencido + ritual el domingo) · **EXAMEN lun 1-feb-2027 (target v5.12)**. NBME 30 (D75 mié 30-dic) sigue siendo el último NBME de la Fase A, pero ya no la cierra: tras él quedan D76 lun 4-ene (ansiedad/personalidad/infancia/sustancias) · D77 mar 5-ene (psicofármacos) · D78 mié 6-ene (biostats) · D79 jue 7-ene (bioquímica 1) · D80 vie 8-ene (UWSA2) · D81 lun 11-ene (bioquímica 2, cierre). Reproductive cierra el **mar 22-dic = D70** y MSK **D71-D73 (mié 23 → lun 28-dic, 25-dic libre)** íntegro DESPUÉS del NBME 29 (D68 vie 18-dic): el corrimiento sigue sin partir el sistema. **Viernes de nivel 4 desde S11: D53 vie 27-nov y D63 vie 11-dic** · viernes de nivel 3: D8, D33, D48 (D18, D23 y D38 caen en 1.º-2.º día de sistema → nivel 1) |
| Días de plan | L-V; feriados fuera del plan: 25-dic, 31-dic, 1-ene. **Verificado con `node` el 15-sep: 95 entradas de `DIAS`, D1 = 2026-09-16 (mié) → D95 = 2027-01-29 (vie), D# 1-95 consecutivos, 0 sáb/dom/feriados.** **S1 = 16-18 sep (3 días: mié-vie)** · S15 = 21-24 dic (4) · S16 = 28-30 dic (3) · **S20 = 25-29 ene (5 días, la última)** |
...
| Examen Step 1 | **TARGET LUN 1-FEB-2027 (v5.12) — FUERA de la ventana original 25-29 ene, que el plan ocupa entera (D95 = vie 29-ene)** · sáb 30 = D-2 y dom 31 = D-1 de descanso fuera del plan · overlay 🎯 `oinh139dsnbuma9r3kfu56dhkc` 07:00-16:00 (FREE, no es la cita real) · **GO/NO-GO vie 15-ene** (NBME 31 = **D85**): 2 NBME consecutivos ≥68 % + UWSA2 low-risk · **⚠ Joseph: agendar/reprogramar Prometric y confirmar eligibility period; cada día más sin estudiar = +1 hábil en la fecha (o recortar)** |
| Secundarios (v5.12, leídos de los `.ts` con `node` el 15-sep) | MIR 15:15-16:15 (**78 días**, mié 16-sep → **mié 6-ene-2027** = 76 temas + D77 mar 5-ene mini-MIR + D78 mié 6-ene corrección; luego mantenimiento **60 días jue 7-ene→mié 31-mar-2027** = 48 lun-jue + 12 viernes, cierre fijo clavado al ENCAPS — cada corrimiento le quita 1 slot lun-jue y re-balancea las cuotas por peso; Tier C express los jueves; **modo reducido hasta el vie 29-ene**, 17 días reducidos) · ENCAPS 16:15-17:15 (mantenimiento **95 días**, mié 16-sep → vie 29-ene-2027 = **77 de banqueo + 18 mini-sims de viernes**, la primera el vie 18-sep; **pre-test de arranque 40Q ciego: mié 16-sep parte 1 (II-3 · I-3 · V-2 · III-5) + jue 17-sep parte 2 (II-5 · I-4 · IV-1+IV-2 · II-4)**, ficheros `DATA/ENCAPS/BANCO_PROPIO/pretest_arranque_2026-09-16/17.html`; Supabase `study_schedule`, backup `study_schedule_bk_0915` (bk_0914/0912/0910/0909/0908 intactos) — **A VERIFICAR (15-sep)** en Supabase en vivo desde este agente: las cifras vienen del orquestador; examen fin-mar 2027, meta ≥17/20; **⚠ la fase intensiva feb-mar arrancaría el día 96 = lun 1-feb = DÍA DEL EXAMEN → propuesta mar 2-feb (`gen_encaps_intensivo_2027.js 2027-02-02`, decisión de Joseph)**) · LIVIANO 17:15-18:00 (**90 días**, mié 16-sep → **vie 22-ene-2027**; 16 casos en viernes · 18 pre-tests de lunes · drills D36 mié 4-nov · D57 jue 3-dic · D75 mié 30-dic · D87 mar 19-ene; revisión trimestral I D45 mar 17-nov · II D89 jue 21-ene; capstone D88 mié 20-ene; **⚠ caso integral 16/16 = D90 vie 22-ene, DESPUÉS del capstone (inversión de 2 días porque los 16 casos van en viernes; decisión de Joseph)**) · Research↔Derma 13:30-14:15 (alternos: **Research ciclo 1 = 42 átomos mié 16-sep → jue 11-feb-2027 SIN cambio** (el 16-sep ya era su D1; D37 mar 29-dic → pausa Step 1 → D38 lun 1-feb = CR-9 **= día del examen**, mover al mar 2-feb es decisión de Joseph) + ciclo 2 = 67 átomos lun 15-feb → mié 18-ago-2027 · **Derma 73 sesiones jue 17-sep → vie 9-abr-2027** (+2 hábiles: el 16-sep es día Research; taper Step 1 = d44-d49 **mar 19-ene → mar 2-feb**; el vie 15-ene (NBME 31) es d43 = sesión NORMAL (Mohs), alternativa registrada: swap d43↔taper, decisión de Joseph; ciclo 2 d74-d103 mar 13-abr → vie 2-jul-2027, cp1 d85 13-may · cp2 d94 8-jun · repaso2 d103 2-jul)) · AURUM 14:15-15:15 (**130 días**, mié 16-sep → **vie 19-mar-2027**; pitches D15 mar 6-oct · D35 mar 3-nov · D55 mar 1-dic · D75 mié 30-dic · D95 vie 29-ene · D115 vie 26-feb · D130 vie 19-mar) · SYNAPSE 12:30 (30') (**132 días · 19 semanas**, mié 16-sep → **lun 25-ene-2027**; sem 1 = 16-20 sep corta (5 días), sem 19 = 8 días hasta el lun 25-ene; F2 sem 13-19 lun 7-dic → lun 25-ene; 110 A-units intactas; la última cae el lunes de la última semana del plan) + vibecoding 04:15 (**60 días L-V en bloques de 5 hábiles**, mié 16-sep → **mar 8-dic-2026**, 12 proyectos, SHIP sábados 26-sep · 3-oct · 10-oct · 17-oct · 24-oct · 31-oct · 7-nov · 14-nov · 21-nov · 28-nov · 5-dic · 12-dic; S7 deload mié 28-oct → mar 3-nov; **taper S13-S20 = bloques secuenciales mié 9-dic → vie 29-ene** (S13 9-15 dic · S14 16-22 dic · S15 23-29 dic (4) · S16 30-dic→5-ene (3) · S17 6-12 ene · S18 13-19 ene · S19 20-26 ene · S20 27-29 ene (3)), 95 días = mismo D# que el Step 1) · Business **121** filas (84 trabajo + 37 descanso, mié 16-sep → jue 14-ene-2027; sin serie propia en el Calendar) |
| Fin de semana | **SÁBADO Y DOMINGO LIBRES de banco de preguntas** (los hitos van en viernes). No es "libre" literal: despertar 04:00 (EKER), caminata, SYNAPSE PC 15:00-17:00 ambos días, video empresa sáb 17:00, Anki sáb 19:00 / dom 17:00, baile. **Excepción v5.12: sáb 30 y dom 31-ene-2027 = descanso pre-examen (D-2/D-1): solo Anki vencido, nada después de las 17:00 del domingo** |
| Corrimiento | cada día hábil sin estudiar = +1: `node DATA/_scripts/remap_inicio.js <fecha>` + `gen_encaps_mantenimiento_2027.js` + `gen_synapse_plan.js` + `gen_aurum_plan.js` + regenerar USMLE. **Los overlays de hito USMLE NO se mueven** (fechas fijas) **salvo que su fecha ya haya pasado** — es lo que le ocurre al UWSA1 en cada corrimiento — pero **su D# sí cambia** y hay que reescribir la `description` de los 12 (ver §5 y §14). **Desde v5.12 hay un 13.º overlay: el del EXAMEN (lun 1-feb)**, que SÍ se mueve +1 hábil en cada corrimiento futuro (o Joseph decide recortar). **Los 12 overlays 🔬 RESEARCH SÍ se mueven** cuando cambia el D1 de Research (no fue el caso en v5.12): `node DATA/_scripts/gen_research_calendar.js` → `update_event` de cada `recrear` → `--set hito=eventId` → `--check` = 0 (§5.1) |
...
### 0.1 · Semana real de arranque v5.12 — mié 16 → vie 18 sep 2026 (S1 corta, 3 días) + lun 21
La semana de arranque vuelve a ser **corta (3 días hábiles, mié-vie)** y el miércoles **es el UWSA1**. Los **13 bloques de estudio** del régimen L-V + el overlay se verificaron en vivo el **15-sep con `list_events` sobre el mié 16** (29 eventos ese día: 04:15 IA · 05:00 Anki AM · 07:15 repaso · 08:15 pre-test · 09:00 deep prime · **09:00-13:00 🎯 UWSA1 (evento suelto, color 6, `transparent`)** · 11:00 30Q · 12:30 SYNAPSE · 13:30 Research↔Derma · 14:15 AURUM · 15:15+15:30 MIR · 16:15 ENCAPS · 17:15 LIVIANO · 18:00 eval; el resto son rutina/comida/BAILE — el miércoles no hay GYM, solo BAILE 19:00-20:30) y **sobre el mar 15**, que ya no tiene ningún overlay (29 eventos, todos series; el martes ALISTARSE 18:30 + VIAJE 18:45 + BAILE 19:00 + GYM 19:30). Solo se movió un evento USMLE (UWSA1); se creó **un** evento nuevo (el del examen, §5) y no se borró ninguno.

| Día | D# USMLE | Bloques de estudio (13/13 series) | Hito |
|---|---|---|---|
| ~~Mar 15-sep~~ | — | **NO estudiado** — origen del corrimiento a v5.12 (el UWSA1 que estaba aquí se movió al mié 16). Los 13 bloques siguen apareciendo en el Calendar (son series) pero el plan no cuenta el día | — |
| **Mié 16-sep** | **D1** — Assessment · nivelUW 5 · 160Q · **el D1 es el UWSA1** · Research **D1** en la franja 13:30 · ENCAPS **pre-test de arranque parte 1** (II-3 · I-3 · V-2 · III-5) a las 16:15 · MIR D1 · AURUM D1 · LIVIANO D1 · vibecoding S1 d1 · SYNAPSE d1 | los 13 bloques — **el overlay 🎯 UWSA1 09:00-13:00 pisa deep prime, 30Q, almuerzo y SYNAPSE** (ver §5) | 🎯 **UWSA1 BASELINE** |
| Jue 17-sep | **D2** — Fundamentos · Pathoma 1-2 (lesión/muerte celular + inflamación) · nivelUW 1 · 30Q · Derma **d1** en la franja 13:30 · ENCAPS **pre-test de arranque parte 2** (II-5 · I-4 · IV-1+IV-2 · II-4) | los 13 bloques, sin overlay | — |
| Vie 18-sep | **D3** — Fundamentos · Pathoma 3 (neoplasia) + setup Anki FSRS · Research **D2** · ENCAPS **1.ª mini-sim 25Q** · GYM 19:00 + BAILE 20:00 | los 13 bloques | — |
| Sáb 19-sep | — | libre de banco · 15:00-17:00 SYNAPSE PC (**aún sin SHIP**: el 1.er SHIP es el sáb 26-sep, S1 = apex-e2e mié 16 → mar 22) | — |
| Dom 20-sep | — | libre de banco · SYNAPSE repaso + PC · Anki 17:00 · baile | — |
| Lun 21-sep | **D4** — Immunology · inmunidad innata/adaptativa + MHC + linfocitos T/B · Derma **d2** | los 13 bloques | — |
| Mar 22-sep | **D5** — Immunology (hipersensibilidad…) · Research **D3** = hito **M1** (Dr. Ciro) → overlay 🔬 13:30-14:15 (§5.1) · vibecoding **último día de S1** | los 13 bloques + overlay Research | 🔬 M1 |

⚠ **El D1 del plan es un hito y eso es lo correcto**: Palmerton pide la línea base *antes* de tocar contenido. Pathoma 1-2, que en v5.11 era el D2 del mié 16-sep, pasa a ser el **D2 del jue 17-sep**; **nada se perdió** — todo el temario se desplazó un día hábil y el plan se alargó por la cola, **ocupando el vie 29-ene (que era el target de examen) como D95 = última sesión y empujando el examen al lun 1-feb**.
...
**En v5.12, 11 de las 12 FECHAS no se movieron — los D# sí (todos exactamente −1). La excepción vuelve a ser el UWSA1: su fecha (mar 15-sep) ya había pasado sin estudiar, así que el 15-sep se movió el evento al mié 16-sep 09:00-13:00 (`update_event` con `startTime`/`endTime` nuevos y `timeZone: America/Lima`; título, `colorId` 6, `transparency` y ausencia de recurrencia verificados intactos en la respuesta del `update_event`, en un `get_event` posterior y en `list_events` del mié 16) y conserva el D1. Novedad v5.12: un 13.º overlay, el del EXAMEN (lun 1-feb-2027), creado con `create_event`.** Cada D# de la tabla se leyó con `node` del array `DIAS` de `src/lib/usmleStep1Daily.ts` (95 entradas, D1 = 2026-09-16 → D95 = 2027-01-29), no se estimó. El **15-sep-2026 se reescribió el campo `description` de los 12 overlays** para sustituir el D# viejo y la referencia "D1 = mar 15-sep / v5.11" por "**D1 = mié 16-sep, v5.12, 95 días**", y donde decía "target vie 29-ene, jue 28 = D95 = D-1 dentro del plan" ahora dice "**D95 = vie 29-ene última sesión · sáb 30 y dom 31 descanso fuera del plan · EXAMEN lun 1-feb-2027 (target v5.12, fuera de la ventana: agendar/reprogramar Prometric)**"; **el protocolo de test-day de Palmerton se conservó íntegro** (§F de `DATA/USMLE/PALMERTON_POR_MATERIA.md`): Ziploc BREAK 1/2/3 preparadas la noche anterior (proteína magra + grasas, **sin carbohidratos simples**), cronograma de descansos (sit-in 1-2' / activo 10' / almuerzo dentro del sim), sin cambiar respuestas por ruido, SYNAPSE de ese día a la tarde, registro de agua/sueño en VITALS como ensayo, y el mapa del día real de 7 bloques. Horas (salvo el UWSA1), títulos, `colorId`, `transparency` y la ausencia de recurrencia quedaron intactos (comparados en la respuesta de cada `update_event`, que devuelve el recurso completo).

| # | Fecha | Hora | Título literal | ID | **D# v5.12** | D# v5.11 (histórico) | Mínimo on-track (texto del overlay) |
|---|---|---|---|---|---|---|---|
| 1 | **16-sep-2026 (mié)** ⚡ movido desde el mar 15-sep | 09:00-13:00 | 🎯 UWSA1 — BASELINE Step 1 (160Q · 4 bloques) | `o1gla7846uae4tgngvc4q45osg` | **D1** | D1 (en el mar 15) | línea base — **el DÍA 1 del plan** (Palmerton: baseline en el primer día) |
| 2 | 02-oct-2026 (vie) | 07:15-11:00 | 🎯 NBME 25 (200Q) + revisión | `4hjv5lkvluj06ahc2qndtsi6as` | **D13** | D14 | ≥51 % |
| 3 | 23-oct-2026 (vie) | 07:15-11:00 | 🎯 NBME 26 (200Q) + revisión | `lr7ktrbiffj4hrlke7lh7cv6h4` | **D28** | D29 | ≥54 % |
| 4 | 13-nov-2026 (vie) | 07:15-11:00 | 🎯 NBME 27 (200Q) + revisión | `sm4baa2v453ifaub325h9v08mg` | **D43** | D44 | ≥57 % (gate 1 ECFMG pide ≥55 %) |
| 5 | 04-dic-2026 (vie) | 07:15-11:00 | 🎯 NBME 28 (200Q) + revisión | `ecu784689p8osrhqabuct8d5jg` | **D58** | D59 | ≥61 % |
| 6 | 18-dic-2026 (vie) | 07:15-11:00 | 🎯 NBME 29 (200Q) + revisión | `ae93qv0nqqs36h439hid1jcq8o` | **D68** | D69 | ≥63 % · lun 21-dic = D69 (mama/masculino) · mar 22-dic = D70 cierra Reproductive · MSK D71-D73 (mié 23 → lun 28-dic, 25-dic libre) íntegro DESPUÉS |
| 7 | 30-dic-2026 (mié) | 07:15-11:00 | 🎯 NBME 30 — CIERRE FASE A (200Q) + plan Fase B | `mael3p9uhp036jbep45ql6v9oc` | **D75** | D76 | ≥65 % · último NBME de la Fase A (el título se conserva; en v5.12 la Fase A sigue hasta el D81 lun 11-ene: D76 psiq · D77 psicofármacos · D78 biostats · D79 bioquímica 1 · D80 UWSA2 · D81 bioquímica 2) |
| 8 | 08-ene-2027 (vie) | 09:00-13:00 | 🎯 UWSA2 — predictor gold-standard (160Q) | `2u7viv0elr6aedo0m9undfa5kg` | **D80** | D81 | low-risk · **cae DENTRO de la Fase A**; la Fase B arranca el mar 12-ene (D82-D85, 4 días) |
| 9 | 15-ene-2027 (vie) | 07:15-11:00 | 🎯 NBME 31 (200Q) — GO/NO-GO fecha de examen | `l771lvcfv0jcebt61do6svia18` | **D85** | D86 | **GO = 2 NBME ≥68 % + UWSA2 low-risk** · cierra Fase B (4 días) · GO → confirmar Prometric lun 1-feb |
| 10 | 18-ene-2027 (lun) | 07:15-11:00 | 🎯 NBME 32 (200Q) + repaso FA 1-5 | `h65e772pqsa4hgc5b6bh9n8h50` | **D86** | D87 | ≥68 % · sprint (arranca la Fase C); mañana D87 = random timed 2×40Q + sistema débil #2 |
| 11 | 20-ene-2027 (mié) | 07:15-11:00 | 🎯 NBME 33 (200Q) + repaso FA 11-14 | `la5rsbuiqj1o35lb5adf93abuo` | **D88** | D89 | ≥68 % · sprint (Fase C); D89 y D91 incorrects 2.ª pasada · D92-D93 AMBOSS 200 mitades 1-2 |
| 12 | 22-ene-2027 (vie) | 07:15-11:00 | 🎯 FREE 120 oficial + logística del examen | `lh9jfjsmoif74ci6jcn8f1mq30` | **D90** | D91 | ≥70 % (heurística comunitaria) · último ensayo · **no es el último día del plan** (D95 = vie 29-ene) |
| 13 | **01-feb-2027 (lun)** 🆕 | 07:00-16:00 | 🎯 USMLE STEP 1 · EXAMEN (target v5.12 — agendar en Prometric) | `oinh139dsnbuma9r3kfu56dhkc` | **—** (día del examen; fuera del plan) | — (no existía) | overlay FREE, color 6, **no es la cita real de Prometric**: recuerda agendar/reprogramar y confirmar el eligibility period; incluye los 3 días previos (D95 · sáb D-2 · dom D-1) y el protocolo de test-day de Palmerton §8.4 |
...
⚠ **El plan no termina con el Free 120.** El **último día es el vie 29-ene-2027 = D95 = D-3 = última sesión** (rapid review de First Aid + Anki maduro + 20Q flagged en **sesión LIGERA**; tarde = logística de test-day: permiso impreso + digital, 2 ID con el nombre EXACTO del permiso, bolsas Ziploc numeradas Break #1-#4, ruta al Prometric — `USMLE_TAPER.d95`), **sin overlay** porque no es un simulacro. **Sáb 30 = D-2 y dom 31-ene = D-1 quedan FUERA del plan** (descanso: solo Anki vencido; el domingo nada después de las 17:00, ritual de test-day — `USMLE_TAPER.dMenos1`) y el **lun 1-feb = EXAMEN** (`USMLE_TAPER.examen`, overlay #13). El **lun 25-ene = D91** (incorrects 2.ª pasada + sistema débil #3), el **mar 26-ene = D92** y el **mié 27-ene = D93** son los últimos días de banco (uWorld incorrects + AMBOSS 200 mitades 1 y 2) y el **jue 28-ene = D94 = D-4** (repaso FA sistemas 6-10 + Anki marathon; taper: solo Anki maduro + 20Q flagged, nada nuevo). Todo esto ya está dentro de la descripción del overlay Free 120 y del overlay del examen.
⚠ Tras el 30-dic (D75) los feriados jue 31-dic y vie 1-ene quedan fuera del plan: **el siguiente día hábil es el lun 4-ene = D76** (ansiedad + personalidad + infancia + sustancias/toxidromes); **mar 5-ene = D77** (psicofármacos); **mié 6-ene = D78** (bioestadística + epidemiología + ética/comunicación, AMBOSS HY 155Q); **jue 7-ene = D79** = 1.º de los 2 días dobles de Bioquímica; **vie 8-ene = D80 = UWSA2 (dentro de la Fase A)**; **lun 11-ene = D81 cierra la Fase A** con el 2.º día doble (biología molecular + genética + farmacología general/toxicología). La **Fase B arranca el mar 12-ene = D82** y dura **4 días** hasta el NBME 31 (vie 15-ene = D85). Esta corrección ya está dentro de la descripción de los overlays NBME 30 y UWSA2.

**Colisiones de un día de hito (aceptadas, no se mueven franjas):**
- UWSA (09:00-13:00 + ~30' de descansos reales → ~13:30): pisa deep prime 09:00, 30Q 11:00, ALMUERZO 12:00 y SYNAPSE 12:30 → el almuerzo se come dentro del sim (Ziploc BREAK 2), SYNAPSE pasa a la tarde, la revisión completa va en las franjas USMLE de la tarde + 18:00. **Aplica ya el mié 16-sep (D1).**
...
- **EXAMEN lun 1-feb (07:00-16:00 + viaje)**: las 7 series USMLE/ENCAPS ya no existen ese día (`UNTIL=20270130`), pero **sí siguen apareciendo las series secundarias** (vibecoding 04:15, SYNAPSE 12:30, Research↔Derma 13:30 + overlay 🔬 CR-9, AURUM 14:15, MIR 15:15/15:30, LIVIANO 17:15, GYM 19:00 + BAILE 19:30) — verificado con `list_events` el 15-sep (23 eventos ese día). Decisión de Joseph: dejarlas (el overlay ya avisa) o vaciar ese lunes a mano (§14.3).
...
> **v5.12 (15-sep): SIN CAMBIOS.** El mié 16-sep ya era el D1 de Research en v5.11 (el plan es interdiario y el 15-sep era día Derma), así que ninguno de los 12 hitos cambió de fecha ni de texto; el orquestador reporta `gen_research_calendar.js --check` = 0 y este agente no tocó ningún overlay 🔬 (ids y fechas de la tabla siguen vigentes). ⚠ **CR-9 SUBMIT (lun 1-feb-2027, `t136recadd6vnv0iebrmmqiv48`) coincide ahora con el DÍA DEL EXAMEN Step 1** (verificado con `list_events`: ambos overlays FREE el mismo lunes, 13:30-14:15 y 07:00-16:00). Moverlo al mar 2-feb es decisión de Joseph: el átomo no se mueve solo (habría que regenerar el plan Research y correr `gen_research_calendar.js`).

| # | Hito | Código · d | Fecha v5.12 (= v5.11, sin cambio) | Fecha v5.10 | ID | Título literal |
|---|---|---|---|---|---|---|
...
## 6 · Descripciones de los bloques — releídas en vivo con `get_event` el **15-sep-2026**
Lo que sigue es un **resumen fiel de 1-2 líneas por serie** sobre el texto **vivo** leído hoy con `get_event`. El 15-sep se reescribió la `description` de **23 series** (10 de estudio/secundarias + 6 de rutina/comida/baile + las 7 rutinas de §6.3 — más los 12 overlays de hito USMLE del §5, uno de ellos además movido de fecha, y el overlay nuevo del examen). En todas las series se tocó **solo `description`**: RRULE, `start`/`end`, `summary` y `colorId` verificados intactos en la respuesta de cada `update_event` (devuelve el recurso completo, igual que `get_event`). **Ninguna serie se borró ni se recreó y no se usó `recurrenceData`.**
**Estado:** 🟢 = releída en vivo el 15-sep · ⚪ = no releída hoy (última captura 05-sep/08-sep/10-sep/12-sep/14-sep, sin cambios conocidos).
**⏸ SIN CAMBIO (releídas el 15-sep, no tocadas):** 4 series de estudio **siguen sin citar ni la versión del régimen ni D1 ni el nº de días ni ninguna fecha del cierre**, así que no había nada que corregir y se dejaron intactas: **repaso 07:15** `54lchqggik96dmljmmg3l88s54` · **pre-test 08:15** `3tbecd5n03ut6lno3hjvc1sr7k` · **30Q 11:00** `2eqmmrnh00jr44plevurgcu2as` · **eval 18:00** `utk2laeob9u0847bbe9rm491v4` (`updated` de las 4 sigue en 2026-08-27).

### 6.1 · Bloques de estudio (L-V)

| Hora · Serie | ID | Resumen fiel del texto VIVO (15-sep) | Estado |
|---|---|---|---|
| 04:15-05:00 · 🧠 IA VIBECODING | `udr09j9ng983o0d4nipkfe4494` | Curso de IA en modo BUILDER, no programar desde cero: 1 proyecto real terminado por semana al servicio de sus sistemas; ciclo diario 5' objetivo → 35' construir con Claude Code → 5' commit + nota (repo synapse-journal); freno 04:55; orden S1-S12 por riesgo para el Step 1 (orden v5.10-b del 12-sep, vigente en v5.12). **Cierre v5.12 (15-sep):** "12 proyectos · 60 días L-V en bloques de 5 hábiles · **mié 16-sep → mar 8-dic-2026** · 1er SHIP sáb 26-sep · último SHIP sáb 12-dic (los 12 sábados listados) · S7 (deload post-NBME 26) mié 28-oct → mar 3-nov · taper S13-S20 mié 9-dic → vie 29-ene-2027 en bloques SECUENCIALES de `pasos.length` hábiles (S13 9-15 dic · S14 16-22 dic · S15 23-29 dic (4) · S16 30-dic→5-ene (3) · S17 6-12 ene · S18 13-19 ene · S19 20-26 ene · S20 27-29 ene (3)); ni un proyecto ni un paso se fusionó ni se recortó; 95 días = mismo D# que el Step 1; D95 = vie 29-ene última sesión · sáb 30 y dom 31 descanso · EXAMEN lun 1-feb-2027" (`VIBE_PROYECTOS` + `VIBE_TAPER` + `VIBE_DIAS` leídos con node: 12 proyectos, 20 bloques, 95 entradas 2026-09-16 → 2027-01-29). | 🟢 |
| 05:00-05:45 · ANKI AM | `i8afj7uppkb3ntj8h9890dhecc` | Anki de madrugada (Palmerton, mente fresca) — Fase A: 45' FSRS deck USMLE; Fases B-C: 05:00-05:12 🔥 Stress Set 10Q + 05:12-05:45 Anki. **Cifra corregida el 15-sep:** "régimen v5.12: 95 días desde **mié 16-sep** hasta **vie 29-ene-2027** ≈ 594h; D95 = vie 29-ene última sesión (solo Anki maduro + 20Q flagged) · sáb 30 y dom 31 descanso fuera del plan (solo Anki vencido) · EXAMEN lun 1-feb-2027 (target v5.12, fuera de la ventana: agendar/reprogramar Prometric)". FSRS retención 0.90, máx 50 nuevas/día, solo Good/Again. | 🟢 ⚠ UNTIL |
...
| 08:15-09:00 · Pre-test ciego | `3tbecd5n03ut6lno3hjvc1sr7k` | 10Q uWorld ciegas en modo tutor del subtema del día, sin haber estudiado + free recall 90 s en papel + anotar los huecos como objetivo del deep prime. Sin referencia de versión (última edición 27-ago; releída el 15-sep). | 🟢 ⚠ UNTIL |
| 09:00-11:00 · DEEP PRIME | `cb2uh20jnvu7pgfev4183pgctc` | Bloque sagrado de 2h: vídeo 09:00-10:00 (B&B/Pathoma/Sketchy a 1.5x) · First Aid active reading 10:00-10:45 · ≤10 tarjetas de mecanismo + APEX 10:45-11:00; jerarquía de material; "💧 AL ABRIR" con 500 ml + snack y micro-pausa 10:00-10:05; ☕ última cafeína ≤ 11:00; los viernes de hito este bloque es el UWSA/NBME. **Cierre v5.12 (15-sep):** 95 días, **D95 = vie 29-ene-2027**, fases **A D1-D81 (16-sep → lun 11-ene; el UWSA2 D80 dentro de la Fase A) / B D82-D85 (12→15-ene) / C D86-D95 (18→29-ene)**, "**el D1 (mié 16-sep) es el UWSA1** — baseline en el primer día, como prescribe Palmerton; es el único hito que cambia de fecha en cada corrimiento (vie 11 → lun 14 → mar 15 → mié 16-sep) — y el contenido arranca el jue 17-sep (D2)", la frase "ni un tema se fusionó ni se recortó", "⚠ CONSECUENCIA v5.12: el plan llena TODA la ventana 25-29 ene → D95 = vie 29-ene última sesión · sáb 30 y dom 31 descanso fuera del plan · EXAMEN lun 1-feb-2027 (agendar/reprogramar Prometric y confirmar eligibility period). Desde aquí cada día no estudiado mueve el examen un hábil más (o exige recortar temario: decisión de Joseph)", GO/NO-GO vie 15-ene (**NBME 31 = D85**), "su contenido ya es v5.12". | 🟢 ⚠ UNTIL |
...
| 12:30-13:00 · SYNAPSE misión | `j99thg3eaqesosmvppj4rfgvh4` | Misión del día en la app (A · 15' lección + B · 10' audio + C · 5' lectura móvil; marcar ✓). **Cierre v5.12 (15-sep):** "**132 días · 19 semanas · mié 16-sep-2026 → lun 25-ene-2027**; con el D1 en miércoles la sem 1 = 16-20 sep es corta (5 días) y la sem 19 se alarga hasta el lun 25-ene: la última A-unit cae el lunes de la última semana del plan Step 1, 4 días antes del D95; las 110 A-units siguen intactas; F0 sem 1-8 · F1 sem 9-12 · **F2 sem 13-19 (lun 7-dic → lun 25-ene)** = Anthropic Academy restante + prep CCA-F; sem 17-19 DELOAD; del mar 26 al vie 29-ene (D92-D95) y el finde 30-31 (descanso pre-examen): sin SYNAPSE; EXAMEN lun 1-feb-2027" (`SYN_PLAN_META` + `SYN_DIAS` leídos con node: 132 entradas, sem 1 = 5 días mié→dom, sem 19 = 8 días lun 18 → lun 25-ene). | 🟢 |
| 13:30-14:15 · RESEARCH ↔ DERMA | `3ofg2ljv8kl3p1adm2e5d5nih3` | **Mapa de arranque recalculado el 15-sep** (el ancla de la alternancia sigue siendo el mié 10-jun-2026 y NO se mueve): **mié 16 RESEARCH (D1) · jue 17 DERMA (d1) · vie 18 RESEARCH (D2) · lun 21 DERMA (d2) · mar 22 RESEARCH (D3 · hito M1) · mié 23 DERMA (d3) · jue 24 RESEARCH (D4) · vie 25 DERMA (d4)**…, contrastado 1:1 con las fechas reales de `researchDailyPlan.ts` (D1 16-sep · D2 18-sep · D3 22-sep · D4 24-sep) y `dermaDailyPlan.ts` (d1 17-sep · d2 21-sep · d3 23-sep · d4 25-sep). Planes vigentes: **Research ciclo 1 = 42 átomos mié 16-sep-2026 → jue 11-feb-2027 SIN cambio** (D37 mar 29-dic → pausa Step 1 → D38 lun 1-feb = CR-9, ⚠ = día del examen; mover al mar 2-feb es decisión de Joseph) + ciclo 2 = 67 átomos lun 15-feb → mié 18-ago-2027 · **Derma 73 sesiones jue 17-sep-2026 → vie 9-abr-2027** (corrió 2 hábiles; taper d44-d49 mar 19-ene → mar 2-feb; vie 15-ene = d43 sesión normal; ciclo 2 d74-d103 mar 13-abr → vie 2-jul). Ambos saltan 25-dic, 31-dic y 1-ene; ni un átomo ni una sesión se fusionó o se recortó. Los 12 hitos Research llevan overlay 🔬 (§5.1, sin cambios). EXAMEN Step 1 lun 1-feb-2027. | 🟢 |
| 14:15-15:15 · AURUM | `at1nak8f24nbnj1mh2jcd4aggg` | Closer de ventas: 26 semanas · 130 lecciones · 7 fases; solo referentes reales con track record verificable (Hormozi, Cardone, Voss, Cialdini, Rackham, Girard, Tracy, Blount, Ross, Hopkins, Bettger + Josué Peña solo gratis) y 11 libros ancla; cada día 1 vídeo + drill de pitch sobre leads reales (ALLPA / Qori Golden); la lectura va en los huecos de viaje. **Cierre v5.12 (15-sep):** D1 = **mié 16-sep-2026 → D130 = vie 19-mar-2027** + pitches v1 D15 mar 6-oct · v2 D35 mar 3-nov · v3 D55 mar 1-dic · v4 D75 mié 30-dic · v5 D95 vie 29-ene · v6 D115 vie 26-feb · v7 D130 vie 19-mar (`AURUM_PLAN_META` + `AURUM_PITCH_DIAS` leídos con node; 130 entradas). | 🟢 |
| 15:15-15:30 · MIR eval anclada | `2ldp6obaapnvo76li28uprrddg` | 4Q ProMIR/AMIR del tema D-1 en modo examen (~90-100 s/Q, sin compendio) + 5' Anki deck MIR + 3' mini-corrección tipificando el error (conceptual / terminológico / aplicación); umbrales 4/4 avanzar · 3/4 zona gris · <3/4 repetir mañana; conserva "💧 AL ABRIR" y la justificación científica (Karpicke, Cepeda, Newport, Croskerry). **Etiqueta v5.12 (15-sep):** MIR = 78 días (**mié 16-sep-2026 → mié 6-ene-2027**: 76 temas + D77 mar 5-ene mini-MIR 40Q + D78 mié 6-ene corrección); del jue 7-ene al mié 31-mar-2027 sigue en banqueo de mantenimiento (**60 días = 48 lun-jue + 12 viernes, cierre fijo clavado al ENCAPS**; Tier C express los jueves; **modo reducido hasta el vie 29-ene** = último día del plan Step 1; `MIR_MANT_META`: inicio 2027-01-07, fin 2027-03-31, 60 días, 17 reducidos); D95 = vie 29-ene última sesión · sáb 30 y dom 31 descanso · EXAMEN lun 1-feb-2027. | 🟢 |
| 15:30-16:15 · MIR deep work mini | `00k364heibh1n6f9hfspcv9dpi` | 1 subtema atómico/día en 4 sub-fases: pre-test + generation 5' · lectura activa + elaboración 25' · free recall libro cerrado 5' · ≤4 APEX 10' (formato completo FRENTE/REVERSO/CCSN/FISIOPATOLOGIA/RELACIONES, ≥1 comparativa MIR vs ENCAPS); identity stacking y métricas en Obsidian 06_MIR/Subtemas. **FUNDAMENTO recalculado el 15-sep:** "78 días × 1 subtema/día × ≤4 cards ≈ 310 cards MIR sólidas al **mié 6-ene-2027** (76 temas + D77 mar 5-ene mini-MIR + D78 mié 6-ene corrección); del jue 7-ene al mié 31-mar-2027 el bloque sigue en banqueo de mantenimiento (60 días = 48 lun-jue + 12 viernes, cierre fijo clavado al ENCAPS; modo reducido hasta el vie 29-ene = D95 última sesión del Step 1; EXAMEN Step 1 lun 1-feb-2027)" + etiqueta **v5.12 (D1 = mié 16-sep-2026)**. | 🟢 |
| 16:15-17:15 · ENCAPS mantenimiento | `papebi46etlo8glgfs5akd5mig` | 16:15-16:30 eval anclada 5Q del tema de ayer · 16:30-17:15 banco 20-25Q por rotación semanal ponderada (II 30 % · I 27 % · V 21 % · III 13 % · IV 9 %) · viernes mini-simulacro 25Q a 72 s/Q · 8 críticos v3 · registrar CADA fallo en TRACKING_ERRORES; conserva el "🥤 CIERRE 17:10-17:15" de cena ligera. **Cierre v5.12 (15-sep):** **95 días** mié 16-sep-2026 → vie 29-ene-2027 (Supabase `study_schedule`, backup **`study_schedule_bk_0915`** — bk_0914/0912/0910/0909/0908 intactos; **77 días de banqueo + 18 mini-sims de viernes**, la primera el vie 18-sep), "el corrimiento se comió otro día de mantenimiento porque el cierre del 29-ene es fijo: **96 → 95** (la sesión que cae es la del mar 15-sep)", **PRE-TEST DE ARRANQUE 40Q ciego: mié 16-sep parte 1 (II-3 · I-3 · V-2 · III-5) + jue 17-sep parte 2 (II-5 · I-4 · IV-1+IV-2 · II-4)** (`BANCO_PROPIO/pretest_arranque_2026-09-16/17.html`), "⚠ la fase intensiva feb-mar 2027 arrancaría en el día 96 = lun 1-feb-2027 = DÍA DEL EXAMEN Step 1 → propuesta: arrancarla el mar 2-feb (`gen_encaps_intensivo_2027.js 2027-02-02`; decisión de Joseph)", "su contenido ya es v5.12". Cifras de Supabase tomadas del orquestador (**A VERIFICAR (15-sep)** en vivo: este agente no toca Supabase). | 🟢 ⚠ UNTIL |
| 17:15-18:00 · LIVIANO | `8epae6hlfmrc9j0h2kib7iuc84` | 17:15-17:40 módulo del día (fisiología del peso · GLP-1/tirzepatida · nutrición · ejercicio · conducta · farmacología/bariátrica) · 17:40-18:00 aplicación explicándolo como a un paciente real; fuentes ProMIR + AMBOSS + guías AACE/OMA; conserva "🥤 ARRANQUE 17:15" y la coherencia LIVIANO (última comida sólida ≥3h antes de dormir). **Cierre v5.12 (15-sep):** 90 días L-V **mié 16-sep-2026 → vie 22-ene-2027** (`LIV_META` + `LIV_DIAS` de `src/lib/livianoStudyPlan.ts` con node: 90 entradas, 16 casos en viernes, 4 drills — D36 mié 4-nov · D57 jue 3-dic · D75 mié 30-dic · D87 mar 19-ene; revisión trimestral I D45 mar 17-nov · II D89 jue 21-ene; capstone D88 mié 20-ene; **⚠ caso integral 16/16 = D90 vie 22-ene, DESPUÉS del capstone (inversión de 2 días porque los 16 casos van en viernes; decisión de Joseph)**; **18** `pretest:true`, todos en lunes; salta 25-dic, 31-dic y 1-ene). | 🟢 |
...
⚠ **UNTIL** = la RRULE de esa serie termina el **vie 29-ene-2027** (`UNTIL=20270130T045959Z`). Son **7 series**, no 3. Detalle y plan de acción en **§14.3** (la de ENCAPS 16:15 es la única bloqueante: hay que extenderla **antes del mar 2-feb-2027** si la fase intensiva arranca ese día). **Reconfirmado el 15-sep en la respuesta de cada `update_event` y con `list_events` del vie 29-ene y del lun 1-feb-2027: ANKI AM, repaso, pre-test, DEEP PRIME, 30Q, ENCAPS y eval conservan `RRULE:FREQ=WEEKLY;UNTIL=20270130T045959Z;BYDAY=MO,TU,WE,TH,FR` — aparecen el vie 29-ene y ya NO el lun 1-feb.**

### 6.2 · Comida, entrenamiento y fin de semana

| Bloque · Serie | ID | Resumen fiel del texto VIVO (15-sep) | Estado |
|---|---|---|---|
| 07:00-07:15 · DESAYUNO L-V | `7agi60f2bp8qnh6cnqvfo22giv` | ≥30 g de proteína + 500 ml de agua + carbohidrato COMPLEJO (nada de azúcar simple ni jugo envasado); 15' de combustible, sin banco de preguntas en la mesa; VITALS quick-log de sueño y primera agua; 1.ª de las 4 tomas de proteína (07:00 · 12:00 · 15:15 · 17:15) y lista de ventanas de hidratación del día. **Etiqueta actualizada el 15-sep → "Régimen v5.12 (D1 = mié 16-sep-2026)"** (regla de cafeína intacta). | 🟢 |
| 06:45-07:15 · DESAYUNO sáb/dom | `2u9je70pif58hgf2f8t3vgu8b8` | Misma regla sin prisa (aquí no hay bloque a las 07:15 → se puede comer sentado 30'); VITALS es donde más se pierde el registro el fin de semana; el Anki de sáb 19:00 / dom 17:00 lo dimensionan las tarjetas **due**, no el reloj (cero backlog el lunes, regla del 100 % de Palmerton). **Etiqueta actualizada el 15-sep a v5.12**. | 🟢 |
| 12:00-12:30 · ALUMUERZO | `43dq3oib16esjcqj1dcd8osot2` | Comida principal dimensionada por VITALS (proteína + verduras + carbohidrato complejo), 500 ml de agua, pausa difusa sin pantalla del Step 1, sin azúcar simple para evitar el bajón de 13:30-15:00; en los UWSA (09:00-13:00) se come DENTRO del sim como Break 2 y en los NBME (07:15-11:00) queda igual a las 12:00; ahora dice "(v5.12: el UWSA1 cae en MIÉRCOLES 16-sep = D1, misma regla)". **Etiqueta actualizada el 15-sep a v5.12**. | 🟢 |
| BAILE mié 19:00-20:30 · sáb 20:00-20:30 · dom 18:00-19:30 | `0tod56pc6pgecm5lf7g4e3ji9v` · `3jkfb6097rtc1jbec19eg3sd57` · `7j6pucil43vuca70p082g5ub3d` | Hidratación durante (300-500 ml); **post-entreno SOLO agua o electrolitos (500-750 ml)**, nada sólido pesado a <2h de las 21:00 (Walker: la digestión activa fragmenta el sueño profundo); la cena ligera va antes (17:15 L-V · ~19:30 sáb tras el Anki · ~17:00 dom); si hay hambre real al llegar, máximo 1 yogur o queso fresco; 0 luz azul tras 20:30 y cama 21:00. **Etiqueta actualizada el 15-sep a v5.12 en las tres**. | 🟢 |
| Sáb 15:00-17:00 · SYNAPSE PC | `hv2lk04orquvivthtkfhilb1ps` | **Sin tocar desde el 10-jun**: "setup, notebooks, repo público synapse-journal, Problem Sets de CS50P + ponerte al día con la semana". **Contradice** el texto vivo de las 04:15 y 12:30, que ya definen el sábado como SHIP del proyecto de vibecoding (1.er SHIP sáb 26-sep en v5.12) → pendiente (§14.3). | ⚪ |
| Dom 15:00-17:00 · SYNAPSE repaso + PC | `s7r8tiu66286t156l0odpv5nvo` | **Sin tocar desde el 10-jun**: repaso semanal Feynman + terminar el bloque PC, y aún explica que "la serie arranca el 28-jun" (histórico obsoleto). El texto vivo de las 04:15/12:30 ya dice que el domingo es Feynman de 10' opcional → pendiente (§14.3). | ⚪ |

⚠ Las 7 rutinas de **§6.3** (EKER, CORRER, CALISTENIA, LECTURA DE LIBRO, VIAJE VUELTA, ANKI sáb y ANKI dom) se **releyeron y re-etiquetaron a 'Régimen v5.12 (D1 = mié 16-sep-2026)' el 15-sep** (solo la última línea de la description; RRULE/horas/títulos intactos; verificado con la respuesta de update_event). **Única excepción de texto en EKER `22bh9m5jhc7ro6aj3m4ffjad7g`:** la frase 6 "IDENTIDAD DE EXAMEN (vigente hasta el vie 29-ene-2027…)" se cambió a "**vigente hasta el lun 1-feb-2027, día del examen Step 1 (target v5.12)**" porque esa fecha ES el examen y dejarla en el 29-ene sería un dato falso; las 5 frases de negocio siguen intactas. DESAYUNO y DEEP PRIME llevan además la regla de la cafeína. ⚪ **No releídas el 15-sep** (última captura 05-sep/08-sep, sin cambios conocidos; ninguna cita la versión del régimen salvo GYM/BAILE): **PREPARAR PARA CORRER / DUCHA / ALISTARSE / VIAJE / DORMIR** — sin descripción o sin versión · **las 9 series de GYM y BAILE que no son BAILE mié/sáb/dom** — descripción común post-entreno del 05-sep (ver §2) que **sigue etiquetada "Régimen v5.6"** (visto en vivo el 12-sep en la lectura del vie 11 con `list_events`: GYM vie `7sf8i7pe62pugfk4t6tcnbb9i4` y BAILE vie `5pd4jhmvl31hvbuh40vphpoc4q`; las otras 7 A VERIFICAR; fuera del alcance de las pasadas 08→15-sep, ver §14.3) · **SYNAPSE PC sáb/dom** — ver tabla.
...
### 14.3 · Pendiente REAL (estado al 15-sep-2026)
