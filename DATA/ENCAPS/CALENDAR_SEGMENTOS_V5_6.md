# 📅 SEGMENTOS DEL CALENDAR — RÉGIMEN v5.9 (corrimiento aplicado en vivo 10-sep-2026)
> **📌 Nota v5.9 (10-sep-2026).** Este documento **ya refleja el corrimiento a D1 = viernes 11-sep-2026** (el 10 de septiembre tampoco se estudió). El nombre del fichero conserva `V5_6` **solo por los enlaces existentes**: lo citan como autoridad varias descripciones de series del Calendar y otros docs del repo, y renombrarlo los rompería. El contenido es v5.9 en todas sus secciones; toda referencia a "v5.6"/"v5.7"/"v5.8" fuera de las citas literales/históricas está superada. Última pasada sobre el Calendar en vivo: **10-sep-2026, 24 descripciones reescritas** (§10).
> ⚠ **La regla permanente de Joseph (confirmada otra vez el 10-sep como "reorganización total"): NO se fusiona ni se recorta contenido — ni un tema ni un subtema se deja atrás.** El desfase se absorbe **alargando el final del plan**, nunca comprimiendo días. En v5.9 el Step 1 termina el **mar 26-ene-2027 (D95)**. Los 2 "días dobles" de Bioquímica que ya existían en v5.7 se conservan tal cual (no se crearon días dobles nuevos).
> ⚡ **Novedad estructural de v5.9: el D1 ES el UWSA1.** El 11-sep era la fecha del UWSA1 (anclada por fecha, no por posición). Al arrancar el plan ese mismo día, el UWSA1 pasa de D2 a **D1** — baseline en el primer día, que es exactamente lo que prescribe Palmerton — y el primer día de **contenido** (Fundamentos · Pathoma 1-2) se desplaza al **lun 14-sep = D2**. Es correcto y esperado: **el hito no se movió**, se movió el plan.
> Fuente: Google Calendar `josephsototocas@gmail.com` · zona `America/Lima` · estructura L-V/sáb/dom extraída el 05-sep (166 eventos), reverificada en vivo el 08-sep, reescrita el 09-sep (28 descripciones) y **reescrita el 10-sep con `get_event` + `update_event` + `get_event` de verificación en los 24 eventos tocados** (12 overlays + 12 series). En los 24 se cambió **solo `description`**: horas, títulos, RRULE, `colorId` y `transparency` verificados intactos.
> **Autoridad de CUÁNDO y CÓMO** se ejecuta cada segmento (L-V + sábado/domingo). El **QUÉ** (tema/misión del día) lo mandan la app YoCPMD y sus planes: `src/lib/usmleStep1Daily.ts` (**95 días**, D1 = 2026-09-11 → D95 = 2027-01-26), Supabase `study_schedule` (ENCAPS **98 días**, `encapsPlan.ts`), `mirDailyPlan.ts`, `researchDailyPlan.ts`/`dermaDailyPlan.ts`, `aurumDailyPlan.ts`, `livianoStudyPlan.ts`, `synapseDailyPlan.ts`, `vibecodingPlan.ts`.
...
## 0 · Reglas del régimen v5.9 (las que el Calendar materializa)
| Regla | Valor |
|---|---|
| **D1** | **viernes 11-sep-2026** (31-ago→10-sep no estudiados; **8 corrimientos acumulados**. v5.8 tenía D1 = jue 10-sep: el 10 de septiembre no se estudió y el plan corrió +1 día hábil) |
| **D1 = hito** | El D1 **es el UWSA1** (baseline, 09:00-13:00). El contenido arranca el **lun 14-sep = D2** (Pathoma 1-2). Es la consecuencia de que los 12 hitos estén **anclados por fecha** desde v5.8 |
| **Fases USMLE** | **A · contenido D1-D80** (11-sep→**5-ene**) · **B · banco D81-D88** (6-ene→15-ene) · **C · sprint D89-D95** (18-ene→**26-ene**). **No se fusionó ni se recortó ningún día de contenido**: el multiconjunto de temas es idéntico al de v5.8 y el desfase se absorbió al final (D95 = mar 26-ene). D90 (19-ene) y D92 (21-ene) siguen siendo días de banco alojados dentro de la semana del sprint |
| Días de plan | L-V; feriados fuera del plan: 25-dic, 31-dic, 1-ene. **Verificado con `node`: 0 sábados, 0 domingos y 0 feriados en las 95 entradas de `DIAS`** |
...
| Examen Step 1 | semana **25-29 ene 2027** (target mié 27) · **GO/NO-GO vie 15-ene** (NBME 31 = **D88**): 2 NBME consecutivos ≥68 % + UWSA2 low-risk |
| Secundarios (v5.9, leídos de los `.ts` con `node`) | MIR 15:15-16:15 (**78 días**, 11-sep → mié 30-dic-2026; luego mantenimiento 63 días 4-ene→31-mar-2027, modo reducido hasta el **26-ene**) · ENCAPS 16:15-17:15 (mantenimiento **98 días**, 11-sep → vie 29-ene-2027 = 79 de banqueo + 19 mini-sims de viernes; examen fin-mar 2027, meta ≥17/20; feb-mar vuelve a principal) · LIVIANO 17:15-18:00 (**90 días**, 11-sep → mar 19-ene-2027) · Research↔Derma 13:30-14:15 (alternos: **Research 42 átomos** lun 14-sep → mar 9-feb-2027 · **Derma 70 sesiones** vie 11-sep → vie 26-mar-2027) · AURUM 14:15-15:15 (**130 días**, 11-sep → mar 16-mar-2027) · SYNAPSE 12:30 (30') (**82 días**, 11-sep → mar 1-dic-2026) + vibecoding 04:15 (**60 días L-V**, 11-sep → jue 3-dic-2026, 12 proyectos, 1.er SHIP sáb 19-sep) |
...
| Corrimiento | cada día hábil sin estudiar = +1: `node DATA/_scripts/remap_inicio.js <fecha>` + `gen_encaps_mantenimiento_2027.js` + `gen_synapse_plan.js` + `gen_aurum_plan.js` + regenerar USMLE. **Los overlays de hito NO se mueven** (fechas fijas) — pero **su D# sí cambia** y hay que reescribir la `description` de los 12 (ver §5 y §10). Desde v5.8 los 5 hitos de enero están **anclados por fecha** en el generador |
...
### 0.1 · Semana real de arranque v5.9 — vie 11 → dom 13 sep 2026
La semana de arranque queda reducida a **1 solo día hábil (viernes 11)** — y ese día **es el UWSA1**. Los **13 bloques de estudio** del régimen L-V se verificaron en vivo el 08-sep con `list_events` precisamente sobre el jue 10 y el **vie 11** (04:15 IA · 05:00 Anki AM · el tramo USMLE 07:15-12:00 · 12:30 SYNAPSE · 13:30 Research↔Derma · 14:15 AURUM · 15:15+15:30 MIR · 16:15 ENCAPS · 17:15 LIVIANO · 18:00 eval; son 13 series porque el tramo USMLE de la mañana son 4 y MIR son 2) y **sáb 12 y dom 13 siguen sin banco de preguntas**. El 09-sep y el 10-sep **solo se reescribieron descripciones**: no se creó, borró ni movió ningún evento, así que esa verificación de estructura sigue vigente.

| Día | D# USMLE | Bloques de estudio (13/13 series) | Hito |
|---|---|---|---|
| ~~Jue 10-sep~~ | — | **NO estudiado** — origen del corrimiento a v5.9 | — |
| **Vie 11-sep** | **D1** — Assessment · nivelUW 5 · 160Q · **el D1 es el UWSA1** · Derma **d1** en la franja 13:30 | 04:15 IA vibecoding · 05:00 Anki AM · 07:15 repaso · 08:15 pre-test · 09:00 deep prime · 11:00 30Q · 12:30 SYNAPSE · 13:30 Research↔Derma · 14:15 AURUM · 15:15+15:30 MIR · 16:15 ENCAPS · 17:15 LIVIANO · 18:00 eval — **el overlay 🎯 UWSA1 09:00-13:00 pisa deep prime, 30Q, almuerzo y SYNAPSE** (ver §5) | 🎯 **UWSA1 BASELINE** |
...
| **Lun 14-sep** | **D2** — Fundamentos · Pathoma 1-2 (lesión/muerte celular + inflamación) · nivelUW 1 · 30Q · Research **D1** en la franja 13:30 | los 13 bloques, sin overlay | — |

⚠ **Viernes 11**: GYM 19:00-20:00 + BAILE 20:00-20:30, con el UWSA1 ya cerrado por la tarde (la revisión completa del sim va en las franjas USMLE de la tarde + 18:00).
⚠ **El D1 del plan es un hito y eso es lo correcto**: Palmerton pide la línea base *antes* de tocar contenido. Pathoma 1-2, que en v5.8 era el D1, pasa a ser el **D2 del lun 14-sep**; **nada se perdió** — todo el temario se desplazó un día hábil y el plan se alargó por la cola.
...
**Las FECHAS no se movieron con el corrimiento v5.9 — los D# sí (todos exactamente −1).** Cada D# de la tabla se leyó con `node` del array `DIAS` de `src/lib/usmleStep1Daily.ts` (95 entradas, D1 = 2026-09-11 → D95 = 2027-01-26), no se estimó. El **10-sep-2026 se reescribió el campo `description` de los 12 overlays** para sustituir el D# viejo y la referencia "D1 = jue 10-sep / v5.8" por "**D1 = vie 11-sep, v5.9, 95 días**"; **el protocolo de test-day de Palmerton se conservó íntegro** (§F de `DATA/USMLE/PALMERTON_POR_MATERIA.md`): Ziploc BREAK 1/2/3 preparadas la noche anterior (proteína magra + grasas, **sin carbohidratos simples**), cronograma de descansos (sit-in 1-2' / activo 10' / almuerzo dentro del sim), sin cambiar respuestas por ruido, SYNAPSE de ese día a la tarde, registro de agua/sueño en VITALS como ensayo, y el mapa del día real de 7 bloques. Horas, títulos, `colorId`, `transparency` y la ausencia de recurrencia quedaron intactos (verificado con `get_event` tras cada `update_event`).

| # | Fecha | Hora | Título literal | ID | **D# v5.9** | D# v5.8 (histórico) | Mínimo on-track (texto del overlay) |
|---|---|---|---|---|---|---|---|
| 1 | 11-sep-2026 (vie) | 09:00-13:00 | 🎯 UWSA1 — BASELINE Step 1 (160Q · 4 bloques) | `o1gla7846uae4tgngvc4q45osg` | **D1** ⚡ | D2 | línea base — **y ahora el DÍA 1 del plan** (Palmerton: baseline en el primer día) |
| 2 | 02-oct-2026 (vie) | 07:15-11:00 | 🎯 NBME 25 (200Q) + revisión | `4hjv5lkvluj06ahc2qndtsi6as` | **D16** | D17 | ≥51 % |
| 3 | 23-oct-2026 (vie) | 07:15-11:00 | 🎯 NBME 26 (200Q) + revisión | `lr7ktrbiffj4hrlke7lh7cv6h4` | **D31** | D32 | ≥54 % |
| 4 | 13-nov-2026 (vie) | 07:15-11:00 | 🎯 NBME 27 (200Q) + revisión | `sm4baa2v453ifaub325h9v08mg` | **D46** | D47 | ≥57 % (gate 1 ECFMG pide ≥55 %) |
| 5 | 04-dic-2026 (vie) | 07:15-11:00 | 🎯 NBME 28 (200Q) + revisión | `ecu784689p8osrhqabuct8d5jg` | **D61** | D62 | ≥61 % |
| 6 | 18-dic-2026 (vie) | 07:15-11:00 | 🎯 NBME 29 (200Q) + revisión | `ae93qv0nqqs36h439hid1jcq8o` | **D71** | D72 | ≥63 % |
| 7 | 30-dic-2026 (mié) | 07:15-11:00 | 🎯 NBME 30 — CIERRE FASE A (200Q) + plan Fase B | `mael3p9uhp036jbep45ql6v9oc` | **D78** | D79 | ≥65 % · cierra Fase A (quedan 2 días de contenido: D79 y D80) |
| 8 | 08-ene-2027 (vie) | 09:00-13:00 | 🎯 UWSA2 — predictor gold-standard (160Q) | `2u7viv0elr6aedo0m9undfa5kg` | **D83** | D84 | low-risk |
| 9 | 15-ene-2027 (vie) | 07:15-11:00 | 🎯 NBME 31 (200Q) — GO/NO-GO fecha de examen | `l771lvcfv0jcebt61do6svia18` | **D88** | D89 | **GO = 2 NBME ≥68 % + UWSA2 low-risk** · cierra Fase B |
| 10 | 18-ene-2027 (lun) | 07:15-11:00 | 🎯 NBME 32 (200Q) + repaso FA 1-5 | `h65e772pqsa4hgc5b6bh9n8h50` | **D89** | D90 | ≥68 % · sprint (arranca la Fase C) |
| 11 | 20-ene-2027 (mié) | 07:15-11:00 | 🎯 NBME 33 (200Q) + repaso FA 11-14 | `la5rsbuiqj1o35lb5adf93abuo` | **D91** | D92 | ≥68 % · sprint (Fase C) |
| 12 | 22-ene-2027 (vie) | 07:15-11:00 | 🎯 FREE 120 oficial + logística del examen | `lh9jfjsmoif74ci6jcn8f1mq30` | **D93** | D94 | ≥70 % (heurística comunitaria) · último ensayo · **no es el último día del plan** |
...
⚠ **Cambio estructural que v5.9 hereda de v5.8**: el plan no termina con el Free 120. El **último día es el mar 26-ene-2027 = D95** (rapid review de First Aid + Anki + laboratorio de dudas), **sin overlay** porque no es un simulacro; cae dentro de la ventana de examen 25-29 ene (target mié 27) y es además la **víspera del examen** → ese día va en versión ligera (taper + preparar las bolsas). El **lun 25-ene = D94** (repaso FA sistemas 6-10 + Anki marathon + incorrects). Ambas líneas ya están dentro de la descripción del overlay Free 120.
⚠ Tras el 30-dic (D78) los feriados jue 31-dic y vie 1-ene quedan fuera del plan: **el siguiente día hábil es el lun 4-ene = D79**, el **1.º** de los 2 días dobles de Bioquímica; el **mar 5-ene = D80** **cierra la Fase A** con el 2.º (biología molecular + genética + farmacología general/toxicología). La **Fase B arranca el mié 6-ene = D81**. Esta corrección ya está dentro de la descripción del overlay NBME 30.
...
## 6 · Descripciones de los bloques — releídas en vivo con `get_event` el **10-sep-2026**
Lo que sigue es un **resumen fiel de 1-2 líneas por serie** sobre el texto **vivo** leído hoy con `get_event`. El 10-sep se reescribió la `description` de **12 series** (10 de estudio/secundarias + 6 de rutina/comida/baile — más los 12 overlays de hito del §5). En todas ellas se tocó **solo `description`**: RRULE, `start`/`end`, `summary` y `colorId` verificados intactos con `get_event` tras cada `update_event`. **Ninguna serie se borró ni se recreó y no se usó `recurrenceData`.**

**Estado:** 🟢 = releída en vivo el 10-sep · ⚪ = no releída hoy (última captura 05-sep/08-sep/09-sep, sin cambios conocidos).
**⏸ SIN CAMBIO (releídas el 10-sep, no tocadas):** 4 series de estudio **siguen sin citar ni la versión del régimen ni D1 ni el nº de días**, así que no había nada que corregir y se dejaron intactas: **repaso 07:15** `54lchqggik96dmljmmg3l88s54` · **pre-test 08:15** `3tbecd5n03ut6lno3hjvc1sr7k` · **30Q 11:00** `2eqmmrnh00jr44plevurgcu2as` · **eval 18:00** `utk2laeob9u0847bbe9rm491v4`.

### 6.1 · Bloques de estudio (L-V)

| Hora · Serie | ID | Resumen fiel del texto VIVO (10-sep) | Estado |
|---|---|---|---|
| 04:15-05:00 · 🧠 IA VIBECODING | `udr09j9ng983o0d4nipkfe4494` | Curso de IA en modo BUILDER, no programar desde cero: 1 proyecto real terminado por semana al servicio de sus sistemas; ciclo diario 5' objetivo → 35' construir con Claude Code → 5' commit + nota (repo synapse-journal). **Cierre v5.9 (10-sep):** "12 proyectos · 60 días L-V · **vie 11-sep → jue 3-dic-2026** · 1er SHIP sáb 19-sep (S1 = fix del parser APEX)", plan en `src/lib/vibecodingPlan.ts` (`VIBE_META` leído con node); sáb 15:00-17:00 = SHIP, dom = Feynman del proyecto (10', opcional). | 🟢 |
| 05:00-05:45 · ANKI AM | `i8afj7uppkb3ntj8h9890dhecc` | Anki de madrugada (Palmerton, mente fresca) — Fase A: 45' FSRS deck USMLE; Fases B-C: 05:00-05:12 🔥 Stress Set 10Q + 05:12-05:45 Anki. **Cifra corregida el 10-sep:** "régimen v5.9: 95 días desde **vie 11-sep** hasta **mar 26-ene-2027** ≈ 594h" (95 × 6h15 = 593,75 h). Se corrigieron además los rangos de fase del texto: Fase A = sep-**ene** (llega al 4-ene) y Fases B-C = **ene**. FSRS retención 0.90, máx 50 nuevas/día, solo Good/Again. | 🟢 ⚠ UNTIL |
...
| 09:00-11:00 · DEEP PRIME | `cb2uh20jnvu7pgfev4183pgctc` | Bloque sagrado de 2h: vídeo 09:00-10:00 (B&B/Pathoma/Sketchy a 1.5x) · First Aid active reading 10:00-10:45 · ≤10 tarjetas de mecanismo + APEX 10:45-11:00; jerarquía de material; "💧 AL ABRIR" con 500 ml + snack y micro-pausa 10:00-10:05; los viernes de hito este bloque es el UWSA/NBME. **Cierre v5.9 (10-sep):** 95 días, **D95 = mar 26-ene-2027**, fases **A D1-D80 / B D81-D88 / C D89-D95**, la frase nueva "**el D1 (vie 11-sep) es el UWSA1** — baseline en el primer día, como prescribe Palmerton — y el contenido arranca el lun 14-sep (D2)", la frase "ni un tema se fusionó ni se recortó en el corrimiento: el desfase se absorbió alargando el final del plan", examen 25-29 ene, GO/NO-GO vie 15-ene (**NBME 31 = D88**). | 🟢 ⚠ UNTIL |
...
| 12:30-13:00 · SYNAPSE misión | `j99thg3eaqesosmvppj4rfgvh4` | Misión del día en la app (A · 15' lección + B · 10' audio + C · 5' lectura móvil; marcar ✓). **Cierre v5.9 (10-sep):** "**82 días · vie 11-sep → mar 1-dic-2026**" (`SYN_PLAN_META` de `src/lib/synapseDailyPlan.ts`, leído con node), sáb 15:00-17:00 PC = SHIP del proyecto del vibecoding de las 04:15, dom = Feynman opcional. Los `<br>` siguen escapados como texto (§10.2 · se conservaron a propósito). | 🟢 |
| 13:30-14:15 · RESEARCH ↔ DERMA | `3ofg2ljv8kl3p1adm2e5d5nih3` | **Mapa de arranque recalculado el 10-sep con `diaEstudioTipo()` de `researchData.ts`** (el ancla de la alternancia sigue siendo el mié 10-jun-2026 y NO se mueve, así que cada corrimiento la invierte): **vie 11 DERMA (d1) · lun 14 RESEARCH (D1) · mar 15 DERMA (d2) · mié 16 RESEARCH (D2) · jue 17 DERMA (d3) · vie 18 RESEARCH (D3) · lun 21 DERMA (d4) · mar 22 RESEARCH (D4)**…, contrastado 1:1 con las fechas reales de `researchDailyPlan.ts` y `dermaDailyPlan.ts`. Planes vigentes: **Research 42 átomos** lun 14-sep-2026 → mar 9-feb-2027 (D38 = mar 29-dic-2026 y luego pausa por el Step 1 hasta el lun 1-feb = D39) y **Derma 70 sesiones** vie 11-sep-2026 → **vie 26-mar-2027** (sin pausa: sigue corriendo en enero); ambos saltan 25-dic, 31-dic y 1-ene. 45' · 1 entregable atómico/día · APEX + Obsidian. | 🟢 |
| 14:15-15:15 · AURUM | `at1nak8f24nbnj1mh2jcd4aggg` | Closer de ventas: 26 semanas · 130 lecciones · 7 fases; solo referentes reales con track record verificable (Hormozi, Cardone, Voss, Cialdini, Rackham, Girard, Tracy, Blount, Ross, Hopkins, Bettger + Josué Peña solo gratis) y 11 libros ancla; cada día 1 vídeo + drill de pitch sobre leads reales (ALLPA / Qori Golden); la lectura va en los huecos de viaje. **Cierre v5.9 (10-sep):** D1 = **vie 11-sep-2026 → D130 = mar 16-mar-2027** (`AURUM_PLAN_META` de `src/lib/aurumDailyPlan.ts`, leído con node). | 🟢 |
| 15:15-15:30 · MIR eval anclada | `2ldp6obaapnvo76li28uprrddg` | 4Q ProMIR/AMIR del tema D-1 en modo examen (~90-100 s/Q, sin compendio) + 5' Anki deck MIR + 3' mini-corrección tipificando el error (conceptual / terminológico / aplicación); umbrales 4/4 avanzar · 3/4 zona gris · <3/4 repetir mañana; conserva "💧 AL ABRIR" y la justificación científica (Karpicke, Cepeda, Newport, Croskerry). **Etiqueta v5.9 (10-sep):** MIR = 78 días (**vie 11-sep → mié 30-dic-2026**); del 4-ene al 31-mar-2027 sigue en banqueo de mantenimiento (**63 días, modo reducido hasta el 26-ene** = último día del plan Step 1, `MIR_MANT_META` de `mirMantenimiento.ts`). | 🟢 |
| 15:30-16:15 · MIR deep work mini | `00k364heibh1n6f9hfspcv9dpi` | 1 subtema atómico/día en 4 sub-fases: pre-test + generation 5' · lectura activa + elaboración 25' · free recall libro cerrado 5' · ≤4 APEX 10' (formato completo FRENTE/REVERSO/CCSN/FISIOPATOLOGIA/RELACIONES, ≥1 comparativa MIR vs ENCAPS); identity stacking y métricas en Obsidian 06_MIR/Subtemas. **FUNDAMENTO recalculado el 10-sep:** "78 días × 1 subtema/día × ≤4 cards ≈ 310 cards MIR sólidas al **mié 30-dic-2026** (76 temas + D77 mini-MIR + D78 corrección)" + etiqueta **v5.9 (D1 = vie 11-sep-2026)**. | 🟢 |
| 16:15-17:15 · ENCAPS mantenimiento | `papebi46etlo8glgfs5akd5mig` | 16:15-16:30 eval anclada 5Q del tema de ayer · 16:30-17:15 banco 20-25Q por rotación semanal ponderada (II 30 % · I 27 % · V 21 % · III 13 % · IV 9 %) · viernes mini-simulacro 25Q a 72 s/Q · 8 críticos v3 · registrar CADA fallo en TRACKING_ERRORES; conserva el "🥤 CIERRE 17:10-17:15" de cena ligera. **Cierre v5.9 (10-sep):** **98 días** vie 11-sep-2026 → vie 29-ene-2027 (Supabase `study_schedule`, backup **`study_schedule_bk_0910`**, **79 días de banqueo + 19 mini-sims de viernes**; cifras contadas con node y contrastadas con `DATA/_scripts/_encaps_mantenimiento_2027.sql`), la nota de que **el corrimiento se comió otro día de mantenimiento** porque el cierre del 29-ene es fijo (**99 → 98**), la frase "ningún tema ni sub-eje se fusionó ni se recortó — cada fila solo se desplazó a la siguiente fecha del plan", y **la fase intensiva NO se mueve: sigue arrancando el lun 1-feb-2027, ahora como día 99**. | 🟢 🔴 UNTIL |
| 17:15-18:00 · LIVIANO | `8epae6hlfmrc9j0h2kib7iuc84` | 17:15-17:40 módulo del día (fisiología del peso · GLP-1/tirzepatida · nutrición · ejercicio · conducta · farmacología/bariátrica) · 17:40-18:00 aplicación explicándolo como a un paciente real; fuentes ProMIR + AMBOSS + guías AACE/OMA; conserva "🥤 ARRANQUE 17:15" y la coherencia LIVIANO (última comida sólida ≥3h antes de dormir). **Cierre v5.9 (10-sep):** 90 días L-V **vie 11-sep-2026 → mar 19-ene-2027** (`LIV_META` de `src/lib/livianoStudyPlan.ts`; contados con node: 16 casos en viernes · **19** pre-tests de lunes · 4 drills de cifras ancla; salta 25-dic, 31-dic y 1-ene). | 🟢 |
...
### 10.2 · Pendiente REAL (estado al 10-sep-2026)
