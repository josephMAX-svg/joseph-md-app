...
> Contexto: régimen **v5.16 (D1 = lun 28-sep-2026; tampoco se estudiaron el mié 23, jue 24 ni vie 25-sep, decimoquinto corrimiento 31-ago→28-sep, 20 hábiles; corrimiento RÍGIDO — solo corren los días, ningún tema se toca; el Step 1 termina el mié 10-feb-2027 = D95 = último día del plan y D-1 real, su D94 mar 9-feb es la última sesión de banco y el examen pasa al jue 11-feb-2027)** apoya 95 días de Step 1 en "≤10 tarjetas de MECANISMO/día + APEX" — exactamente el contenido multilínea que el parser perdía. Verificado el 05-sep que los 3 ficheros seguían con mtime 07-may y el bug intacto; corregidos entonces. Vibecoding arranca con el **START mié 23-sep-2026** (fechas exactas de S1 en `vibecodingPlan.ts`, no releídas en esta pasada de docs; en v5.14 era lun 21 → vie 25-sep con SHIP sáb 26, en v5.13 jue 17 → mié 23, en v5.12 mié 16 → mar 22, en v5.11 mar 15 → lun 21, mismo SHIP; en v5.10 lun 14 → vie 18, SHIP sáb 19) = validar esto en vivo.
...
Sin cambios hoy: P0-1 (scheduler), P0-4 (cleanup loop), P1-2 (drift de plan — ahora el plan vigente es **v5.16, D1 = lun 28-sep-2026** (26-sep-2026), Step 1 principal con examen target jue 11-feb-2027; `config/fases.json` sigue con `FASE_4.is_current_phase=true` y `FASE_7.inicio=2026-10-01`, mientras `CLAUDE.md` ya dice FASE_7 desde el 7-sep — al alinearlo, `FASE_7.inicio` debe ser `2026-09-28`), seguridad §4 (**sigue abierta**: `datos_tesis` RLS OFF + anon key en repo).
