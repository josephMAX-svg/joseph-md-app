...
> Contexto: régimen **v5.12 (D1 = mié 16-sep-2026; el mar 15-sep tampoco se estudió, undécimo corrimiento 31-ago→16-sep; el Step 1 llena la ventana 25-29 ene y su examen pasa al lun 1-feb-2027)** apoya 95 días de Step 1 en "≤10 tarjetas de MECANISMO/día + APEX" — exactamente el contenido multilínea que el parser perdía. Verificado el 05-sep que los 3 ficheros seguían con mtime 07-may y el bug intacto; corregidos entonces. Vibecoding S1 (**mié 16-sep → mar 22-sep, SHIP sáb 26-sep**, fechas leídas de `vibecodingPlan.ts` con node el 15-sep; en v5.11 era mar 15 → lun 21, mismo SHIP; en v5.10 lun 14 → vie 18, SHIP sáb 19) = validar esto en vivo.
...
Sin cambios hoy: P0-1 (scheduler), P0-4 (cleanup loop), P1-2 (drift de plan — ahora el plan vigente es **v5.12, D1 = mié 16-sep-2026** (15-sep-2026), Step 1 principal con examen target lun 1-feb-2027; `config/fases.json` sigue con `FASE_4.is_current_phase=true` y `FASE_7.inicio=2026-10-01`, mientras `CLAUDE.md` ya dice FASE_7 desde el 7-sep — al alinearlo, `FASE_7.inicio` debe ser `2026-09-16`), seguridad §4 (**sigue abierta**: `datos_tesis` RLS OFF + anon key en repo).
