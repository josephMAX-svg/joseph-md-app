...
export const USMLE_CHECKPOINTS: UsmleCheckpoint[] = [
  { form: 'NBME CBSSA Forms 25–33', kind: 'NBME', when: 'plan v5.11: 7 in Phase A (every ~3 weeks) · NBME 31 (D86, vie 15-ene) = GO/NO-GO · 32/33 final week', predictor: 'Best single predictor of a Step 1 pass (Palmerton: the ONLY one — UWorld % is a vanity metric)', band: '≥65% ≈ 95% pass · ≥70% ≈ 99% · plan GO = 2 consecutive ≥68% · on-track minimums per milestone in usmleScores.HITOS_ONTRACK', url: 'https://www.nbme.org/examinees/self-assessments', gated: true },
  { form: 'USMLE Free 120', kind: 'FREE120', when: '1–2 weeks pre-exam (plan: D91, vie 22-ene; D94-D95 27-28 ene = taper, D95 = D-1 · target exam vie 29-ene) — ideally at the real Prometric', predictor: 'Current NBME item wording · no official explanations → audit the PROCESS, not theory', band: '≥70% heuristic (community, not Palmerton) · 3 blocks of 40, timed', url: 'https://www.usmle.org/prepare-your-exam', gated: false },
  { form: 'UWorld Self-Assessment 1–2', kind: 'UWSA', when: 'UWSA1 = day-1 baseline (D1, mar 15-sep) · UWSA2 = D81 (vie 8-ene)', predictor: 'Systematically OVER-predicts (Palmerton, same authors as the bank) — endurance only, never a date decision', band: 'trend > absolute number · the date is decided by NBMEs', url: 'https://www.uworld.com/', gated: true },
...
export const USMLE_READINESS = {
  ...
  next: 'UWSA1 mar 15-sep (D1, baseline) → luego NBME 25/26/27/28/29 cada ~3 semanas (plan v5.11). Registra el % de cada hito en 📏 Medición (Cola de hoy): el gauge se ancla al último hito y muestra la distancia al mínimo on-track.',
};
