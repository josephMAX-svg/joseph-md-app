...
// v5.14 (19-sep): SOLO FALLBACK (antes fijo: 102 → 95 → 94 → 92). El valor vivo es study_metrics.extra.dias_ciclo (92 = lun 28-sep-2026
// → vie 5-feb-2027; desde v5.15 el corrimiento es RÍGIDO: el fin del ciclo SÍ se mueve y se amplía por detrás para no perder sesiones) y
// crece solo con max(dia) de study_schedule cuando se siembre la FASE INTENSIVA (modo='INTENSIVO' · gen_encaps_intensivo_2027.js).
export const STUDY_TOTAL_DAYS: Record<string, number> = { ENCAPS: 92 }; // v5.16: siguen 92 (el fin se alargó al vie 5-feb para NO perder sesiones)
...
export function useEncapsPlan(examen: string = 'ENCAPS'): UseEncapsPlan {
  ...
  // v5.14 (19-sep): D1 y total VIVOS — study_metrics.extra.d1 / dias_ciclo (hoy 2026-09-28 / 92) → fila dia=1 y max(dia) de
