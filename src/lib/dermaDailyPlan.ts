...
export const DERMA_DAILY_META = {
  ...
  postStep1D1: 50,
  /** Ventana del taper Step 1 (inclusive). v5.13 (16-sep): el taper es POSICIONAL (d44-d49) y con D1 Derma = jue 17-sep cae del mar 19-ene (día tras el NBME 32)
   *  al mar 2-feb (= DÍA DEL EXAMEN Step 1 en v5.13, sesión opcional); el vie 15-ene (NBME 31) es d43 = sesión normal (Mohs). Alternativa registrada en PENDIENTES: swap d43↔taper. */
...
const TAPER_STEP1 = (nota: string): DermaTaper => ({ modo: 'step1', motivo: 'Ventana v5.13: mar 19-ene (día tras el NBME 32) → mar 2-feb (día del examen Step 1)', casos: 1, nota });
/**
...
export const DERMA_DIAS: DiaDerma[] = [
  ...
  { d: 46, fecha: '2027-01-25', bloque: B.X, bKey: 'X', tier: 'MED', sub: 'TAPER Step 1 (última semana completa del plan Step 1, D90-D94) · Contorno corporal (criolipólisis, HIFU) + escleroterapia básica (ambos en el temario CORE surgical) — átomo ligero X absorbido en la ventana del examen (antes d66)', referente: null,
    ...
    atlasUrl: `${DN}/topics/sclerotherapy`, dermatoscopiaImg: DSP.glob, taper: TAPER_STEP1('Átomo ligero MED (X) en la última semana completa del plan Step 1 (lun 25-ene, D90): 1 caso ciego + FSRS de fallos + lectura ligera 10′.') },
  { d: 47, fecha: '2027-01-27', bloque: B.H, bKey: 'H', tier: 'MED', sub: 'TAPER Step 1 · Segunda pasada parcial I: 1 caso ciego + repaso FSRS de los fallos del ledger de los módulos A-D (0 lectura nueva — última semana del plan Step 1)', referente: null,
    ...
    atlasUrl: IMG_LIB, dermatoscopiaImg: DSA, taper: TAPER_STEP1('Última semana completa del plan Step 1 (mié 27-ene, D92): 0 lectura nueva. 1 caso ciego + repaso FSRS de los casos y preguntas fallados de los módulos A-D (ledger), en voz baja y sin cronómetro.') },
  { d: 48, fecha: '2027-01-29', bloque: B.H, bKey: 'H', tier: 'MED', sub: 'TAPER Step 1 · Segunda pasada parcial II: 1 caso ciego + repaso FSRS de los fallos de los módulos E-G + drill HDPH mental sin cronómetro (vie 29-ene = D94, última sesión de banco del Step 1; finde libre; lun 1-feb = D95 = D-1)', referente: null,
    ...
    atlasUrl: DN_QUIZ, dermatoscopiaImg: DSP.three, taper: TAPER_STEP1('Taper final del Step 1 (vie 29-ene = D94 = última sesión de banco; finde libre; lun 1-feb = D95 = D-1; examen mar 2-feb): 0 lectura nueva. 1 caso ciego + FSRS de fallos de los módulos E-G + drill HDPH mental (recitar, sin cronómetro).') },
  { d: 49, fecha: '2027-02-02', bloque: B.H, bKey: 'H', tier: 'MED', sub: 'TAPER Step 1 · Segunda pasada parcial III — DÍA DEL EXAMEN Step 1 (mar 2-feb; sesión OPCIONAL: 1 caso ciego + 5′ FSRS; si se salta no se pierde nada, los fallos siguen en la cola)', referente: null,
    ...
    atlasUrl: IMG_LIB, dermatoscopiaImg: DSA, taper: TAPER_STEP1('DÍA DEL EXAMEN Step 1 (mar 2-feb): sesión OPCIONAL. Si hay energía por la tarde: 1 caso ciego + 5′ de FSRS. Si se salta, no se pierde nada: los fallos siguen en la cola y el ciclo continúa el jue 4-feb (d50).') },
