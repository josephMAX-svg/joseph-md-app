...
export const DERMA_DAILY_META = {
  ...
  nota: 'PLAN ÉLITE v3: cada sesión = casos CIEGOS fijos (casoIds, permutación seeded de los 200: 2/sesión · 1 en el taper d44-d49 · 3 desde d50) + 1 imagen dermatoscópica ciega + ~10Q review (presupuesto por banco DERMA_BANCOS, cursor "retoma en Q#" desde el ledger; fallos etiquetados med/ped/surg/path) + 10′ lectura del módulo (o módulo DermNet Dermoscopy CME en las pares d6-d44). TAPER Step 1 d44-d49 (v5.15: lun 25-ene → lun 8-feb, d49 = día del examen): 1 caso + FSRS de fallos + 0 lectura nueva. Progreso REAL marcable (studyProgress key "derma"). El día mostrado salta los días-Research. Ciclo 2 (d74-d103) en dermaCiclo2.ts.',
...
export const DERMA_DIAS: DiaDerma[] = [
  ...
  { d: 49, fecha: '2027-02-08', bloque: B.H, bKey: 'H', tier: 'MED', sub: 'TAPER Step 1 · Segunda pasada parcial III — DÍA DEL EXAMEN Step 1 (lun 8-feb; sesión OPCIONAL: 1 caso ciego + 5′ FSRS; si se salta no se pierde nada, los fallos siguen en la cola)', referente: null,
    ...
    morfologia: 'Repaso', sitio: 'Difuso', ddx: ['Sesión opcional: el examen Step 1 dura ~8 h (lun 8-feb); Derma solo si queda energía real por la tarde', 'Si se hace: 1 caso ciego + FSRS de las tarjetas vencidas (5′) y cerrar', 'Nada se recorta: el ciclo sigue el jue 4-feb con d50 (cicatrización, cierre del módulo G)'],
